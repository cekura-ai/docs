...
Cekura's MCP server exposes documentation search and Cekura API operations through the US endpoint by default:
...
Use the endpoint that matches your Cekura workspace region. Keep the US endpoint above as the default unless your workspace is hosted in India or EU.

| Workspace region | MCP endpoint |
|---|---|
| US (default) | `https://api.cekura.ai/mcp` |
| India | `https://api.in.cekura.ai/mcp` |
| EU | `https://api.eu.cekura.ai/mcp` |

For India or EU, replace the US URL in the client examples below with the matching regional endpoint.
...
<Tabs>
  ...
  <Tab title="Codex">
    ...
    The example uses the US default. For a regional Codex connection, use one of these commands instead:

    ```bash
    # India
    codex mcp add cekura-india --url https://api.in.cekura.ai/mcp
    codex mcp login cekura-india

    # EU
    codex mcp add cekura-eu --url https://api.eu.cekura.ai/mcp
    codex mcp login cekura-eu
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="Tools are not showing up">
    Restart your assistant after changing MCP config. Confirm the URL matches your workspace region in the [regional endpoint table](#regional-endpoints), check JSON or TOML syntax, and inspect your assistant's MCP logs.
  </Accordion>
