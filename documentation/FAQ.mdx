...
### Creating a Neutral Female Baseline Personality

If you want a calm, neutral female voice matching the characteristics of the default *Normal Male* personality (no background noise, no interruptions, clear speech), create a custom personality: go to **Personalities → Create Personality**, configure it with the same behavioural settings as Normal Male, and select a female voice ID in the **Voice ID** field. See [Creating Custom Personalities](/documentation/advanced/creating-custom-personalities) for full setup steps.
...
<Note>
**Admin users automatically have access to all projects.** If you try to add a user with the Admin role to a project and see that they already appear as a member, this is expected behaviour — Admins can view and access all projects in the organisation by default. For a full breakdown of Admin, Member, and Viewer permissions, see [Enterprise Setup](/documentation/advanced/enterprise-setup).
</Note>
