...
<AccordionGroup>
  ...
  <Accordion title="Provider Chat">
    ...
    Provider chat is configured automatically using your voice agent settings. However, **Bland AI** requires a pathway ID for chat. **Retell** chat works automatically — when no chat agent ID is provided, a temporary chat agent is cloned from the voice agent for each run. You can optionally provide a specific Retell chat agent ID to point chat runs at a dedicated agent.
