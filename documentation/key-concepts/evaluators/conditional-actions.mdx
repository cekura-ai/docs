...
<Note>
**Tag Validation**: All tags are validated when you create or update a conditional actions evaluator. If a tag has an invalid format, missing required attributes, or invalid attribute values, you will receive a validation error. Make sure to follow the exact syntax shown in the examples below.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Tag Validation Errors" icon="circle-exclamation">
    **Issue**: You receive a validation error when creating or updating a conditional actions evaluator.

    **Common causes and solutions**:
    - **Escaped quotes**: Do not use escaped quotes like `time=\"2s\"`. Use normal quotes: `time="2s"` or `time='2s'`
    - **Missing required attributes**: Tags like `<silence>`, `<hold>`, `<dtmf>`, and `<ivr>` require specific attributes (e.g., `time`, `digits`, `text`)
    - **Invalid attribute values**: Ensure values are in the correct format (e.g., time as `"2s"`, volume ratio between 0.5 and 2.0)
    - **Incorrect closing format**: Self-closing tags must end with `/>` (e.g., `<endcall />`, not `<endcall>`)
    - **Mismatched tags**: Wrapper tags like `<spell>` and `<background_noise>` need both opening and closing tags
  </Accordion>
