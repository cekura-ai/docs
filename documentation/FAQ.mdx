...
No. [Pipecat Chat](/documentation/integrations/pipecat/chat-testing) uses the same Pipecat Cloud deployment that serves voice. The bot only needs the Daily transport and RTVI enabled, which is Pipecat's default. Cekura starts one session per run and sends every turn as text, so no speech-to-text or text-to-speech runs.

## Why does my Pipecat chat transcript show tool calls without a name or arguments?

Pipecat hides function names, arguments, and results in RTVI events by default. Use a Cekura SDK version with text-session support and let it build the pipeline task, or set `function_call_report_level` to `FULL` in your `rtvi_observer_params`. See [Pipecat Chat Testing](/documentation/integrations/pipecat/chat-testing#troubleshooting).
