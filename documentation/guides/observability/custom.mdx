...
<Tabs>
  <Tab title="Custom API">
    <Note>
      `transcript_type` must be one of `cekura`, `vapi`, `retell`, `elevenlabs`, `livekit`, `pipecat`, `deepgram`, `none-mono`. There is no `custom` value. Use `cekura` for your own transcript format, where the only valid roles are `"Testing Agent"` (the caller) and `"Main Agent"` (the agent under test). See the [Send Calls API reference](/api-reference/observability/send-calls#body-transcript-type).
    </Note>
