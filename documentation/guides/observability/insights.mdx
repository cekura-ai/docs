...
<Note>
Generating insights costs **10 credits per metric that produces an insight**. Metrics with no failing calls in the analysis window produce nothing and are not charged — you only pay for the insight cards you receive. If a view filter is active, only visible metrics are processed. If your balance is insufficient, the request returns a 402 error and no insights are queued.
</Note>
