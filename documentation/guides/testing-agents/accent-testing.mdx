...
<Steps>
  <Step title="Create a personality per accent variant">
    ...
    The accent comes entirely from the **voice ID** — the `accent` shown on a personality is a read-only label derived from it, not a setting, and writing an accent into the personality prompt does not change pronunciation.

    See [Creating Custom Personalities](/documentation/advanced/creating-custom-personalities) for step-by-step instructions.

    <Note>
    **Need an accent or voice that isn't in the catalog?** The available voices are not a hard limit — Cekura can add voices, accents and languages on request. Reach out at **support@cekura.ai** or via your dedicated Slack support channel with the accent or voice you need.
    </Note>
  </Step>
