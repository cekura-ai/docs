...
**Requirements:**
- Add key-value pairs to your test profile
- Header keys must start with `"X-"` (e.g., `"X-Auth-Token"`, `"X-Customer-Type"`)
- Only keys with the `"X-"` prefix are included as SIP headers
...
- Note: `customer_name` is NOT included as a header (doesn't start with "X-")
...
**Required field:**
- **SIP Endpoint**: The SIP URI where your agent receives calls, following RFC 3261 format: `sip:user@host[:port][;params][?headers]`
  - Examples: `sip:agent@yourdomain.com`, `sips:agent@secure.example.com`, `sip:192.168.1.100:5060`, `sip:user@domain.com;transport=udp`, `sip:user@domain.com?Subject=call`
...
```json
{
  "sip_endpoint": ["Invalid SIP endpoint format. Expected: sip:user@domain.com (supports sips:, ports, ;transport=udp, ?headers)"]
}
```
