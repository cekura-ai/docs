...
| Method | Endpoint |
|--------|----------|
| `GET` | `https://new-prod.cekura.ai/test_framework/v1/results-external/:result-id/` |
...
```bash Curl
curl -X GET https://new-prod.cekura.ai/test_framework/v1/results-external/:your-result-id/ \
...
def get_result(api_key, result_id):
    url = f'https://new-prod.cekura.ai/test_framework/v1/results-external/{result_id}/'
    headers = {
...
async function getResult(apiKey, resultId) {
  const url = 'https://new-prod.cekura.ai/test_framework/v1/results-external/' + resultId + '/';
