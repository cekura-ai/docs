...
**Simulating caller muting**: To test how your agent handles a user who goes silent or mutes their microphone, use `<silence>` (the agent can interrupt after the silence starts) or `<hold>` (the silence is guaranteed for the full duration regardless of what the agent says). There is no separate `<mute>` tag — silence and hold cover this use case.
</Note>

<Note>
For pre-built tests of common event-loop behaviors — including hold periods, interruptions, and silence handling — see [Infrastructure Suite](/documentation/guides/testing-agents/infrastructure-suite). It is the recommended starting point when validating these behaviors after migrating between voice providers.
</Note>
