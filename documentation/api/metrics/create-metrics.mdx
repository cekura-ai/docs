...
| `evaluation_trigger` | string | No | When to evaluate the metric: `always`, `automatic`, or `custom`. Defaults to `always` |
| `trigger_type` | string | No | Type of trigger evaluation when evaluation_trigger is `custom`: `llm_judge` or `custom_code`. Defaults to `llm_judge` |
| `evaluation_trigger_prompt` | string | No | Prompt for evaluation trigger when trigger_type is `llm_judge` |
| `evaluation_trigger_custom_code` | string | No | Python code for evaluation trigger when trigger_type is `custom_code`. See [Custom Code Triggers](/documentation/key-concepts/metrics/python-code#custom-code-triggers) |
