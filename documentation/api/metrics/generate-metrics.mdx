...
| Method | Endpoint |
|--------|----------|
| `POST` | `https://new-prod.cekura.ai/test_framework/v1/metrics-external/generate_metrics/` |
...
```bash Curl
curl -X POST https://new-prod.cekura.ai/test_framework/v1/metrics-external/generate_metrics/ \
...
def generate_metrics(api_key, agent_id, num_metrics):
    url = 'https://new-prod.cekura.ai/test_framework/v1/metrics-external/generate_metrics/'
...
async function generateMetrics(apiKey, agentId, numMetrics) {
  const url = 'https://new-prod.cekura.ai/test_framework/v1/metrics-external/generate_metrics/';
