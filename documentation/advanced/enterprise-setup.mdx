...
<Tip>
With project-scoped API keys, you can omit the `project_id` parameter on most endpoints. Cekura infers the project from your API key. This works for both read operations (listing agents, scenarios, metrics, results, dashboards, call logs, and cron jobs) and write operations (creating dashboards, metrics, or evaluators). If you provide a `project_id` explicitly, that value takes precedence.
</Tip>
