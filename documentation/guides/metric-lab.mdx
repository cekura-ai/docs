...
The Metric Lab workflow described above uses the Cekura dashboard UI. The same feedback can be submitted programmatically for integration into CI/CD pipelines or automated review workflows.

**For test run metrics** — use the CLI or Python SDK to submit thumbs-up/down votes with optional expected values and free-text feedback:
...
**For production call logs (Observability)** — the workflow has two steps:
...
<Note>
To schedule automatic metric optimisation that runs the Auto Improve step on a recurring cadence — without any UI clicks — see [Auto-optimise Metrics on a Schedule](/mcp/auto-optimize-metrics).
</Note>