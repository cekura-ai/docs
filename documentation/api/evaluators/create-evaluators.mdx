...
| Method | Endpoint |
|--------|----------|
| `POST` | `https://new-prod.cekura.ai/test_framework/v1/scenarios-external/` |
...
```bash Curl
curl -X POST https://new-prod.cekura.ai/test_framework/v1/scenarios-external/ \
...
async function createEvaluator(apiKey, updateData) {
  const url = 'https://new-prod.cekura.ai/test_framework/v1/scenarios-external/';
