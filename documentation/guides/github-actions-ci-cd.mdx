...
<AccordionGroup>
  ...
  <Accordion title="Tests take 5+ minutes to complete after the call ends">
    If your CI/CD run shows the evaluation stuck in `running` status for around 5 minutes after the call has finished, the most common cause is Cekura waiting for a post-call webhook from your agent platform that never arrives. This is not a queue or priority issue — there is no run prioritisation feature.

    See [Why does my simulation stay in "running" status for 5 minutes even after the call ends?](/documentation/FAQ#why-does-my-simulation-stay-in-running-status-for-5-minutes-even-after-the-call-ends) in the FAQ for the full explanation and resolution steps (disabling the webhook requirement in Agent Settings, or fixing the webhook delivery from your CI/CD pipeline).
  </Accordion>
