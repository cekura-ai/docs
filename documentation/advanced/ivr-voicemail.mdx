...
icon: "phone-volume"
iconType: "regular"
description: "Configure Interactive Voice Response (IVR) and voicemail scenarios using Structured Tests and XML tags"
---
...
Learn how to create voicemail and IVR (Interactive Voice Response) scenarios in Cekura using Structured Tests. These powerful features allow you to build voicemail receivers and menu-driven phone systems that respond to DTMF tones.

## Voicemail

### What is Voicemail?

Voicemail systems allow callers to leave messages when the recipient is unavailable. With Cekura, you can create intelligent voicemail receivers that guide callers through the message recording process using Structured Tests.
...
Navigate to the evaluator creation form and click on "Create Evaluator" to begin setting up your voicemail scenario. Select **Structured Tests** as the evaluator type. Structured Tests allow you to define specific behaviors based on caller interactions.
...
**Provider Support:** The `<voicemail>` tag works with all TTS providers (Cartesia, ElevenLabs, etc.)
...
#### Step 3: Enable Receive DTMF

In the final step of evaluator creation, you need to enable the **Receive DTMF** toggle. This allows our testing agent to receive DTMF tones from the user agent side, which is essential for voicemail functionality since your agent needs to send dtmf tones to our testing agent.

Enable the **Receive DTMF** toggle in the evaluator configuration as shown below:

<img src="/images/voicemail/assign_number.png" alt="Step 3: Enable Receive DTMF and Assign Phone Number" />
...
### Nested Tags in Voicemail Messages

Embed inline tags inside the `text` attribute of `<voicemail>` to add pauses, sounds, and timing control within your voicemail greeting. Supported inline tags include `<silence>`, `<break>`, and `<noise>`.

```xml
<voicemail text="<silence time="2s" /> Hello, you've reached support. <break time="1s" /> Please leave a message after the beep." />
```

**Example with multiple nested tags:**

```json
{
  "id": 1,
  "condition": "The main agent starts the conversation",
  "action": "<voicemail text=\"<silence time=\"2s\" /> Hello. <noise sound=\"office\" /> <break time=\"1s\" /> Please leave your message.\" />",
  "type": "standard",
  "fixed_message": true
}
```

**Rules for nested tags:**
- The `<voicemail>` tag must still be the entire action (no text before or after it)
- Only one `<voicemail>` tag is allowed per action
- The `text` attribute cannot be empty when provided
- Nested tags are validated separately by their own validators
...
- **Structured Tests**: Define specific behaviors based on caller interactions
- **Nested Inline Tags**: Embed `<silence>`, `<break>`, and `<noise>` tags within voicemail messages
...
- Ensure the Receive DTMF toggle is enabled for your evaluator
...
#### Step 1: Create Evaluator with Structured Tests

Navigate to the evaluator creation form and select **Structured Tests** as the evaluator type, similar to the voicemail setup.
...
Just like voicemail, IVR systems require our testing agent to receive DTMF tones from the user agent side, since our bot acts as an IVR system.

Enable the **Receive DTMF** toggle in the evaluator configuration. This feature works with all phone number providers, making it easy to set up IVR functionality.
...
### Nested Tags in IVR Messages

Embed inline tags inside the `text` attribute of `<ivr>` to add pauses, sounds, and timing control within your IVR prompts. Supported inline tags include `<silence>`, `<break>`, and `<noise>`.

```xml
<ivr text="<silence time="1s" /> Welcome to our service. <break time="1.5s" /> <noise sound="beep" /> Press one for sales." />
```

**Example with nested tags in a condition:**

```json
{
  "id": 0,
  "condition": "FIRST_MESSAGE",
  "action": "<ivr text=\"<silence time=\"1s\" /> Welcome. <break time=\"1.5s\" /> <noise sound=\"beep\" /> Press one for support.\" />",
  "type": "standard",
  "fixed_message": true
}
```
...
**Rules for nested tags:**
- The `<ivr>` tag must still be the entire action (no text before or after it)
- Only one `<ivr>` tag is allowed per action
- The `text` attribute cannot be empty
- Nested tags are validated separately by their own validators
...
- **Nested Inline Tags**: Embed `<silence>`, `<break>`, and `<noise>` tags within IVR prompts
