...
Both are required: if signaling is allowed but the global media range is not, calls connect with no audio. We also recommend allowing Twilio's Oregon signaling range (`54.244.51.0/30`) so calls keep working if Twilio fails over from Virginia. See [IP Whitelisting: SIP Calls](/documentation/advanced/ip-whitelisting#sip-calls) for a setup checklist and links to Twilio's documentation.
