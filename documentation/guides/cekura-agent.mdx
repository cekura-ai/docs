...
Get browser alerts when the agent finishes a long-running task, hits an error, or needs your input.

<Steps>
  <Step title="Enable notifications">
    Click the bell icon in the top navigation bar, then click **Enable notifications**. Your browser prompts you to allow notifications from Cekura.
  </Step>
  <Step title="Receive alerts">
    When the agent completes, fails, or waits for clarification, a browser notification appears. Click it to jump to the conversation.
  </Step>
</Steps>

Disable notifications any time from the bell icon menu or your browser settings.
