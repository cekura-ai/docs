...
<Note>
Cekura's SIP testing works in one direction: Cekura sends the SIP INVITE to your configured endpoint. Having your system dial into a Cekura SIP endpoint — for example, to pass a call to the testing agent during a handoff or transfer — is not currently supported. For outbound testing where your agent dials a Cekura-provided number, see [Outbound Call Testing](/documentation/guides/testing-agents/outbound-evaluators) (that flow uses telephony rather than SIP).
</Note>
