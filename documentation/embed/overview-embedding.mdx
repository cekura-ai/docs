...
```
https://dashboard.cekura.ai/embed/{agentId}/overview?token={token}
```
...
const VoceraOverviewEmbed = ({ agentId }) => {
  ...
  return (
    <iframe
      src={`https://dashboard.cekura.ai/embed/${agentId}/overview?token=${token}`}
...
export default {
  ...
  computed: {
    embedUrl() {
      return `https://dashboard.cekura.ai/embed/${this.agentId}/overview?token=${this.token}`;
    }
  },
  methods: {
...
export class VoceraOverviewEmbedComponent implements OnInit, OnDestroy {
  ...
  get embedUrl(): SafeUrl {
    const url = `https://dashboard.cekura.ai/embed/${this.agentId}/overview?token=${this.token}`;
...
class VoceraOverviewEmbed {
  ...
  updateIframe() {
    if (this.iframe && this.token) {
      this.iframe.src = `https://dashboard.cekura.ai/embed/${this.agentId}/overview?token=${this.token}`;
    }
  }
