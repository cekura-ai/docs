...
icon: "flask-vial"
iconType: "regular"
description: "Automatically configure and test Retell voice and chat agents in Cekura."
---
...
<Steps>
  <Step title="Select Retell">
    In Cekura, go to **Agents**, create an agent, and select **Retell** as the provider.
  </Step>
  <Step title="Enter your Retell credentials">
    Enter the Retell voice or chat Agent ID and your Retell API key. Cekura detects the agent type automatically.

    <img src="/images/retell/automatic-configuration.png" alt="Automatically configure a Retell agent in Cekura" />
  </Step>
  <Step title="Configure the agent">
    Click **Configure Agent**. Cekura imports the available name, prompt or conversation flow, language, start-speaker behavior, phone number, tools, dynamic variables, and knowledge base.
  </Step>
...
<Note>
Retell voice and chat agents use separate Agent IDs. Import the ID for the connection you want to test. To configure either connection manually, click **Set up manually instead**.
</Note>
...
Test your Retell chat agents through text-based interactions. Cekura automatically clones a temporary chat agent from your voice agent for each run, so a dedicated chat agent is optional.

<Steps>
  <Step title="(Optional) Create a Dedicated Chat Agent in Retell">
    If you want chat runs to use a specific chat agent instead of an auto-cloned one:
    ...
    4. Note the **Agent ID** of your chat agent.

       ![Retell Convert to Chat Agent Pn](/images/retell_convert_to_chat_agent.png)

       > **Note:** The chat version enables text-based interactions while maintaining the same conversation logic as your voice agent. When no chat agent ID is configured, Cekura clones a temporary chat agent from the voice agent automatically.

  </Step>
  <Step title="Connect Retell to Cekura">
    Link your Retell agent to the Cekura platform:
    ...
    3. Under **Integration Settings**, enter the **External Assistant ID** of your Retell voice agent and your **Retell API Key**.
    4. Optionally, enter the chat agent ID if you created a dedicated chat agent in the previous step.
