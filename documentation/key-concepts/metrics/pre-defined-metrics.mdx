...
<AccordionGroup>
  ...
  <Accordion title="Transcription Accuracy">
    ...
    **Rendering-only differences are not errors.** Differences that only change how a value is *rendered* are treated as equivalent and don't count at all — spelled-out vs digit numbers, and spelled-out vs abbreviated units (e.g. "five kilowatts" vs "5 kW", "two hundred thirty volt" vs "230 V"). Equivalent multilingual text written in different scripts, such as Hindi in Devanagari versus Latin transliteration, is also evaluated as the same utterance. Only a genuine change in the value, unit, or spoken content ("20 watts" vs "20 volts") is a real error. Backchannel acknowledgements ("uh-huh" vs "aha") are treated as minor variations.
