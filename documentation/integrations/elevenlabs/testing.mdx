...
<Step title="Enter your ElevenLabs credentials">
...
<img src="/images/elevenlabs/run-tests-websocket.png" alt="Run ElevenLabs Websocket Tests" />
...
Once your ElevenLabs agent is connected, **Improve Agent** can fix failing scenarios for you. It works on a private copy of your ElevenLabs agent, edits the copy, re-runs your evaluators, and shows you a before/after per metric. Your live agent is never touched until you click **Promote**.

<Steps>
<Step title="Connect the agent">
The loop reads and edits the agent through the ElevenLabs API key and Agent ID you configured above, so the Websocket or Chat connection must already be set up. Phone-only connections are not enough — Cekura needs API access to duplicate and edit the agent.
</Step>

<Step title="Start from a failure">
Launch **Improve Agent** from a finished run, from a failing category on the **Insights** page, or by selecting evaluators in the **Evaluators** tab and using the bulk action.
</Step>

<Step title="Review and promote">
When the loop finishes, review the per-metric before/after and the exact changes it made. **Promote** applies them to your live ElevenLabs agent. **Discard copy** deletes the private copy.
</Step>
</Steps>

**What Cekura can change on an ElevenLabs agent**

- The system prompt, LLM settings, and tool configuration
- The first message and language
- Turn-taking settings (turn eagerness, initial wait time)
- Speech recognition (ASR) settings
- For workflow agents: each subagent node's prompt and message, and the conditions on edges between nodes

**What it never changes**

- Voice and TTS settings
- API keys, webhook URLs, and phone numbers
- Transfer destinations, `transfer_to_agent` tools, and agent-transfer or transfer-to-number workflow nodes

<Note>
The private copy is created with ElevenLabs' duplicate API and named with a `[selfimprove-clone]` suffix, so it is easy to spot in your ElevenLabs dashboard. If your agent transfers callers to other ElevenLabs agents, those agents are cloned too and every transfer is pointed at the corresponding clone, so test calls never reach your live agents. Agents that transfer to more than 10 other agents, or nest transfers more than 3 levels deep, are refused rather than partially cloned. Copies are deleted when you discard them, shortly after you promote, and automatically after about a week if you do neither.
</Note>

See the full [Optimize Agent](/documentation/guides/optimize-agent) guide for how the loop reproduces failures, what each outcome means, and troubleshooting.
...
- [Optimize Agent](/documentation/guides/optimize-agent)
