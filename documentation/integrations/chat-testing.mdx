...
<Steps>
  <Step title="Set Up WebSocket Server">
    ...
    **Messages Cekura sends to your server** (testing agent messages):
    ...
    Message with test profile metadata:
    ```json
    {
      "content": "I'd like to book an appointment for tomorrow",
      "metadata": {
    ...
    <Info>
    When a test profile is attached to the evaluator, each message includes a `metadata` field containing the test profile information. This provides inline access to the test profile data with every message, in addition to the connection-time headers described in the [Test Profile Headers](#test-profile-headers) section below.
    </Info>
