...
## Alternative Authentication: MCP Tokens

If your MCP client supports OAuth or you prefer Bearer tokens over custom headers, you can use an MCP token from Cekura instead.

### Obtaining an MCP Token

MCP tokens are issued through the Cekura API. You must be logged into the Cekura dashboard to request one.

<Steps>
  <Step title="Log into Cekura">
    Sign in to the [Cekura Dashboard](https://dashboard.cekura.ai) to establish your session.
  </Step>

  <Step title="Request an MCP Token">
    Call the MCP token endpoint using your existing session:

    ```bash
    curl -X POST https://api.cekura.ai/user/api/v1/mcp-token/ \
      -H "Authorization: Bearer YOUR_SUPABASE_TOKEN"
    ```

    The response contains your MCP token:
  ...
  <Step title="Configure Your MCP Client">
    Use the token as a Bearer token in the `Authorization` header instead of `X-CEKURA-API-KEY`.
  </Step>
</Steps>
...
| Refresh | Tokens cannot be refreshed; request a new token before expiration |

### Using MCP Tokens with Claude Code

```bash
claude mcp add --transport http cekura --scope user https://api.cekura.ai/mcp --header "Authorization:Bearer YOUR_MCP_TOKEN"
```

<Note>
MCP tokens provide the same access as API keys. Choose whichever authentication method works best with your MCP client.
</Note>
