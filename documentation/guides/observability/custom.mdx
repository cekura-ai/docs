...
<Steps>
  ...
  <Step title="Create an agent">
    ...
    <img src="/images/create-agent.png" alt="Create Agent" width="500" />
  </Step>
...
<Tabs>
  <Tab title="Custom API">
    <Note>
      `transcript_type` must be one of `cekura`, `vapi`, `retell`, `elevenlabs`, `livekit`, `pipecat`, `deepgram`, `none-mono`. There is no `custom` value — use `cekura` for your own transcript format (omitting the field auto-detects the format).

      In the `cekura` format each turn is `{role, content, start_time, end_time}`:

      - Roles are `"Testing Agent"` (the caller) and `"Main Agent"` (the agent under test); `"user"` and `"assistant"` are accepted as aliases for them.
      - `start_time` and `end_time` (seconds, floats) are **required** on every `Testing Agent` / `Main Agent` turn, and must be non-decreasing within a speaker.
      - Tool calls use `"Function Call"` / `"Function Call Result"` (aliases `"tool_call"` / `"tool_call_result"`) with a `data` object of `{id, name, arguments, result}` instead of timestamps.

      See the [Send Calls API reference](/api-reference/observability/send-calls#body-transcript-type).
    </Note>
  ...
  <Tab title="Vapi">
    <Tabs>
      ...
      <Tab title="No Code">
        ### Auto-fetch production calls (recommended)

        Cekura can pull completed VAPI calls every 30 seconds without requiring a webhook.

        <Steps>
          <Step title="Connect your VAPI agent">
            In Cekura, create or open an agent, select **VAPI** as its provider, and enter the VAPI API key and Assistant ID.
          </Step>
          <Step title="Enable automatic call import">
            In the provider settings, enable **Auto-fetch Production Calls** and save the agent.

            <img src="/images/vapi/auto-fetch-production-calls.png" alt="Auto-fetch Production Calls enabled in Cekura provider settings" />

            Cekura checks VAPI for new completed calls every 30 seconds. The option is disabled by default.
          </Step>
          <Step title="Test your integration">
            Complete a call with the VAPI agent. The call should appear in **Observability → Calls** within 30 seconds.
          </Step>
        </Steps>

        ### Webhook integration

        Use this alternative when calls must appear in Cekura in real time.
        ...
        <Steps>
          ...
          <Step title="Add credentials to Cekura">
            ...
            ![Navigate to API Keys](/images/vapi/agent-settings.png)
          </Step>
          <Step title="Test your integration">
  ...
  <Tab title="ElevenLabs">
    <Tabs>
      ...
      <Tab title="No Code">
        ...
        Cekura can pull completed ElevenLabs calls every 30 seconds without requiring a webhook.

        <Steps>
          <Step title="Connect your ElevenLabs agent">
            In Cekura, create or open an agent, select **ElevenLabs** as its provider, and enter the ElevenLabs API key and Agent ID.
          </Step>
          <Step title="Enable automatic call import">
            In the provider settings, enable **Auto-fetch Production Calls** and save the agent.

            <img src="/images/elevenlabs/auto-fetch-production-calls.png" alt="Auto-fetch Production Calls enabled in Cekura provider settings" />

            Cekura checks ElevenLabs for new completed calls every 30 seconds. The option is disabled by default.
          </Step>
          <Step title="Test your integration">
            Complete a call with the ElevenLabs agent. The call should appear in **Observability → Calls** within 30 seconds.
          </Step>
        </Steps>
        ...
        <Steps>
          ...
          <Step title="Add credentials to Cekura">
            ...
            ![ElevenLabs Agent Settings](/images/elevenlabs/agent-settings.png)
          </Step>
          <Step title="Test your integration">
  ...
  <Tab title="Retell">
    <Tabs>
      ...
      <Tab title="No Code">
        ...
        Cekura can pull completed Retell calls every 30 seconds without requiring a webhook.

        <Steps>
          <Step title="Connect your Retell agent">
            In Cekura, create or open an agent, select **Retell** as its provider, and enter the Retell API key and Agent ID.

            <Note>
              The Retell API key must include the **History.Read** scope. If Retell rejects call-history access, Cekura automatically disables auto-fetch to avoid repeated failures.
            </Note>
          </Step>
          <Step title="Enable automatic call import">
            In the provider settings, enable **Auto-fetch Production Calls** and save the agent.

            <img src="/images/retell/auto-fetch-production-calls.png" alt="Auto-fetch Production Calls enabled in Cekura provider settings" />

            Cekura checks Retell for new completed calls every 30 seconds. The option is disabled by default.
          </Step>
          <Step title="Test your integration">
            Complete a call with the Retell agent. The call should appear in **Observability → Calls** within 30 seconds.
          </Step>
        </Steps>
        ...
        <Steps>
          ...
          <Step title="Add credentials to Cekura">
            ...
            ![images/Screenshot2025-03-17at11.35.20AM.png](/images/retell/agent-settings.png)
          </Step>
          <Step title="Test your integration">
