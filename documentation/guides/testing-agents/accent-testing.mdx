...
<Steps>
  <Step title="Create a personality per accent variant">
    In the *Personalities* section, create a custom personality for each accent you want to test.

    <Note>
    The accent comes entirely from the **voice** you pick — the `accent` field on a personality is a read-only label derived from it, not something you set. Browse `GET /test_framework/v1/personalities/elevenlabs_voices/?language=<code>` and match on each voice's `labels.accent`; Cartesia publishes no accent label, so prefer ElevenLabs when the variant needs to be named. Writing the accent into the personality prompt does not change pronunciation.
    </Note>
