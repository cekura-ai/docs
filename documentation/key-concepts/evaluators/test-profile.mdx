...
Older test profiles store `information` as a single flat dict (no section keys):
...
These keep working — at call time the same flat dict is delivered to **both** the agent under test and the testing agent (preserving pre-split behavior). The sectioned shape is the new default for newly created profiles; legacy profiles can be edited to migrate.
