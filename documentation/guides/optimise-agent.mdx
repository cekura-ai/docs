---
title: "Optimise Agent"
sidebarTitle: "Optimise Agent"
icon: "wand-magic-sparkles"
iconType: "regular"
description: "Close the loop on agent quality. The Optimise Agent button takes failing evaluators, diagnoses where the agent's prompt or config has gaps, applies edits, and re-validates — iterating until the agent passes."
---
...
The **Optimise Agent** button hands a set of failing evaluators to the Cekura AI Assistant, which runs a diagnose → propose → apply → re-validate loop against your agent's prompt and configuration. The loop continues until the agent reaches **100% pass rate** on the validation set or the iteration cap is reached.

<Note>
**Optimise Agent is BETA.** The optimiser edits a live agent (VAPI assistants directly via the VAPI API, self-hosted websocket agents via source-file edits). Review proposed diffs before applying for the first few runs.
</Note>
...
<Steps>
  <Step title="Evaluators list — bulk action">
    Open an agent's **Evaluators** tab, select one or more evaluators (the scenario rows), and the **Optimise Agent** button appears in the bulk-action bar next to **Run** and the delete control. The optimiser will use every selected evaluator as the validation set.
  </Step>
  <Step title="Run result page">
    Open any completed evaluator run and click **Optimise Agent** in the header. The optimiser receives the failing scenarios *and* the original `result_id`, so it starts from the call transcripts that already exist — no fresh run needed before the first diagnosis.
  </Step>
</Steps>
...
<Note>
**Retell is not currently supported by Optimise Agent** and will be re-enabled in a future revision. ElevenLabs, LiveKit, and Pipecat are routed through the self-hosted path when applicable.
</Note>
...
|---|---|
| Click **Optimise Agent** on the evaluators list with N evaluators selected | New chat opens with `/self-improve via these evaluators - <N evaluator IDs>` |
| Click **Optimise Agent** on a run result page | New chat opens with `/self-improve via these evaluators - <IDs> (result ids: <result_id>)` |
