...
**Important:** Python metrics execute **after** all other metrics (LLM Judge and pre-defined metrics). This means:

1. LLM Judge metrics evaluate first
...
### Example for Calling LLM Judge Metrics

To call LLM Judge metrics from your Python code, you can use the `evaluate_advance_metric` function. This function requires your API key, a description or prompt for the metric, and the metric name.
