...
<Note>
**Credit consumption:** Auto Improve charges for each metric evaluation across the test set. The total cost is `(iterations run + 1) × test set size × per-evaluation rate`. The per-evaluation rate depends on the metric type — audio-enabled metrics cost more than text-only metrics because they analyze the voice recording. If the optimizer terminates early, you only pay for the iterations that ran.
</Note>
