...
## Alternative Authentication: MCP OAuth

Cekura supports MCP OAuth for Claude clients. Instead of manually configuring an API key, you can authenticate through an OAuth flow that issues short-lived access tokens with automatic refresh.

### Using OAuth with Claude

When Claude's MCP client initiates an OAuth connection to Cekura:

<Steps>
  <Step title="Claude Redirects to Cekura">
    Claude opens Cekura's authorization page in your browser.
  </Step>

  <Step title="Authenticate with Cekura">
    Log in to your Cekura account if you're not already signed in.
  </Step>

  <Step title="Authorization Complete">
    Cekura issues access and refresh tokens, then redirects back to Claude. Your MCP connection is ready to use.
  </Step>
</Steps>
...
| Scope | User-level access (all organizations the user belongs to) |
| Refresh behavior | Claude automatically refreshes tokens before they expire |

<Note>
OAuth tokens provide the same access as API keys. Use OAuth for streamlined setup, or API keys if you prefer manual configuration.
</Note>
