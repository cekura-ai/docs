...
If `/upgrade-skills` reports stale plugin entries, or `/plugin install` fails with "Source path does not exist", run these 4 commands in order in any Claude Code session:
...
### `/plugin install` errors with "Source path does not exist" — what's wrong?

Claude Code is using cached marketplace metadata. Re-run `/plugin marketplace update cekura-skills`, then retry the install. If the error persists, fully restart Claude Code (close and reopen the session), then redo `/plugin marketplace add … → update → install`.
...
`git pull` on the source dir alone doesn't refresh the cache — run `/plugin marketplace update cekura-skills` (or reinstall) to rebuild it.
