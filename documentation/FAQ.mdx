...
## Why am I getting a 403 error when trying to update or delete a personality via the API?

Global personalities provided by Cekura are read-only. Attempting to update or delete a global personality returns a 403 Forbidden response.

### What Are Global Personalities?

Global personalities are Cekura's curated collection of testing personas (such as "Normal Male", "Interrupter", or "Normal Male - Bg Noise") available to all users. These differ from custom personalities that you create within your organization.

### How to Customize a Global Personality

To modify a global personality's settings, duplicate it to create an editable copy:

1. Use the `POST /test_framework/v1/personalities/{id}/duplicate/` endpoint to create a copy within your organization
2. The duplicated personality will be fully editable and deletable
3. Alternatively, use the duplicate button in the Cekura dashboard

This lets you build on Cekura's pre-defined personalities while maintaining full control over your customized versions.
