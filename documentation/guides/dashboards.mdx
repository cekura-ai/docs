...
|-----------|------|-------------|
| `group_by` | string | Field path to group by (e.g., `metric_evaluations.value`) |
...
**Example use case:** To visualize how metric scores are distributed over time, set `field` to `metric_evaluations.value`, `chart_type` to `bar`, and `group_by` to `metric_evaluations.value`. The chart shows a stacked or grouped bar for each distinct score value (e.g., 1, 2, 3, 4, 5) per time period.
