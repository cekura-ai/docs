...
Select the platform your agent is built on. All of the following providers are available in the provider selection screen for every Cekura account — if you don't see a provider in your sandbox, scroll through the full list. You can explore each provider's integration-specific docs to know more.
...
<CardGroup cols={4}>
  ...
  <Card title="Pipecat" icon="/images/providers/pipecat.svg" href="/documentation/integrations/pipecat/automated" />
  <Card title="ElevenLabs" icon="/images/providers/elevenlabs.svg" href="/documentation/integrations/elevenlabs/testing" />
  ...
  <Card title="Bland AI" icon="/images/providers/bland.svg" href="/documentation/integrations/bland"/>
  <Card title="Synthflow" icon="/images/providers/synthflow.svg" />
  <Card title="Kore.ai" icon="/images/providers/koreai.svg" href="/documentation/integrations/koreai" />
  <Card title="Gemini Enterprise CX" icon="/images/providers/gemini-cx.svg" href="/documentation/integrations/gemini-cx" />
  <Card title="Trillet" icon="/images/providers/trillet.svg" />
  <Card title="Genesys" icon="/images/providers/genesys.svg" href="/documentation/integrations/genesys" />
  ...
  <Card title="Amazon Connect" icon="/images/providers/amazon-connect.svg" />
</CardGroup>

<Note>All providers listed above are available across every Cekura account — including Amazon Connect, Cisco Webex, Kore.ai, Trillet, Agentforce, and Synthflow. If you don't see a specific provider in your sandbox, scroll through the full provider list in Agent Settings. Providers without a linked guide use the same connection setup described in the [Custom integration guide](/documentation/integrations/custom-integration).</Note>
...
### Configure automatically from your provider

Cekura can automatically configure agents hosted on **VAPI, Retell, ElevenLabs, Synthflow, and Bland AI**.

<Steps>
  <Step title="Select your provider">
    Create an agent, then select one of the supported providers. The **Import directly from provider** panel opens automatically.
  </Step>
  <Step title="Enter the provider credentials">
    Enter the agent or assistant ID and the provider API key. For ElevenLabs, also select the **US** or **EU Residency** API region.
  </Step>
  <Step title="Configure the agent">
    Click **Configure Agent**. Cekura imports the available name, prompt, language, call settings, phone number, tools, dynamic variables, and knowledge base while showing progress for each stage.
  </Step>
  <Step title="Review the imported agent">
    Open the agent after the import finishes and review its connections before running an evaluator.
  </Step>
</Steps>

<Note>
The imported fields depend on what the provider exposes. To enter the connection details yourself, click **Set up manually instead**.
</Note>
...
<Tabs>
  ...
  <Tab title="Retell">
    <img src="/images/retell/automatic-configuration.png" alt="Automatically configure a Retell agent in Cekura" />
  </Tab>
  <Tab title="ElevenLabs">
    <img src="/images/elevenlabs/automatic-configuration.png" alt="Automatically configure an ElevenLabs agent and select its API region in Cekura" />
  </Tab>
  <Tab title="Bland AI">
    <img src="/images/bland/automatic-configuration.png" alt="Automatically configure a Bland AI Persona or Pathway in Cekura" />
  </Tab>
  <Tab title="Synthflow">
    <img src="/images/synthflow/automatic-configuration.png" alt="Automatically configure a Synthflow agent in Cekura" />
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="WebRTC">
    **WebRTC** enables real-time browser-based voice testing without needing a phone number or external device.
    ...
    <Note>WebRTC calls are only supported by [LiveKit](/documentation/integrations/livekit/testing), [Pipecat](/documentation/integrations/pipecat/automated), [VAPI](/documentation/integrations/vapi/testing), [Retell](/documentation/integrations/retell/testing), and [Agora](/documentation/integrations/agora).</Note>
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Provider Chat">
    ...
    <Note>Native provider chat is only available with [ElevenLabs](/documentation/integrations/elevenlabs/testing), [VAPI](/documentation/integrations/vapi/testing), [LiveKit](/documentation/integrations/livekit/testing), [Retell](/documentation/integrations/retell/testing), and [Bland AI](/documentation/integrations/bland).</Note>

    Provider chat is configured automatically using your voice agent settings. **Bland AI** requires a separate chat agent ID (pathway ID). **Retell** optionally accepts a chat agent ID — when omitted, Cekura clones a temporary chat agent from your voice agent for each run.
