...
<Note>
**Improve Agent** — When you select one or more evaluator rows in the Evaluators tab, an **Improve Agent** button appears in the bulk-action bar (next to Run and the delete control). Clicking it works on a private copy of your agent, changing the instructions and re-testing until the selected evaluators pass, then hands you the before/after to accept or discard. [See the full guide](/documentation/guides/optimize-agent).
</Note>