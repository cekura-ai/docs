...
| `livekit_data` | no | LiveKit connection overrides (object). Only valid when `channel` is `livekit_v2`. Accepts `agent_name`, `config`, and `url`. |
| `pipecat_data` | no | Pipecat Cloud connection overrides (object). Only valid when `channel` is `pipecat_v2`. Accepts `pipecat_agent_name`, `config`, and `room_properties`. |
...
| `channel` | `voice` for a phone call (the default), `text` for chat, or `elevenlabs`, `livekit_v2`, or `pipecat_v2` for a WebRTC session through the respective provider. |
| `livekit_data` | LiveKit connection overrides for this run only. Valid keys: `agent_name`, `config`, `url`. Requires `channel: livekit_v2`. |
| `pipecat_data` | Pipecat Cloud connection overrides for this run only. Valid keys: `pipecat_agent_name`, `config`, `room_properties`. Requires `channel: pipecat_v2`. |
...
Anything the channel needs beyond that comes from the agent's own configuration: a phone number for voice, a chat provider for text, ElevenLabs credentials for `elevenlabs`, or LiveKit/Pipecat Cloud credentials for `livekit_v2`/`pipecat_v2`. If the agent isn't configured for the channel you pick, the run is rejected with a message naming what's missing.

Connection overrides for WebRTC channels (`livekit_data`, `pipecat_data`) apply to a single run and are never stored in the spec, keeping committed files portable.
