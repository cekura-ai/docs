...
| Method | Endpoint |
|--------|----------|
| `POST` | `https://new-prod.cekura.ai/test_framework/v1/results-external/:result-id/evaluate_metrics/` |
...
```bash Curl
curl -X POST https://new-prod.cekura.ai/test_framework/v1/results-external/:your-result-id/evaluate_metrics/ \
...
def reevaluate_metrics(api_key, result_id, run_ids, metric_ids):
    url = f'https://new-prod.cekura.ai/test_framework/v1/results-external/{result_id}/evaluate_metrics/'
    headers = {
...
async function reevaluateMetrics(apiKey, resultId, runIds, metricIds) {
  const url = `https://new-prod.cekura.ai/test_framework/v1/results-external/${resultId}/evaluate_metrics/`;
