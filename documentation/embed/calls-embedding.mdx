...
```
https://dashboard.cekura.ai/embed/{agentId}/calls?token={token}
```
...
const VoceraCallsEmbed = ({ agentId }) => {
  ...
  return (
    <iframe
      src={`https://dashboard.cekura.ai/embed/${agentId}/calls?token=${token}`}
...
export default {
  ...
  computed: {
    embedUrl() {
      return `https://dashboard.cekura.ai/embed/${this.agentId}/calls?token=${this.token}`;
    }
  },
  methods: {
...
export class VoceraCallsEmbedComponent implements OnInit, OnDestroy {
  ...
  get embedUrl(): SafeUrl {
    const url = `https://dashboard.cekura.ai/embed/${this.agentId}/calls?token=${this.token}`;
...
class VoceraCallsEmbed {
  ...
  updateIframe() {
    if (this.iframe && this.token) {
      this.iframe.src = `https://dashboard.cekura.ai/embed/${this.agentId}/calls?token=${this.token}`;
    }
  }
