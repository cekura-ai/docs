...
| Method | Endpoint |
|--------|----------|
| `POST` | `https://new-prod.cekura.ai/test_framework/v1/aiagents-external/` |
...
```bash Curl
curl -X POST https://new-prod.cekura.ai/test_framework/v1/aiagents-external/ \
...
def create_agent(api_key, agent_name, contact_number, inbound, description, language, assistant_id="", websocket_url=""):
    url = 'https://new-prod.cekura.ai/test_framework/v1/aiagents-external/'
...
async function createAgent(apiKey, agentName, contactNumber, inbound, description, language, assistantID="", websocketURL="") {
  const url = 'https://new-prod.cekura.ai/test_framework/v1/aiagents-external/';
