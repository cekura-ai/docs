...
<AccordionGroup>
  ...
  <Accordion title="Gibberish Detection (Beta)">
    ...
    **Troubleshooting — metric returned True when you expected a failure**: Gibberish Detection targets acoustically unintelligible audio — degraded speech, random noise, or audio segments that cannot be parsed as any recognisable language. Words that are non-standard, unusual, or from another language but are acoustically coherent (i.e. they sound like plausible speech to the model) will typically not be flagged. If you need to detect whether a specific word or phrase outside your agent's vocabulary is present in the output, an LLM Judge or Python custom metric configured for that vocabulary is a better fit than Gibberish Detection.
  </Accordion>
  ...
  <Accordion title="Pronunciation Check (Beta)">
    **Result**: Score (0-100) | **Cost**: 0.2 credits per call
    ...
    Evaluates pronunciation accuracy for specific words you define. The metric identifies the target words in the call transcript, then analyses the corresponding audio segment to score how accurately they were pronounced.

    **Requirements**: Audio recording and a call transcript. Configure `pronunciation_words` on your agent as a list of word-phoneme pairs (e.g., `[["Cekura", "suh-KYUR-uh"]]`). By default the metric uses the transcript your integration provides. If your integration does not supply a transcript, contact support to enable Cekura's generated transcript instead.

    **Interpretation**: Higher scores indicate better pronunciation accuracy. Useful for brand names or technical terms.

    **Troubleshooting**: If the metric is not producing results, verify that a transcript source is available. By default the metric uses the transcript your integration provides. If none is configured, Cekura can run the evaluation against its own generated transcript — contact support to enable this.
  </Accordion>
