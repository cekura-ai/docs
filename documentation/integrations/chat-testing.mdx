...
<Steps>
  <Step title="Set Up WebSocket Server">
    ...
    **Authentication:**
    ...
    | Header | Description |
    |--------|-------------|
    | `X-VOCERA-SECRET` | The Websocket Secret you configured in your agent's Custom Chatbot Settings. Cekura sends this exact value on every connection so your server can verify the request came from Cekura. This is **not** your Cekura API key — it's a shared secret you choose. |
    ...
    <Note>
    `X-VOCERA-SECRET` is **not** your Cekura API key. It is a shared secret you choose and configure in your agent's Custom Chatbot Settings (see **Configure in Cekura** below). Cekura sends whatever value you saved there, so your server should validate against the same value.
    </Note>
  ...
  <Step title="Configure in Cekura">
    ...
    4. Set a **Websocket Secret** — any string you choose. Cekura will send this value on every connection in the `X-VOCERA-SECRET` header so your server can authenticate the request. Use the **Update** button to change it later.
    5. Save configuration

    <img src="/images/custom-websocket/websocket-secret-config.png" alt="Custom Chatbot Settings showing WebSocket URL and Websocket Secret fields" />

    <Note>
    The Websocket Secret is a value **you** choose — it is **not** your Cekura API key. Keep the same value in your WebSocket server so it can validate the `X-VOCERA-SECRET` header. See the **Authentication** subsection under **Set Up WebSocket Server** above for a code example.
    </Note>
  </Step>
