...
The `<dtmf>` tag allows you to send specific button sequences at exact moments in your test scenarios. You can use it in two ways: as an XML tag or as inline DTMF.
...
- **digits** (XML format): The buttons to press (valid characters: `0-9`, `*`, `#`, `A-D`)
- **Inline format**: Place `<dtmf>` between text to send DTMF tones at that point in the message
...
You can combine DTMF tags with text messages in two ways:

**Using XML Tag Format:**
...
```json
{
  "action": "Let me enter that now.<dtmf> Done"
}
```

The inline format allows you to place DTMF at specific points in your text naturally.
...
Both send only the button press with no spoken message.
...
## Inline DTMF Examples

The inline DTMF format provides a cleaner way to embed DTMF input in natural text:

```json
{
  "action": "I'll enter my account number now.<dtmf> Let me confirm that for you"
}
```

This speaks the text and sends DTMF at the point marked by `<dtmf>`. You can use this format anywhere in your evaluator instructions or conditional actions for readability.
