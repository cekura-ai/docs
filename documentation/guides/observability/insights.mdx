...
## Viewing insights in the dashboard
...
Each card shows either the metric's failure modes for the selected window, or a message that no failures were found in that window. There is no **Generate** or **Regenerate** button: insights are not run on demand. A failing call is categorized automatically at the moment its metric is evaluated, and the card fills in as those calls arrive.

Each failure mode includes a title, a brief description, and clickable call IDs that link directly to the corresponding call logs.

### Generating insights for calls that were already evaluated

Enabling insights on a metric does not reprocess calls that were evaluated before you turned it on. To get insights for those calls, re-evaluate the metric on them: categorization is triggered whenever a metric's evaluation is written to a call log, and a re-evaluation counts the same as a fresh one.

1. Go to **Observability → Calls** and select the calls you want covered (individual checkboxes or the header checkbox).
2. Click **Reevaluate Metrics** in the toolbar and choose the insights-enabled metric(s) to rerun.
3. Each call that fails the metric's passing threshold is triaged and added to the metric's failure modes. Widen the date range on the Insights page to the period those calls belong to — the card defaults to the last 24 hours.

Re-evaluation is also how you refresh a call's existing insight: rerunning the metric drops the call's previous root-cause analysis and produces a new one, and a call that now passes is removed from the failure mode it was in.

<Note>
  Re-evaluation consumes credits at the standard metric-evaluation rate on top of the per-metric insights cost, and each metric triages only up to its daily failure limit — failures past the limit still count toward the failure rate but are not categorized.
</Note>
...
Click **Trend** on a failure mode to chart it over time. The line is the mode's rate — its failing calls as a percentage of the calls evaluated for that metric — and the coloured band beneath it shows which agent version was live, so a step up that starts where a band changes is a regression that release introduced.

Every point is one interval's own rate, never a moving average. That matters: a window wide enough to smooth the line would blend across the release boundaries the chart exists to distinguish, and report rates no version ever ran at. Where traffic is thin, Cekura widens the interval each point covers instead — still a real measurement of a real period.
...
The threshold uses the same condition shape as a rubric rule. It applies only to calls evaluated after Insights is enabled: Cekura does not reprocess historical calls on its own. Failed calls are triaged as they arrive, so the metric's failure modes populate over time. To cover calls evaluated before you enabled insights, [re-evaluate the metric on them](#generating-insights-for-calls-that-were-already-evaluated). To change the threshold later, reopen the metric's card configuration; to stop analysing a metric, turn insights off.
