...
If your assistant supports HTTP MCP, use the US endpoint by default:

```plaintext
https://api.cekura.ai/mcp
```

For a workspace in another region, replace the URL with the matching endpoint:
...
If your assistant requires a local command shim, use `mcp-remote` and replace the US URL with the matching regional endpoint:
