...
- **Agent `contact_number` set to your outbound caller ID**: Cekura validates the caller ID (ANI) of every incoming outbound-test call against the agent's `contact_number` field. If your agent dials from `+15551234567`, the agent's `contact_number` must be exactly `+15551234567` in E.164 format. Set this in Agent settings or via the [Update Agent API](/api-reference/test_framework/update-agent-partial).
  ...
  **Multiple caller IDs**: If your system may dial from more than one number, set `outbound_numbers` to a list of authorized E.164 caller IDs via the [Update Agent API](/api-reference/test_framework/update-agent-partial) (API-only, no UI). When `outbound_numbers` is non-empty, Cekura validates the incoming caller ID against that list; `contact_number` serves as a fallback only when the list is empty.

  ```bash
  curl -X PATCH "https://api.cekura.ai/test_framework/v2/aiagents/<AGENT_ID>/" \
    -H "X-CEKURA-API-KEY: <YOUR_API_KEY>" \
    -H "Content-Type: application/json" \
    -d '{"telephony": {"outbound_numbers": ["+12223334444", "+15556667777"]}}'
  ```
