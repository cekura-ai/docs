...
| `agent_number` | no | Your agent's phone number for this run, in E.164 format. Overrides the number configured on the agent. `voice` and `text` only. |
| `outbound_phone_number` | no | For an inbound voice agent, the number Cekura dials, in E.164 format. Defaults to `agent_number`, then the agent's configured number. Rejected for outbound voice agents — set `phone_number` on the test case instead. For SMS and WhatsApp agents, the contact number for the conversation. `voice` and `text` only. |
| `mode` | no | `same_number` (default) places every call from one Cekura number; `different_numbers` spreads calls across your pool and can't be combined with `phone_number` on a test case. Inbound `voice` agents only. |
| `frequency` | no | Run every test case this many times, on top of each case's own `frequency`. Default `1`. |
| `concurrency_limit` | no | Maximum number of runs in parallel. Overrides the spec's `defaults.concurrency_limit`. |
| `mock_tool_names` | no | Mock tools to activate for this run. Omit or send `null` to mock every tool configured on the agent; send `[]` to mock none. |

\* Send exactly one of `spec` or `file`.

This endpoint is also reachable at `POST /test_framework/v2/scenarios/run_scenarios_json/`, which behaves identically; existing integrations on either path keep working.
...
```
{
  ...
  "defaults": {                                optional, applied to every case
    ...
    metrics, personality, test_profile, tags, phone_number
  },
...
| `phone_number` | no | E.164 number on Cekura's side of the call. See [Pinning a phone number](#pinning-a-phone-number). |
...
Settable inline: `name`, `prompt`, `language`, `voice_id`, `voice_model`, `provider`, `speed`, `background_noise`, `end_call_enabled`.

`accent` is not in that list and a spec that sets it is rejected: the caller is synthesised from `voice_id` alone, so accent is a property of the voice and is reported back as a read-only label. Set `voice_id` to a voice that has the accent you want.
...
A test case overrides `defaults` field by field. That includes `metrics`: a test case that sets its own `metrics` list replaces the default list rather than adding to it.

### Pinning a phone number

By default a voice run uses the least-used number from your pool, so the Cekura-side number can differ from run to run. Set `phone_number` when a case needs a fixed one — for example when your agent whitelists caller IDs, or when your agent places the call and must be told which number to dial (a handoff or transfer flow):

```json
{ "name": "Warm transfer", "instructions": "...", "phone_number": "+14155550123" }
```

When Cekura dials your agent, this is the caller ID it shows. When your agent dials Cekura, this is the number Cekura answers on. The number must be one available to your organization — a Cekura pool number or one you [imported](/documentation/key-concepts/phone-numbers/twilio) — in E.164 format (`+` followed by country code and digits). Anything else is rejected at `scenarios[i].phone_number` before any call is placed. Like the other case fields, `phone_number` can go in `defaults` to apply to every case in the file, and it is included when you [export existing evaluators](#starting-from-evaluators-you-already-have) that have a number configured.
...
Everything that describes a test case lives in the file: instructions, metrics, personality, test data, and the Cekura-side `phone_number`. Settings that describe *this run* go on the request instead, so one committed file can run against different numbers or load levels without edits:

```bash
--data "{\"agent_id\": 42, \"agent_number\": \"+14155550123\", \"frequency\": 3, \"concurrency_limit\": 5, \"spec\": $(cat cekura.tests.json)}"
```

These are the same run options the evaluator run endpoints accept, and they apply to every test case in the spec:

- **`agent_number`** replaces the agent's configured number for this run — for example to test a staging line with the same agent. It is your agent's number; the case-level `phone_number` is Cekura's side of the call.
- **`outbound_phone_number`** sets the number Cekura dials when your agent receives calls. If you only pass `agent_number`, Cekura dials that. For an agent that places calls, choose the Cekura number it dials with `phone_number` on the test case instead; `outbound_phone_number` is rejected there.
- **`frequency`** multiplies each case's own `frequency`: a case with `"frequency": 2` run with `frequency: 3` runs 6 times. The dry run's `total_runs` and estimated cost include it.
- **`concurrency_limit`** wins over `defaults.concurrency_limit` in the file.
- **`mode`** applies to agents that receive calls. `different_numbers` picks the Cekura number for every call, so it can't be combined with a test case's `phone_number`.
- **`mock_tool_names`** works as it does on evaluator runs. Send `mock_tool_names: []` to mock no tools; leave it out to mock every tool configured on the agent.

Personalities and test profiles are chosen per test case in the file (see [Personalities](#personalities) and [Test data](#test-data)), so `personality_ids` and `test_profile_ids` are not accepted here. Neither are `scenarios`, `tags` or `folder_path` — the spec is the list of test cases. Sending any of them with a value returns a `400` that says where the setting belongs, rather than silently ignoring it; empty placeholders such as `[]` or `null` are ignored.

A dry run runs the same checks as the real run, so a rejected override, missing channel credentials, or a missing LiveKit `agent_name` or Pipecat `pipecat_agent_name` fails the dry run too. It echoes the overrides it applied under `plan.run_overrides`, and the numbers the run will use under `plan.agent_number` and `plan.outbound_phone_number`, so you can confirm them before placing any calls.
...
Runs are asynchronous: the run call returns once they are queued, before anything is dialled. A CI step has to wait for the result to finish and then fail the job unless the runs passed. `cekura run spec` does both: `--wait` polls the result until it is `completed`, `failed`, `timeout` or `cancelled`, and `--fail-on-threshold 1.0` exits non-zero unless every run passed. See [Result statuses and gating](/documentation/guides/testing-agents/result-statuses) for what each status and count means if you call the API directly instead.
...
jobs:
  cekura:
    ...
    steps:
      ...
      - run: pip install "cekura[cli]"

      - name: Check the spec
        run: cekura run spec cekura.tests.json --agent-id ${{ vars.CEKURA_AGENT_ID }} --dry-run

      - name: Run the tests
        run: |
          cekura run spec cekura.tests.json --agent-id ${{ vars.CEKURA_AGENT_ID }} \
...
agent-tests:
  ...
  script:
    - cekura run spec cekura.tests.json --agent-id "$CEKURA_AGENT_ID" --dry-run
    - cekura run spec cekura.tests.json --agent-id "$CEKURA_AGENT_ID" --wait --fail-on-threshold 1.0
```
...
`--channel` picks the target (`voice` by default, `text`, or `elevenlabs`). Lower `--fail-on-threshold` (e.g. `0.9`) to allow some runs to fail; a result that ends `failed`, `timeout` or `cancelled` fails the job either way. The GitHub Actions, GitLab and Bitbucket integrations run dashboard evaluators by ID, tag or folder; for a committed spec, use the CLI as above.
