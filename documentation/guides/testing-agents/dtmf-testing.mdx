...
To specify which digit the testing agent should press, add explicit instructions to your evaluator. For example:

```
"When the agent presents a menu, press 1 for the first option or press 2 for the second option through DTMF."
```

Explicit instructions ensure the testing agent presses the correct digit for each scenario rather than relying on inference. For cases where you need exact timing or a guaranteed digit sequence at a precise moment, use the `<dtmf>` tag in Structured Tests instead — see the section below.
