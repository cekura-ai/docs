...
<AccordionGroup>
  <Accordion title="<ivr> - Interactive Voice Response" icon="volume-high">
    ...
    **Nested inline tags:** The `text` attribute can contain inline tags like `<silence>`, `<break>`, and `<noise>` for timing and sound control:

    ```json
    {
      "action": "<ivr text=\"<silence time=\\\"1s\\\" /> Welcome. <break time=\\\"1.5s\\\" /> Press one for support.\" />"
    }
    ```
  </Accordion>
  ...
  <Accordion title="<voicemail> - Voicemail Greeting" icon="voicemail">
    ...
    **Nested inline tags:** The `text` attribute can contain inline tags like `<silence>`, `<break>`, and `<noise>` for timing and sound control:
    ...
    ```json
    {
      "action": "<voicemail text=\"<silence time=\\\"2s\\\" /> Hello. <noise sound=\\\"office\\\" /> Please leave a message.\" />"
    }
    ```
  </Accordion>
