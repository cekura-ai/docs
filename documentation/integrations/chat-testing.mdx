...
<Note>
  SMS and WhatsApp chat connections are also supported for testing. These are configured per-agent rather than per-provider — see [Agent Setup Guide → Chat Connection](/documentation/key-concepts/agents/Agent_Setup_Guide#chat-connection) for setup details.
</Note>
