...
<Note>
**API contract — `dynamic_variable_values` (deprecated).** The scenario endpoint historically also returned a top-level `dynamic_variable_values` field with the same flat dict as `main_agent_variables`. It is now **deprecated** and kept read-only for legacy rows. New scenarios leave it `null`. When supplied on `POST`/`PATCH`, the values are bridged into the attached (or auto-created) test profile's `main_agent_variables` section. Migrate callers to read/write `test_profile.information.main_agent_variables` directly.
</Note>
