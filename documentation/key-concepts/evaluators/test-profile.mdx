...
You can also use global variables in your instructions alongside test profile fields:

| Variable | Description | Example Value |
|----------|-------------|---------------|
| `{{date}}` | Today's date in YYYY-MM-DD format | `2025-03-15` |

Combine global and test profile variables in the same instruction:

```
Schedule a follow-up appointment for {{test_profile.name}} on a date after {{date}}.
```

<Tip>
The `{{date}}` variable outputs a date string. Any date arithmetic (like "3 days from now") is interpreted by the AI at runtime, not by the template system. For example, "Book an appointment 3 days from {{date}}" becomes "Book an appointment 3 days from 2025-03-15", and the evaluator's AI interprets this to mean March 18, 2025.
</Tip>
