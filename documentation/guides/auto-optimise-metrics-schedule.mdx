...
The [Metric Lab](/documentation/guides/metric-lab) optimiser rewrites a metric's prompt based on the labels reviewers have given its calls in [Human Review](/documentation/guides/human-review). This page covers running that optimiser on a recurring schedule **from within Cekura** — no external tooling needed.
...
- You label calls in [Human Review](/documentation/guides/human-review) regularly and want those annotations to feed back into the metric prompt without manual clicks.
...
<Steps>
  ...
  <Step title="Add a metric-optimiser schedule">
    ...
    - **Metrics** — one or more LLM-judge metrics in this project. The scheduled optimiser uses every call already labelled for each metric, across its review datasets.
...
**The schedule reports "no changes proposed" every run.** No new labels have been added for that metric since the last run. The optimiser is deterministic on a fixed annotation set.

**A run reports "regressed — not applied" for a metric.** The optimised prompt scored lower than the current prompt on the labelled set. This is the regression guard working as intended — review the diff in the email and either rewrite the metric prompt by hand or review more calls to clarify the failing cases.
