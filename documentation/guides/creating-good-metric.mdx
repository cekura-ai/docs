...
<Steps>
  ...
  <Step title="Provide Feedback">
    ...
    **Example:**
    ...
    Then add the call to a [Human Review](/documentation/guides/human-review)
    dataset for this metric, so the label is reachable by an optimization run:
    select it on **Calls** and choose **Add to human review**.
  </Step>
</Steps>

<Tip>
With Slack integration, you can submit feedback directly from Slack alerts. When a metric fails, click the 👎 button next to **Go to call** to open a feedback modal. Explain why the metric evaluation was incorrect and the correction is recorded against that metric.
</Tip>

<Note>
**Best Practice:** Repeat this process for at least **6 calls** to create a robust dataset for optimization. Reviewing a whole dataset in one pass is usually faster than correcting calls one at a time — see [Human Review](/documentation/guides/human-review).
</Note>

## Step 5: Optimization (Labs)

Once reviewers have labelled a set of calls, use **Labs** to optimize the metric
against those labels.

<Steps>
  <Step title="Collect the labels">
    Create a [Human Review](/documentation/guides/human-review) dataset with this
    metric, add the calls, and review them. The dataset's **Progress** tab shows
    the metric's current agreement rate — your baseline.
  </Step>
  <Step title="Navigate to Labs">
    Navigate to **Labs**, select the metric, then select the review datasets
    holding its labels. You can pick more than one.
  </Step>
  <Step title="Optimize">
    Labs shows how many labelled calls the selection adds up to and the metric's
    current agreement. Click **Optimize metric**.

    The system uses the labels and the reviewers' explanations to rewrite the
    metric's internal logic/prompt to handle the edge cases you identified.
  </Step>
  <Step title="Verify & Save">
    1. Review the **View Changes** screen to see the old vs. new logic
    2. Check the new score against the labelled calls
    3. Click **Save** to push the optimized metric to production
  </Step>
</Steps>

<Note>
Only calls a reviewer has actually labelled are used. An unreviewed call carries
the metric's own answer, so training on it would only teach the metric to agree
with itself.
</Note>
