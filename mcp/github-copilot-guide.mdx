...
<Warning>
These surfaces don't support OAuth-authenticated remote MCP. For live Cekura tools, register the US-default endpoint `https://api.cekura.ai/mcp` in the repository's [Copilot MCP configuration](https://docs.github.com/en/copilot/how-tos/copilot-on-github/customize-copilot/configure-mcp-servers) with an `X-CEKURA-API-KEY` header. For India or EU, use the matching endpoint in [manual setup](/mcp/mcp-only#regional-endpoints). See [API-key setup](/mcp/mcp-only#api-key-setup). Copilot CLI needs no key.
</Warning>
