...
breakdowns, metric counts) for date ranges older than 30 days. The call logs
API (`GET /observability/v1/call-logs/`) also enforces a maximum window of
**31 days** per request — pass `timestamp_from`/`timestamp_to` (ISO 8601) to
select a window within that limit.
</Note>
