...
## What are the file upload limits when uploading voice recordings through the dashboard?

When uploading voice recordings to call logs through the Cekura dashboard:

- **Maximum files per upload:** 5 files
- **Maximum total size per upload:** 500 MB

These limits apply to manual uploads through the dashboard interface. For supported audio formats, see [Supported Audio Formats](#what-audio-formats-are-supported-for-voice-recording-uploads).

To upload larger files or more recordings at once, use the [Send Calls API](/api-reference/observability/send-calls) with a `voice_recording_url` pointing to your hosted recording files.
