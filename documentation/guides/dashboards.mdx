...
<Note>
  `dropoff_point` values are natural language strings generated from the conversation — they are not a fixed enum. The exact values depend on how your agent describes the end of the call in context. Voicemail calls do not produce a `dropoff_point` value; to exclude them from a widget, filter using the `isnull` operator on `dropoff_point` (or add a value-based filter that excludes your agent's voicemail phrasing).
</Note>
