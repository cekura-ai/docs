...
## Does Cekura support Spanish (Castilian) for voice testing and simulation, specifically for speakers from Spain?

Yes, Cekura fully supports Spanish (Castilian) for both testing and monitoring your AI voice agents. We understand that regional nuances, vocabulary, and accents are critical when targeting specific demographics, such as elderly customers in Spanish regions like Galicia, Toledo, and Ciudad Real.

### How Cekura Supports Castilian Spanish

*   **Voice Simulation:** Cekura provides access to various **ElevenLabs multilingual voices** specifically designed for Spanish (Spain). This allows you to simulate test callers with the correct Castilian accent and vocabulary, ensuring your agent is tested against realistic scenarios rather than generic or Latin American Spanish variants.
*   **Testing Workflow:** During the testing phase, you can define the context of your agent (e.g., an outbound agent for elderly residents in Spain). Cekura will generate test cases (evaluators) that interact with your agent using these localized voices to verify performance and accuracy.
*   **Monitoring and Evaluation:** For agents already in production, you can send call recordings or transcripts to Cekura. The platform evaluates these interactions based on metrics you define, ensuring the agent maintains the high quality required for the Spanish market.

### Getting Started
To set up your Spanish-language agent for testing, you can follow our standard workflow:
1.  **Provide Agent Context:** Specify that the agent is designed for the Spanish (Spain) market.
2.  **Generate Evaluators:** Create or automatically generate test cases that simulate your target demographic.
3.  **Run Tests:** Use our integrated voices to call your agent (e.g., your Retell AI agent) and observe the interaction.
4.  **Analyze Results:** Review the metrics to ensure the agent handled the Castilian nuances correctly.

For more details on configuring your agent's profile, please visit our [Agent Setup Guide](https://docs.cekura.ai/documentation/key-concepts/agents/Agent_Setup_Guide).
