...
<Note>
Cekura provides an MCP server that automatically exposes all API endpoints, allowing AI assistants like Claude Desktop, Cursor, and VS Code to interact with your agents, metrics, and test results.
</Note>
...
Cekura exposes core API endpoints as MCP tools. To optimize performance and reduce context window usage, we selectively expose the most commonly used endpoints:
...
For a complete list of available tools, see the [API Reference](/api-reference).
