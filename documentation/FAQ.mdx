...
Currently, Cekura does not have a dedicated free-form note-taking feature for individual test call runs. The closest option is **Human Review annotations**: add the call to a review dataset and, on the reviewer surface, write a note against any metric explaining why the call passed or failed it. These notes are primarily designed to refine the metric definition when you optimize it in Labs, but they also serve as a per-call qualitative record tied to that metric. A dataset with **triage** enabled also lets you tag the call itself with a category independent of any metric. See the [Human Review](/documentation/guides/human-review) guide for the full annotation workflow.
