...
Then connect the MCP server so your assistant can act on your workspace. The
examples use the US endpoint by default; for India or EU, replace the MCP URL
with the matching endpoint in [regional MCP setup](/mcp/mcp-only#regional-endpoints):
