...
| Method | Endpoint |
|--------|----------|
| `GET` | `https://new-prod.cekura.ai/test_framework/v1/runs-external/bulk/` |
...
```bash Curl
curl -X GET "https://new-prod.cekura.ai/test_framework/v1/runs-external/bulk/?run_ids=181,180" \
...
def get_runs_bulk(api_key, run_ids):
    url = 'https://new-prod.cekura.ai/test_framework/v1/runs-external/bulk/'
    headers = {
...
async function getRunsBulk(apiKey, runIds) {
  const url = new URL('https://new-prod.cekura.ai/test_framework/v1/runs-external/bulk/');
