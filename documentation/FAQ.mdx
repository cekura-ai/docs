...
## What is the difference between "Failed" and "Error" badges on test runs?

Cekura uses two distinct badges to help you understand why a test run didn't complete successfully and whether you can fix the issue yourself.

### Failed Badge (Reviewable)

A **Failed** badge indicates a client-side configuration error that you can likely resolve. These runs are marked as "reviewable," meaning you can change their status after investigating.

Common causes:
- **Invalid API key:** Your provider API key is incorrect, expired, or lacks sufficient permissions
- **Invalid agent or assistant ID:** The configured agent ID doesn't exist or is inaccessible
- **Invalid phone number:** The phone number format is incorrect or unreachable
- **Rate limit exceeded:** You've hit rate limits on your provider's API
- **Payment required:** Your provider account has billing issues
- **Validation errors:** Configuration parameters don't meet the provider's requirements

### Error Badge (Non-Reviewable)

An **Error** badge indicates a server-side or infrastructure issue on Cekura's end. These runs are marked as "non-reviewable" because the problem isn't related to your configuration.

Error runs typically result from:
...
- **For Failed runs:** Review the error message, check your API keys and configuration, and verify your provider account is in good standing. Once you've fixed the underlying issue, re-run the test.
- **For Error runs:** These are typically transient. Try running the test again. If the problem persists, contact Cekura support.
