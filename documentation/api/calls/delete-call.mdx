...
| Method | Endpoint |
|--------|----------|
| `DELETE` | `https://new-prod.cekura.ai/observability/v1/call-logs-external/:call_log_id/` |
...
```bash Curl
curl -X DELETE https://new-prod.cekura.ai/observability/v1/call-logs-external/:call_log_id/ \
...
def delete_call(api_key, call_log_id):
    url = f'https://new-prod.cekura.ai/observability/v1/call-logs-external/{call_log_id}/'
...
async function deleteCall(apiKey, callLogId) {
  const url = `https://new-prod.cekura.ai/observability/v1/call-logs-external/${callLogId}/`;
