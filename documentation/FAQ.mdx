...
## Can I assign phone numbers to generated evaluators automatically in a round-robin fashion?

Yes. When using the **Generate Evaluators** form, you can select which phone numbers are allocated to the generated evaluators without having to configure each one manually afterward.

### How to Set It Up

1. Open the **Generate Evaluators** form for your agent.
2. Scroll to the **Additional Configuration** section at the bottom of the form.
3. Set the phone number selection to **Enter Manually**.
4. Select all the phone numbers you want to use from the available list.

Cekura will assign the selected numbers to the generated evaluators in a round-robin fashion — each evaluator receives one number, cycling through the list until all evaluators are assigned.

This works for evaluators generated from any source (knowledge base, structured test cases, or manual generation). If you have multiple numbers imported into Cekura and want to distribute test runs across them without editing evaluators individually, this is the recommended approach.

For more on importing your own numbers, see the [Twilio](/documentation/key-concepts/phone-numbers/twilio) and [Plivo](/documentation/key-concepts/phone-numbers/plivo) integration guides.
