...
<Note>
This page covers testing agents that connect via a raw `wss://` WebSocket endpoint (no phone number required). If your agent uses a phone number or a telephony provider, see [Import Twilio Phone Numbers](/documentation/key-concepts/phone-numbers/twilio) for Twilio-based testing or [SIP Integration](/documentation/integrations/sip-integration) for SIP-based agents.
</Note>
