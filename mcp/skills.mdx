...
After step 4, `claude plugin list` should show one `cekura@cekura-skills` entry, and all 14 slash commands resolve.

### `/plugin install` errors with "Source path does not exist: …/plugins/cekura" — what's wrong?

Claude Code is using cached marketplace metadata that points at the old (pre-v0.5.0) layout. Re-run `/plugin marketplace update cekura-skills`, then retry the install. If the error persists, fully restart Claude Code (close and reopen the session), then redo `/plugin marketplace add … → update → install`.
...
### What are those leftover `cekura-evals` / `cekura-metrics` cache dirs?

Harmless orphans from the v0.4.x layout (cekura-skills used to ship as 3 separate plugins). After upgrading to v0.5.0+, `~/.claude/plugins/cache/cekura-skills/cekura-evals/` and `cekura-metrics/` may still exist on disk. Claude Code doesn't read them anymore — they're a few MB of inert files. Leave them alone, or remove them manually if you'd like the disk space back.
