...
## How can I resolve a 'Failed to load invitation — AAL2 required' error when accepting a project invitation?

The "AAL2 required" error occurs when Multi-Factor Authentication (MFA/2FA) is enabled on the invitee's Cekura account. The invitation link requires a higher authentication assurance level than a standard login, which can block acceptance when MFA is active but a valid session isn't already present.

**If the invitee already has a Cekura account:** do not use the invitation link to sign up again. Go directly to [dashboard.cekura.ai](https://dashboard.cekura.ai) and log in with your existing credentials. Once logged in, the organisation's pending invitation should appear in the dashboard for you to accept. If you have forgotten your password, use the [Forgot Password](https://dashboard.cekura.ai/forgot-password) page to reset it before trying to log in.

**If the AAL2 error persists after logging in:** 2FA may have been enabled on the account previously and needs to be cleared by a Cekura administrator. Contact [support@cekura.ai](mailto:support@cekura.ai) and ask for MFA to be removed from the account so the invitation can be accepted.

**Captcha errors ("captcha protection: request disallowed — no captcha_token found"):** These typically appear when clicking "Resend code" after a session timeout or when a browser extension is blocking the captcha script. Try reloading the page or opening the link in an incognito/private browser window with extensions disabled.
...
## Can Cekura evaluate a warm transfer scenario that spans two separate LiveKit rooms as a single test run?

Yes. Warm transfer flows where participants start in separate LiveKit rooms and then merge into one can be evaluated as a single Cekura test run — covering the full conversation from initial pickup through the merge.

For flows that include a DTMF acceptance step (e.g. "press 1 to confirm"), enable *Send DTMF* on the evaluator or add a `<dtmf digits="1" />` tag inside a `fixed_message` condition, and include an instruction telling the testing agent which digit to press when prompted. See the [DTMF Testing guide](https://docs.cekura.ai/documentation/guides/testing-agents/dtmf-testing) for setup details.

For multi-turn verification sequences (e.g. verbal accept → identity prompt → confirmation → concluding message before merge), model each turn as a separate condition using `action_followup` and chain them in order. Submit the run via the [manual LiveKit endpoint](https://docs.cekura.ai/documentation/integrations/livekit-integration) using the room URL and access token for the merged room that Cekura's testing agent joins. Your infrastructure handles the pre-merge room setup; Cekura evaluates from the point it enters the room.
