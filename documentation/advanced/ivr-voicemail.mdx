...
**Important:** When using XML tags like `<voicemail>`, make sure to set the `fixed_message` field to `true` in your condition configuration. The tag must be lowercase (`<voicemail>`, not `<VOICEMAIL>` or `<Voicemail>`).
...
<Note>
**Important:** When using XML tags like `<ivr>` in your actions, always set the `fixed_message` field to `true`. Only set `fixed_message` to `false` when you want the agent to respond naturally without using XML tags. The tag must be lowercase (`<ivr>`, not `<IVR>` or `<Ivr>`).
</Note>
