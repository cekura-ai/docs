...
## What does the "first message" clarification question mean during scenario generation?

When generating scenarios, Cekura may ask: "Will your agent always speak first at the start of a call?" This determines how the testing agent behaves at the beginning of each simulated conversation.

- **Answer "yes"** if your main agent initiates with a greeting or opening message. The testing agent will wait for your agent to speak first before responding.

- **Answer "no"** if your main agent waits for the caller to speak first. The testing agent will initiate the conversation to start the simulation.

Your answer is saved as a preference for the agent and applies to all future scenario generation. This ensures generated scenarios accurately reflect your agent's real-world conversation flow.
