...
## Should I use custom metrics or an expected outcome metric when starting new tests?

When you are beginning to run tests on your AI voice or chat agents in Cekura, it is highly recommended to start with the **Expected Outcome** metric rather than multiple custom metrics.

### Why Start with Expected Outcome?

*   **Easier Tuning:** Custom metrics often require specific configurations and fine-tuning to provide accurate results. The Expected Outcome metric is much more flexible and easier to tweak as you refine your test cases.
*   **Simplicity:** Starting with a single, clear expected outcome allows you to validate the core logic of your agent without the complexity of managing multiple scoring parameters simultaneously.
*   **Faster Setup:** You can get your initial evaluations running more quickly, providing immediate feedback on whether the agent is meeting its primary objectives.

### Transitioning to Custom Metrics

Once you have established a baseline using Expected Outcomes and your tests are yielding consistent results, you can then begin to introduce custom metrics. These are useful for measuring more granular aspects of the conversation, such as specific script adherence, sentiment analysis, or technical data extraction.

For more information on how to configure these settings, you can check our documentation on [Basic and Advanced Metrics](https://docs.cekura.ai/documentation/key-concepts/metrics/basic-advanced-metrics).
