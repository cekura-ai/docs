    ...
    **WebSocket** enables real-time, two-way chat between your agent and the LLM, allowing dynamic and continuous conversation testing.
    ...
    <Steps>
      ...
      <Step title="Add custom headers (optional)">
        Add any headers you want Cekura to include when establishing the WebSocket connection. Enter headers as key-value pairs, or switch to JSON mode for bulk editing.

        <Note>`X-VOCERA-*` headers are reserved and cannot be used.</Note>
      </Step>
    </Steps>
