...
Test your LiveKit agents with text instead of voice calls. Uses the same credentials as voice testing — faster and cheaper for workflow validation before production.

<Steps>
  <Step title="Configure LiveKit Credentials">
    If you haven't configured LiveKit yet, go to **Agent Settings** and enter your LiveKit credentials:
    ...
    See [LiveKit Automated Testing](/documentation/integrations/livekit/testing) for detailed setup instructions.
  </Step>

  <Step title="Configure Chat Provider">
    1. Go to **Agent Settings** → **Chatbot Integrations**
    2. Select **LiveKit** from the provider dropdown
    3. Save configuration

    <Note>
    LiveKit chat testing reuses your existing LiveKit credentials. No additional configuration required beyond selecting LiveKit as the chat provider.
    </Note>
  </Step>
...
<AccordionGroup>
  <Accordion title='Error: "Chat provider not configured"'>
    **Solution:**
    1. Go to Agent Settings → Chatbot Integrations
    2. Select a provider (ElevenLabs, Retell, Vapi, Bland, LiveKit, or Custom)
    3. Enter required configuration (Assistant ID, Pathway ID, or WebSocket URL)
    4. Save and retry
  </Accordion>
