...
<Note>
**Text Output Behavior**: Some voice control tags are automatically filtered from the text response (transcript) while still affecting audio playback:
- **Filtered from text**: `<silence>`, `<speed>`, `<volume>` — these tags control speech but do not appear in the transcript
- **Kept in text**: `<hold>`, `<interruption>`, `<noise>`, `<background_noise>` — these tags remain visible in the text output
</Note>
