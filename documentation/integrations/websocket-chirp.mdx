...
Results appear in your dashboard like any other run — status, metrics, transcripts, and audio playback are all available. A run whose connection fails does not pass: its `error_message` explains the cause and `metadata.ended_reason` records it.

| What happened | What the run shows |
|------------|-----------------|
| Server returned `401`/`403` on the WebSocket upgrade (bad Basic Auth or rejected origin) | Does not pass; `ended_reason` is `websocket-authentication-failed` and `error_message` points to the credentials in Agent Settings. |
| Host unreachable, TLS failure, or server closed the connection before the test could complete | Does not pass; `error_message` gives the cause. |
| Audio exchange finished cleanly | Evaluator scores are available; the run passes or fails on its evaluation. |
...
3. **Result:** A valid handshake establishes the live connection. A rejected handshake ends the run without a conversation, and the run does not pass.
...
| Scenario | Run outcome | Behavior |
|----------|-----------|----------|
| `HTTP 401`/`403` on upgrade | Does not pass (`websocket-authentication-failed`) | Cekura logs the rejection and stops. |
| Host unreachable, TCP refused, TLS failure | Does not pass | Cekura retries 3 times with backoff before giving up. |
| Upgrade accepted but immediately closed | Does not pass | Close code is logged in the run details. |
...
| Server closes gracefully | depends on call | The outcome is determined by whether the test completed before close. |
| Network drop or crash | Does not pass | Same handling as a TCP failure. |
