...
## How can I configure Synthflow to use a regional API endpoint?

If your Synthflow account uses a regional API endpoint (for example, EU customers using `https://api.eu.synthflow.ai` instead of the default `https://api.synthflow.ai`), you can configure Cekura to use your regional endpoint.

### How to Configure

1. Navigate to **Agent Settings** in the Cekura dashboard.
2. Go to **Voice Integration** and select **Synthflow** as the provider.
3. In the **Synthflow Base URL Override** field, enter your regional API endpoint URL.

### Example

For EU-based Synthflow accounts, enter:
```
...
When creating or updating an agent via the API, include the `synthflow_data` field with the `synthflow_base_url_override` property:

```json
{
  "synthflow_data": {
    "synthflow_base_url_override": "https://api.eu.synthflow.ai"
  }
}
```

If not specified, Cekura uses the default Synthflow API endpoint (`https://api.synthflow.ai`).


## Does Synthflow support auto-sync prompts in Cekura?

Yes. When you configure a Synthflow agent in Cekura, you can enable the **Auto-sync Prompts** toggle in the agent's voice integration settings. With auto-sync enabled, Cekura pulls your agent's prompt from the Synthflow API every 30 seconds and updates the agent description field.

### How to Enable

1. Navigate to **Agent Settings** in the Cekura dashboard.
2. Go to **Voice Integration** and select **Synthflow** as the provider.
3. Enter your Synthflow **API Key** and **Model ID**.
4. Toggle on **Auto-sync Prompts**.

New Synthflow agents created in Cekura have auto-sync enabled by default.

<Note>
Auto-sync requires a valid Synthflow API Key and Model ID. If credentials become invalid or the agent is deleted from Synthflow, auto-sync will automatically disable itself.
</Note>
