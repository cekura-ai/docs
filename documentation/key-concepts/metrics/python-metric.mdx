...
| <span title="Provider execution logs as list of entries">**logs**</span> | ✅ | ✅ |
...
<AccordionGroup>
  ...
  <Accordion title="logs" href="#logs">
    **Availability:** ✅ Simulation | ✅ Observability

    Provider execution logs as a list of entry dicts. Only populated when execution logs were captured for the call or run.

    Each entry has:
    - `ts` — timestamp (ISO 8601, unix epoch, or seconds into the call)
    - `level` — severity (`info`, `warn`, `error`, `debug`)
    - `message` — what happened
    - `logger` (optional) — the component that emitted the line
    - `raw` (optional) — extra provider-specific detail
    ...
    # Example:
    # [
    #   {"ts": "00:05", "level": "info", "message": "Final transcript | provider=deepgram confidence=0.98"},
    #   {"ts": "00:07", "level": "error", "message": "Tool call failed: get_order timed out after 5s",
    #    "raw": {"tool": "get_order", "timeout_ms": 5000}}
    # ]

    # Check for tool-call failures
    tool_errors = [e for e in logs if e.get("level") == "error" and "tool" in e.get("message", "").lower()]
    ```
  </Accordion>
</AccordionGroup>
...
# Available Data 
{
  ...
  "logs": [
    {"ts": "00:05", "level": "info", "message": "Final transcript received"},
    {"ts": "00:07", "level": "error", "message": "Tool call failed: get_order timed out after 5s",
     "raw": {"tool": "get_order", "timeout_ms": 5000}}
  ],
