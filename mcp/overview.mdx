...
2. **Node.js 20.18.1 or higher** installed (only if you're configuring Cursor, VS Code, or another client that uses the `mcp-remote` shim — Claude Code and Claude Desktop don't need it)
   - Check your version: `node --version`
   - Download from: [nodejs.org](https://nodejs.org/)

## 1. Choose Your Auth Method

Most users should pick **OAuth**. Only switch to **API Key** if one of the conditions below applies.

<CardGroup cols={2}>
  <Card title="OAuth (recommended)" icon="user-shield" color="#A6A7EA">
    One-click browser sign-in, no keys to manage. Session ties to your dashboard user. This is the default for individual use.
  </Card>
  <Card title="API Key (only if you need it)" icon="key" color="#A6A7EA">
    Use **only** when you need a project- or org-scoped credential, want to share one key across CI / teammates, or have a client that doesn't support OAuth. Generate one in **Settings → API Keys** in the [dashboard](https://dashboard.cekura.ai).
  </Card>
</CardGroup>
...
<Tabs>
  <Tab title="OAuth (recommended)">
    **This is the recommended path.** Skip to the API Key tab only if you need project/org scoping or a shared credential.
