...
| Method | Endpoint |
|--------|----------|
| `POST` | `https://new-prod.cekura.ai/test_framework/v1/scenarios-external/run_scenarios/` |
...
curl -X POST https://new-prod.cekura.ai/test_framework/v1/scenarios-external/run_scenarios/ \
...
def run_evaluators(api_key, evaluators, agent_id=None, freq=1):
    url = 'https://new-prod.cekura.ai/test_framework/v1/scenarios-external/run_scenarios/'
...
async function runEvaluators(apiKey, evaluators, agentId=null, freq=1) {
  const url = 'https://new-prod.cekura.ai/test_framework/v1/scenarios-external/run_scenarios/';
...
| Method | Endpoint |
|--------|----------|
| `POST` | `https://new-prod.cekura.ai/test_framework/v1/scenarios-external/run_scenarios_text/` |
...
```bash Curl
# Running using evaluator IDs
curl -X POST https://new-prod.cekura.ai/test_framework/v1/scenarios-external/run_scenarios_text/ \
  -H "X-VOCERA-API-KEY: <your-api-key-here>" \
  -H "Content-Type: application/json" \
  -d '{
      "scenarios": [<evaluator-id-1>, <evaluator-id-2>, ...],
...
def run_evaluators(api_key, evaluators, agent_id=None, frequency=1):
    url = 'https://new-prod.cekura.ai/test_framework/v1/scenarios-external/run_scenarios_text/'
...
async function runEvaluators(apiKey, evaluators, agentId=null, frequency=1) {
  const url = 'https://new-prod.cekura.ai/test_framework/v1/scenarios-external/run_scenarios_text/';
