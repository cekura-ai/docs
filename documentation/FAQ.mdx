...
#### 5. Credit Allocation on Enterprise Contracts

For enterprise contracts, the timing of credit allocation matches the payment option selected at signing:

- **Annual payment** — the full contract's credits are allocated upfront for the year.
- **Quarterly payment** — credits are allocated in quarterly tranches.
- **Monthly payment** — credits are allocated monthly.

If you are unsure which payment schedule applies to your contract, or if your available credit balance doesn't match your expectation, contact [support@cekura.ai](mailto:support@cekura.ai) or reach out via your dedicated Slack support channel.
...
## Why am I getting a "402 Payment Required" error when triggering scenario runs, even though I have credits or auto-refill enabled?

When you trigger a batch of scenario runs via the API or dashboard, Cekura performs an upfront credit check before starting any runs. The check requires your current balance to be at least **5 credits × the number of runs** in the batch. If your balance is below this minimum at the moment the request is made, the API returns a `402 Payment Required` error — even if your account has auto-refill configured.

**Why auto-refill may not prevent this**

Auto-refill triggers when your balance drops below your configured auto-refill threshold. If that threshold is set lower than the per-batch minimum (5 × number of runs), the refill will not fire in time to satisfy the upfront check. For example, triggering 100 runs requires 500 credits upfront; if your auto-refill threshold is set to 125, auto-refill will not kick in before the 402 error is raised.

**To resolve this:**
- **Add credits immediately**: Purchase credits from **Settings → Billing** to bring your balance above the required minimum for the batch.
- **Raise your auto-refill threshold**: Set your threshold to at least 5 × your typical batch size so that automatic top-ups occur before the credit check fails.
...
<Note>
When you trigger a batch of runs that exceeds your concurrency limit, Cekura displays an informational warning banner on the result page. This is not an error — it indicates that runs are being processed in parallel batches up to the limit, with the remainder queued automatically until capacity is available.
</Note>
...
The Developer plan is seat-based: the first user is free, and each additional user on the same plan costs $30/user/month. Credits are purchased separately and are not bundled with the seat fee.
