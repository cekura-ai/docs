...
1. You upload an audio file to the scenario (from the editor UI or the API), optionally giving it a human-readable `name`.
...
3. Reference the clip with `<audio id="name"/>` in any fixed-message action. You can write this tag by hand or let the upload endpoint insert it when you pass `condition_id`.
4. The same clip can appear in multiple conditions and even multiple times in one action — every reference plays the same recording.
5. When a condition fires, text, recordings, and tags run from left to right.

<Warning>
The transcript is **derived from the recording** and is read-only. References resolve at **run time**, not at save time — you can write the tag before uploading the clip (or upload before writing the tag). The run gate blocks until every referenced clip is `ready`.
</Warning>
...
```bash Upload
# Add a named clip to the scenario, attached to condition 2
...
```bash Upload (unattached)
# Add a clip without inserting it into any action — reference it yourself
curl -X POST "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/" \
  -H "Authorization: Bearer $CEKURA_API_KEY" \
  -F "name=hold-music" \
  -F "file=@hold.mp3"
# Then paste <audio id="hold-music"/> into as many actions as needed
```

```bash List
# List every clip on the scenario with its references
curl "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/" \
  -H "Authorization: Bearer $CEKURA_API_KEY"
```

```bash Replace
# Swap a clip's audio in place — every reference picks up the new recording
curl -X PUT "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/greeting/" \
  -H "Authorization: Bearer $CEKURA_API_KEY" \
  -F "file=@new-greeting.mp3"
# 202 Accepted — re-transcribes; runs blocked until ready
```

```bash Delete
# Remove a clip and strip every <audio> reference to it
...
<ParamField path="name" type="string">
  The clip's id, used in its `<audio>` tag and as its stored object name. Letters, digits, underscores, and hyphens; max 64 characters. Must be unique within the scenario (case-insensitive). Defaults to a generated id when omitted.
</ParamField>

<ParamField path="condition_id" type="integer">
  Attach the clip to this condition, inserting the tag into its action. The condition must have `fixed_message: true`. Omit to add the clip without touching any action — then reference it yourself with `<audio id="..."/>` from as many actions as needed.
</ParamField>
...
<ParamField path="replace_audio_id" type="string">
  Requires `condition_id`. Points that condition's existing tag at the new clip. To swap a clip's audio while keeping every reference to it, use the PUT endpoint instead.
</ParamField>
