...
The `group_by` parameter accepts a JSON object:

```json
{
  "type": "field",
  "field": "metric_evaluations.value"
}
```

| Property | Type | Description |
|----------|------|-------------|
| `type` | string | Grouping type. Currently supports `field`. |
| `field` | string | Field path to group by (e.g., `metric_evaluations.value`) |

<Note>
Grouping is only supported for the `metric_evaluations.value` field on `bar` and `line` charts. When `group_by` is set, the chart aggregates counts per time bucket for each distinct metric value.
</Note>

**Example use case:** To visualize how metric scores are distributed over time, set `field` to `metric_evaluations.value`, `chart_type` to `bar`, and `group_by` to `{"type": "field", "field": "metric_evaluations.value"}`. The chart shows a stacked or grouped bar for each distinct score value (e.g., 1, 2, 3, 4, 5) per time period.
