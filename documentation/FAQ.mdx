...
**Note:** If you want to use your own phone numbers (Bring Your Own Number), you can find import instructions for [Twilio](/documentation/key-concepts/phone-numbers/twilio) and [Plivo](/documentation/key-concepts/phone-numbers/plivo). Both providers let you import any number — including non-US numbers such as Indian (+91) numbers — into Cekura. No additional integration is required on your side: once the number is imported, outbound tests simply place a regular phone call to the Cekura-provided number using your imported number as the caller ID.
...
## What does the `NORMAL_TEMPORARY_FAILURE` hangup cause mean, and how can I reduce it?

`NORMAL_TEMPORARY_FAILURE` is a telephony error that originates from the carrier network, not from Cekura or your agent configuration. It indicates the carrier temporarily could not route the call — typically due to network congestion or transient routing issues. It is intermittent by nature and usually resolves on its own, but you can take steps to reduce the frequency:

1. **Enable High Availability Numbers** — In your telephony provider's console (for example, the Plivo console), enable high-availability routing for your phone numbers. This activates more resilient carrier paths so calls are less likely to hit a temporary failure.
2. **Configure carrier failover** — Enable fallback carrier routing. When the primary carrier returns a temporary failure, the call is automatically rerouted through a secondary carrier instead of dropping.
3. **Monitor via call metadata** — Track `hangup_cause` values by analysing call metadata in Cekura. This lets you identify whether failures are concentrated on specific carriers, numbers, or regions, which helps narrow down whether the issue is environmental or systematic. See [Metadata in metrics](/documentation/key-concepts/metrics/basic-advanced-metrics#2-metadata) for details on accessing metadata in your evaluations.
