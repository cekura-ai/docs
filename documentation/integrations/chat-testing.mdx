...
Retell chat runs work out of the box with just a voice agent configured — Cekura automatically clones a temporary chat agent from your voice agent for each run. Optionally, you can point chat runs at a dedicated chat agent.

<Steps>
  <Step title="(Optional) Create a Dedicated Chat Agent in Retell">
    If you want chat runs to target a specific chat agent:
    ...
    > **Note:** When no chat agent ID is configured, Cekura clones a temporary chat agent from the voice agent automatically for each run.
  </Step>

  <Step title="Configure in Cekura">
    1. Go to **Agent Settings** → **Configuration** → **Provider**, select **Retell**
    2. Under **Integration Settings**, enter the **External Assistant ID** of your Retell voice agent and your **Retell API Key**
    3. Optionally, enter the chat agent ID if you created one in the previous step
...
<Info>
Pipecat Chat runs against your Pipecat Cloud deployment over RTVI text messages. The same bot serves voice and text. See the [Pipecat Chat Testing guide](/documentation/integrations/pipecat/chat-testing) for the Daily/RTVI flow, SDK behaviour, and readiness errors.
</Info>

<Steps>
  <Step title="Enable RTVI in your bot">
    Create the pipeline task with `enable_rtvi=True` and keep the Daily transport branch, which is what Pipecat Cloud sessions use.
  </Step>

  <Step title="Configure in Cekura">
    1. Go to **Agent Settings** → **Configuration** → **Provider**, select **Pipecat**
    ...
    <img src="/images/pipecat/chat-agent-settings.png" alt="Pipecat Chat connection in Cekura agent settings" />
  </Step>
  ...
  <Step title="Run Tests">
    ...
    <Note>Mock tools are not available for Pipecat Chat runs.</Note>
  </Step>
</Steps>
...
<Info>
For voice Personas, automatic configuration, outbound calls, and mock tools, see the [Bland AI integration guide](/documentation/integrations/bland).
</Info>
...
<Steps>
  <Step title="Start the run">
    ```bash
    curl --request POST \
      --url https://api.cekura.ai/test_framework/v1/scenarios/run_scenarios_api_chat/ \
      --header 'X-CEKURA-API-KEY: <api-key>' \
      --header 'Content-Type: application/json' \
      --data '{ "agent_id": 123, "scenarios": [456], "frequency": 1, "timeout": 1800 }'
    ```

    Each `runs[].id` in the response is a conversation to drive. `timeout` is the overall run lifetime in seconds (defaults to your project setting, max 3600).
  </Step>

  <Step title="Wait until the run is ready">
    Poll the run until its `status` is `in_progress` (it opens once capacity is available). Stop if it reaches `completed`, `failed`, `timeout` or `cancelled` first — the run ended before it opened, so there is nothing to drive:

    ```bash
    curl --request GET \
      --url https://api.cekura.ai/test_framework/v1/runs/<run_id>/ \
      --header 'X-CEKURA-API-KEY: <api-key>'
    ```

    Then fetch the opening as turn 0 — `GET /runs/<run_id>/chat/0/` returns the scenario's opening line in `message` (the simulated user speaks first), or `null` when you speak first.
  </Step>

  <Step title="Submit a turn">
    Send your agent's latest message. The reply is generated in the background, so this returns a `turn_id` to poll.

    ```bash
    curl --request POST \
      --url https://api.cekura.ai/test_framework/v1/runs/<run_id>/chat/ \
      --header 'X-CEKURA-API-KEY: <api-key>' \
      --header 'Content-Type: application/json' \
    ...
    ```json
    { "run_id": 987, "result_id": 654, "turn_id": 1, "status": "generating" }
    ```

    <Info>
    Submitting a new turn while the previous one is still generating interrupts it — the older turn becomes `dropped` but its message is kept, and the new reply answers all messages received since the last reply.
    </Info>
  </Step>

  <Step title="Poll for the reply">
    Poll the turn until `status` is `ready`; the reply is in `message`. Feed it to your agent and submit the next turn. Repeat until `ended` is `true`.

    ```bash
    curl --request GET \
      --url https://api.cekura.ai/test_framework/v1/runs/<run_id>/chat/<turn_id>/ \
      --header 'X-CEKURA-API-KEY: <api-key>'
    ```

    ```json
    { "run_id": 987, "result_id": 654, "turn_id": 1, "status": "ready",
      "message": "Hi! I'd like to check the status of my order.", "ended": false, "ended_reason": null }
    ```

    Statuses: `generating` (keep polling), `ready` (reply in `message`), `dropped` (interrupted by a newer turn — its message is kept and the newer reply answers it), `error` (submit again). Turns stay pollable after the run finishes.
  </Step>

  <Step title="Finish (optional)">
    End explicitly to score it right away; otherwise it finalizes when the simulated user ends the chat or `timeout` elapses.

    ```bash
    curl --request POST \
      --url https://api.cekura.ai/test_framework/v1/runs/<run_id>/end-chat/ \
      --header 'X-CEKURA-API-KEY: <api-key>' \
      --header 'Content-Type: application/json' \
      --data '{ "reason": "client-finished" }'
    ```

    Results appear in the dashboard with the full transcript and metrics, like any other run.
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Pipecat Chat run fails before the conversation starts">
    The run ended with a `pipecat-*` readiness reason. The most common causes are RTVI disabled on the bot (`enable_rtvi=False`), a wrong agent name, an invalid API key, or no deployment capacity. See [readiness errors](/documentation/integrations/pipecat/chat-testing#troubleshooting-and-readiness-errors) for every reason.
  </Accordion>
