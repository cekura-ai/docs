...
<Steps>
  <Step title="Optionally Create a Dedicated Chat Agent">
    By default, Retell chat runs clone a temporary chat agent from the voice agent automatically — no separate agent needed. If you want to point chat runs at a specific Retell chat agent, create one in the Retell Dashboard:
  ...
  <Step title="Connect Retell to Cekura">
    ...
    3. Under **Integration Settings**, enter the **External Assistant ID** of your Retell chat agent (if you created one in the previous step).
    4. Enter your **Retell API Key** in the designated field.
    5. Click **Save Changes**.

       ![Retell Agent Settings](/images/retell/agent-settings.png)

       > **Important:** If you provide a chat agent ID, chat runs use that agent. Otherwise, Retell clones a temporary chat agent from the voice agent for each run.
