...
<Steps>
  <Step title="Navigate to Evaluators and Select Scenarios">
    ...
    <Note>
      Evaluators must be selected individually when creating a cron job. Tag-based selection (running every evaluator matching a given tag) is not currently supported — add each evaluator to the job explicitly.
    </Note>
  </Step>
