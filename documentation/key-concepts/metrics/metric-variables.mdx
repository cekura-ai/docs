...
Test profile `information` is sectioned into `main_agent_variables` (sent to the agent under test) and `testing_agent_variables` (persona for the simulated caller). In metric prompts, `{{test_profile.<key>}}` resolves against the merged view of both sections — metrics run after the call and can see the full context regardless of which section a key lives in. Legacy flat profiles resolve directly against the flat dict.
