...
<Note>
Website scraper connectors **do not support automatic syncing**. The API rejects any attempt to enable auto-sync for website connectors. To update content, use the manual sync endpoint (`POST /v1/knowledge-base-connectors/{id}/sync/`) instead. Other connector types (like BigQuery) support auto-sync and have it enabled by default.
</Note>
...
Website connectors receive an initial sync when created. To update content afterward, trigger a manual sync:

```bash
POST /v1/knowledge-base-connectors/{id}/sync/
```

This returns HTTP 202 and queues the sync operation.
...
- **No automatic syncing**: Website connectors do not support automatic periodic syncing. Use the manual sync endpoint to update content.
- **100-page limit**: Website scraping is limited to 100 pages maximum per sync operation.
...
5. **Schedule Manual Syncs**: Since website connectors do not support auto-sync, schedule manual sync calls as needed to keep content updated
...
### "Website connectors do not support automatic syncing"

- This is expected behavior. Website scraper connectors do not support automatic periodic syncing.
- Use the manual sync endpoint (`POST /v1/knowledge-base-connectors/{id}/sync/`) to update content when needed.
- If you need automatic syncing, consider other connector types like BigQuery.
...
- Trigger manual syncs to update content
