...
Retention for monitoring data (call recordings, transcripts, and observability data) depends on your plan:

- **Pay-as-you-go (Developer) plan**: Call log and observability data is retained for **30 days**, after which it is automatically removed. This limit applies to both newly captured data and any existing call logs — once the 30-day window passes, older records are removed regardless of when they were originally ingested.
- **Annual and Enterprise plans**: Data is retained for **one year** by default. Custom retention windows (shorter or longer) are available — contact our support team or your account manager to arrange an exception.
...
If you need to delete monitoring data before your plan's retention period — for compliance or privacy reasons — you can do so at any time using Cekura's delete endpoint. If your organisation requires a custom retention window, contact our support team.
