...
<AccordionGroup>
  <Accordion title="<ivr> - Interactive Voice Response" icon="volume-high">
    Mark a message as an IVR (Interactive Voice Response) message that cannot be interrupted. The tag name must be lowercase.
    ...
    <Warning>
    `<ivr>` must be the **entire action** — it cannot be combined with other text or tags in the same action string. The tag must be lowercase (`<ivr>`, not `<IVR>` or `<Ivr>`).
    </Warning>
  ...
  <Accordion title="<voicemail> - Voicemail Greeting" icon="voicemail">
    Play a voicemail greeting with beep. The tag name must be lowercase.
    ...
    <Warning>
    `<voicemail>` must be the **entire action** — it cannot be combined with other text or tags in the same action string. The tag must be lowercase (`<voicemail>`, not `<VOICEMAIL>` or `<Voicemail>`).
    </Warning>
