...
## What voice AI providers are most commonly used in production, and how do native realtime AI models compare?

### Established TTS Providers

**ElevenLabs** and **Deepgram** are the most widely adopted TTS providers for production voice agent deployments, offering a reliable balance of voice quality, latency, and cost. Most teams building production voice agents use one of these two.

### Native Realtime AI Models

Native realtime AI models (such as OpenAI Realtime API and Gemini Live) are an emerging category but currently see limited production adoption for complex voice agents. Key limitations to consider:

- **Tool call support**: Realtime models have less mature support for tool calls compared to standard LLM + TTS pipelines.
- **Voice output control**: Voice characteristics are harder to configure compared to dedicated TTS providers.
- **Interruption and turn-taking**: VAD behavior can be inconsistent in complex, multi-step interactions.

**Multilingual use cases** are one area where realtime AI models show more competitive performance.

### Choosing a Provider

For production agents requiring reliable tool calls, controllable voice output, and natural interruption handling, established TTS providers remain the standard choice. Realtime AI models tend to be better suited for simpler agent use cases or scenarios where multilingual support outweighs the need for fine-grained voice control.

Cekura supports testing with all major provider types. Reach out via your support channel for guidance tailored to your specific use case.
