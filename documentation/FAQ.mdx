...
## Why am I getting a 405 error when trying to create results via the API?

The API returns a 405 Method Not Allowed response because direct result creation via `POST /test_framework/v1/results/` is not supported. Results are automatically generated by the system when you run evaluators.

### How Results Are Created

Results in Cekura are created automatically when you execute test runs. You cannot create results directly through a POST request to the results endpoint. Instead, follow the standard testing workflow:

1. **Create an Agent**: Set up the AI agent you want to test.
2. **Define Evaluators**: Create test cases that specify the conversation flow and expected outcomes.
3. **Run Tests**: Execute the evaluators against your agent using the dashboard or the [Run Scenarios API](https://docs.cekura.ai/api-reference/test_framework/post-test_frameworkv1scenarios-externalrun_scenarios).
4. **View Results**: Once the test completes, results are automatically generated and available via the [List Results API](https://docs.cekura.ai/api-reference/test_framework/list-results) or the dashboard.

### Available Result Operations

While you cannot create results directly, you can perform other operations on existing results:
- **List Results**: Retrieve all test results
- **Get Result**: Fetch a specific result by ID
- **Rerun Tests**: Re-execute tests on an existing result
- **Evaluate Metrics**: Run additional metrics on a result
