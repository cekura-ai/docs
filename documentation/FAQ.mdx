...
If the reactivation option is not available in the Stripe portal, or if your plan does not reactivate automatically, contact support and the team can activate it for you directly.
...
## Why am I getting a 400 error when setting dynamic_variable_values on a scenario?

The `dynamic_variable_values` field has been removed from the Scenario API. Submitting a non-null value for this field in a POST or PATCH request returns a 400 error directing you to the new approach.

### Migration path

Store dynamic-variable values in a test profile instead:

1. **Create a test profile** via `POST /test-profiles/` with an `information` object that uses the sectioned shape:
   ```json
   {
   ...
   ```
2. **Attach the test profile** to your scenario using the `test_profile` field on the scenario create or update request.

### Why the change?

The sectioned test profile structure separates values that reach your agent under test (`main_agent_variables`) from persona context used by the evaluator (`testing_agent_variables`). This gives you precise control over which data flows where. See [Test Profile — Variable Sections](/documentation/key-concepts/evaluators/test-profile#variable-sections) for details.

### Legacy scenarios

Existing scenarios with `dynamic_variable_values` already stored continue to work at runtime — the values are still delivered to your agent. Only new writes via the API are rejected.
