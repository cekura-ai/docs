...
<Note>
Website scraper connectors have auto-sync **disabled by default**. The dashboard does not display sync settings for website connectors. To enable automatic periodic syncing, use the API to explicitly set `sync_enabled` to `true` when creating or updating the connector. Other connector types (like BigQuery) have auto-sync enabled by default.
</Note>
...
- The dashboard does not display sync settings for website connectors
- Enable auto-sync using the API by setting `sync_enabled` to `true` when creating or updating the connector
- Verify the connector was created with auto-sync enabled, or update it via API to enable syncing
