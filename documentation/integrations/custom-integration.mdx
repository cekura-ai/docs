...
<Note>
**This webhook is for simulation testing** — sending transcripts for calls that Cekura's Testing Agent places as part of your evaluators. If you want to monitor your actual production calls (calls that do not originate from Cekura), use the [Custom Observability endpoint](/documentation/guides/observability/custom) instead.

For simulation calls, Cekura already captures the audio recording on its side, so you do not need to include a `voice_recording_url` in the webhook payload. Audio-based metrics (latency, transcription accuracy, etc.) will still work without it. The webhook is primarily for sending your transcript data — in particular, any `function_call` and `function_call_result` messages — so that tool-based metrics have the information they need to evaluate your agent.
</Note>
