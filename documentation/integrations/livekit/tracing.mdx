...
export const livekitPrompt = [
  ...
  "- Optional for Python SDK 1.6.5+: pass async_recording=True to run egress-related APIs in the background and avoid adding their latency to session startup. This can miss opening audio while egress starts; the default False waits for recording to start.",
  "",
  ...
  "1. Install the SDK: pip install \"cekura[livekit]>=1.6.7,<2\"",
...
```bash
pip install "cekura[livekit]>=1.6.7,<2"
```
...
<Note>
With Python SDK 1.6.5 or later, you can pass `async_recording=True` to run egress-related APIs in the background. This avoids adding their latency to session startup. Transcript, tool-call, metric, and session-report collection are unaffected, but audio produced before egress is ready may be absent from the recording. The default is `False`, which waits for recording to start and preserves opening audio.
</Note>
...
For the LiveKit integration, use Cekura Python SDK `>=1.6.7,<2`. Updates within the `1.x` release line are backward compatible.

| Package | Supported versions |
|---|---|
| Python | `>=3.10` |
| Cekura Python SDK | `>=1.6.7,<2` |
| `livekit-agents` | `>=1.3.12,<2` |
| `livekit` (RTC SDK) | `>=1.0.23,<2` |
| `livekit-api` | `>=1.0.7,<2` |
| OpenTelemetry | `>=1.39,<2` |
...
await cekura.observe_session(
    ...
    session,               # Required: LiveKit AgentSession
    capture_logs=True,     # Optional: Capture session logs (default: True)
    async_recording=False, # Optional: Run egress-related APIs in background
    **metadata             # Optional: Custom metadata
)
```

Set `async_recording=True` to keep egress-related APIs off the session-start path. The SDK waits for background startup during shutdown so successful recording identifiers are included in the final export. Audio generated before egress becomes ready may not be recorded.
