...
| `outbound_phone_number` | no | For an inbound voice agent, the number Cekura dials, in E.164 format. Defaults to `agent_number`, then the agent's configured number. For `text`, the number Cekura messages from. `voice` and `text` only. |
| `mode` | no | `same_number` (default) places every call from one Cekura number; `different_numbers` spreads calls across your pool. `voice` only. |
...
The same endpoint is also served at `POST /test_framework/v2/scenarios/run_scenarios_json/`, and `validate_scenarios_json` and `json_schema` likewise have `v2` paths. The `v1` and `v2` paths behave identically for spec runs — use either.
...
- **`outbound_phone_number`** sets the number Cekura dials when your agent receives calls. If you only pass `agent_number`, Cekura dials that.
...
- **`mode`**, **`mock_tool_names`** and **`agent_version_id`** work as they do on evaluator runs.
...
A dry run echoes the overrides it applied under `plan.run_overrides`, so you can confirm them before placing any calls.
