...
icon: "user-cog"
iconType: "regular"
description: "Personalities in Cekura define how the simulated caller behaves when it tests your agent — its voice, language, speaking pace, interruption and idle timing, background noise and persona. A personality is the caller; an evaluator is the test case that caller runs."
---
...
## Personality or evaluator?

A personality configures the **testing agent** — Cekura's simulated caller — not the agent under test. It is worth being precise about the split, because the two are not interchangeable:

| | Personality | Evaluator |
|---|---|---|
| Answers | Who is calling, and how do they sound? | What are they trying to get done? |
| Owns | Voice and accent, language, speaking speed, interruption timing, idle timing, background noise, volume, network conditions, persona prompt | The objective, the caller's turns, the expected outcome, the attached metrics |
| Scope | Every call it is assigned to | One test case |

<Warning>
**Evaluator instructions cannot change anything on this page.** Writing "speak with a strong Indian accent", "interrupt constantly", or "stay silent for 30 seconds" into an evaluator's instructions has no effect on the audio — the call runs, the result comes back, and the behavior never happened. Anything that lasts the whole call belongs on a personality.
</Warning>

A trait that lasts the whole call (an accent, a pace, ambient noise, a bad line) is a personality. Something that happens at one moment ("gets frustrated at step 4", one interruption) belongs in the evaluator's instructions.

## Accent comes from the voice

A personality's accent is a property of the **voice** you select, not a field you fill in. Speech is synthesised from the voice ID alone, so testing a different accent always means choosing a different voice.

The `accent` shown on a personality is a **read-only label derived from that voice**, which is why there is no accent input on the create form. It is empty for voices whose provider publishes no accent label (all Cartesia voices) — empty means "unlabelled", not "accent-free".

To pick a voice by accent, browse the catalog:

- `GET /test_framework/v1/personalities/elevenlabs_voices/?language=en` — includes an accent label per voice
- `GET /test_framework/v1/personalities/cartesia_voices/?language=en` — no accent label; read the name and description

See [Accent Testing](/documentation/guides/testing-agents/accent-testing) for the full workflow.
...
- **Personality Prompt**: Define how the simulated caller behaves and responds throughout the call — its persona, tone and attitude. This shapes word choice, not pronunciation: an accent written here will not change how the voice sounds.

The language model driving the simulated caller is managed by Cekura and is not configurable per personality.
...
- **Voice ID**: The voice the caller speaks with. This determines the accent and the perceived gender, both of which are then shown on the personality as read-only labels. Preview voices in the picker before choosing.
- **Voice Provider**: ElevenLabs or Cartesia. The voice model must belong to the selected provider (`eleven_*` models with ElevenLabs, `sonic-3.5` with Cartesia).
