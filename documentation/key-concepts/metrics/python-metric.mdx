...
icon: "code"
iconType: "solid"
description: "Write custom evaluation logic in Python to evaluate your AI agent's performance with complete control"
---
...
| <span title="Structured transcript with timing and speaker info">**transcript_json**</span> | ✅ | ✅ |
| <span title="Call duration in seconds">**call_duration**</span> | ✅ | ✅ |
| <span title="Reason why the call ended">**call_end_reason**</span> | ✅ | ✅ |
| <span title="URL to the voice recording file">**voice_recording**</span> | ✅ | ✅ |
| <span title="Description of the AI agent">**agent_description**</span> | ✅ | ✅ |
...
| <span title="Audio metadata and analysis results">**recording_data**</span> | ✅ | ✅ |
...
| <span title="Dynamic variables from API (Observability only)">**dynamic_variables**</span> | ❌ | ✅ |
| <span title="CallLog ID (Observability only)">**call_log_id**</span> | ❌ | ✅ |
| <span title="Tags from scenario (Simulation only)">**tags**</span> | ✅ | ❌ |
| <span title="Provider call identifier (Simulation only)">**provider_call_id**</span> | ✅ | ❌ |
| <span title="Provider-specific call details (Simulation only)">**provider_call_data**</span> | ✅ | ❌ |
| <span title="Cekura transcript format (Simulation only)">**cekura_transcript_json**</span> | ✅ | ❌ |
| <span title="Test scenario data (Simulation only)">**test_profile**</span> | ✅ | ❌ |
| <span title="Run ID (Simulation only)">**run_id**</span> | ✅ | ❌ |
| <span title="Expected outcome value (Simulation only)">**expected_outcome**</span> | ✅ | ❌ |
| <span title="Expected outcome explanations (Simulation only)">**expected_outcome_explanation**</span> | ✅ | ❌ |
...
<ParamField path="audio" type="bool" default="False">
  Enable audio-based evaluation using Gemini multimodal. When `True`, the voice recording is analyzed rather than just the transcript. The recording is retrieved from the associated call log or run.
</ParamField>

<ParamField path="audio_start_time" type="float|None" default="None">
  Start time in seconds for evaluating a specific segment of the recording. Only applies when `audio=True`. Values exceeding the recording duration are clamped to the valid range.
</ParamField>

<ParamField path="audio_end_time" type="float|None" default="None">
  End time in seconds for evaluating a specific segment of the recording. Only applies when `audio=True`. Must be greater than `audio_start_time`. Values exceeding the recording duration are clamped to the valid range.
</ParamField>
...
Evaluate the voice recording rather than just the transcript:

```python
key = "<your_cekura_api_key>"

# Evaluate the full audio recording
response = evaluate_llm_judge_metric(
    data,
    key,
    description="Was the agent's greeting professional and warm?",
...
To evaluate a specific segment of the recording, specify start and end times:

```python
key = "<your_cekura_api_key>"

# Evaluate only the first 30 seconds (e.g., the greeting)
response = evaluate_llm_judge_metric(
    data,
    key,
    description="Did the agent introduce themselves and the company?",
...
For the evaluation step, call `evaluate_llm_judge_metric` with a description tailored to the audio content, or apply your own signal processing logic. See [Calling LLM Judge Metrics from Python](#calling-llm-judge-metrics-from-python) for the function reference.
