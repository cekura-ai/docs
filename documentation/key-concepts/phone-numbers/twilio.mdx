...
## Enabling WhatsApp for Testing

If you want to use a Twilio number for WhatsApp testing, you need to configure additional metadata:

<Steps>
  <Step title="Enable WhatsApp on Twilio">
    1. Log in to your [Twilio Console](https://console.twilio.com/).
  ...
  <Step title="Configure WhatsApp Metadata in Cekura">
    After importing your Twilio number, you need to add WhatsApp metadata:
    ...
    <Info>
      The `whatsapp_enabled` flag tells Cekura that this phone number can be used for WhatsApp testing. Without this flag, the number will only be available for voice and SMS testing.
    </Info>
  </Step>

  <Step title="Verify WhatsApp Configuration">
    Once configured, you can use this number for WhatsApp testing:
    - In **Agent Settings**, select this number as the contact number
    - Set **Chatbot Provider** to **WhatsApp**
    - Run tests via **Evaluators** → **Run Test as Text**

    See the [WhatsApp Testing Guide](/documentation/guides/testing-agents/whatsapp-testing) for detailed usage instructions.
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="WhatsApp Tests Not Working">
    **Solution:**
    - Verify WhatsApp is enabled on your Twilio number in the Twilio Console
    - Ensure `whatsapp_enabled: true` is set in the phone number metadata in Cekura
    - Check that your Twilio WhatsApp Business Profile is approved
    - Confirm Twilio credentials are configured in **Settings → Provider API Keys**
  </Accordion>
</AccordionGroup>
