...
<Note>
The first message is sent **once** and is never retried. If the main agent's speech cuts it off, whatever had not played yet is dropped and never spoken later. A `<silence>` always gives way when the main agent starts talking, so a greeting that arrives during a leading `<silence>` drops the whole line; spoken text gives way according to the personality's interruption settings. The conversation then continues through the other conditions as usual. To let the main agent greet first and then say an exact line, leave `id: 0` empty and put the line on a `standard` condition with `id: 1` that matches the greeting:

```json
[
  { "id": 0, "condition": "FIRST_MESSAGE", "action": "", "type": "standard", "fixed_message": true },
  { "id": 1, "condition": "The agent greets the caller", "action": "Hi, I'm calling because my fare came out higher than the estimate.", "type": "standard", "fixed_message": true }
]
```
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="<speed> - Control Speech Speed" icon="gauge-high">
    ...
    The tag may appear anywhere in the action, and applies from that point until the
    next `<speed>` tag or the end of the action. Add `text` to scope it to just those
    words instead — the previous speed resumes afterwards:

    ```json
    {
      "action": "My date of birth is <speed ratio=\"1.8\" text=\"August fifth, nineteen seventy-nine\" />, got it?"
    }
    ```

    Speed is applied after synthesis and preserves pitch, so it works the same on
    every TTS provider. There is no `<speed>...</speed>` block form.

    **Attributes:**
    - `ratio`: Speed multiplier (required) - Valid range: **0.1 to 2.0**<br />
      0.8–1.2 keeps speech natural; beyond that it becomes a stress test.
    - `text`: Words to scope the ratio to (optional)<br />
      Cannot contain other tags — use the point form above if you need tags inside.
  </Accordion>
  ...
  <Accordion title="<volume> - Control Volume" icon="volume-up">
    ...
    Like `<speed>`, it may appear anywhere and holds until the next `<volume>` tag or
    the end of the action, and `text` scopes it to just those words:

    ```json
    {
      "action": "My card number is <volume ratio=\"0.3\" text=\"four two one nine\" /> — did you catch that?"
    }
    ```

    Volume is applied as amplitude scaling after synthesis, so it works on every TTS
    provider. There is no `<volume>...</volume>` block form.

    **Attributes:**
    - `ratio`: Volume multiplier (required) - Valid range: **0 to 2**<br />
      Examples: `"1.5"` for 50% louder, `"0.5"` for 50% quieter. Above `1.0` the
      output is clipped against the TTS signal, so prefer `<= 1.0` unless distortion
      is what you are testing.
    - `text`: Words to scope the ratio to (optional)
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="<interruption> - Interrupt After Timeout" icon="hand">
    ...
    <Warning>
    `<interruption>` has three requirements:
    1. The condition must have `type: "action_followup"`
    2. The tag must appear at the **very start** of the action string
    3. Something audible must follow it: spoken text, or a sound clip (`<noise>` or `<audio>`). A bare tag, or one followed only by `<silence>` or `<hold>`, is rejected
    </Warning>

    To cut in with a sound and no words, for example a cough over the agent:
    ...
    ```json
    {
      ...
      "type": "action_followup",
      "condition": 2,
      "action": "<interruption time=\"1.2s\" /> <noise sound=\"cough1\" />",
      "fixed_message": true
    }
    ```
...
<AccordionGroup>
  <Accordion title="<background_noise> - Background Sounds" icon="waveform-lines">
    ...
    **Attributes:**
    ...
    - `volume`: Volume level (optional) - between **0 and 1.0**; default applies if not specified
  ...
  <Accordion title="<noise> - Play Sound Effects" icon="music">
    ...
    **Attributes:**
    - `sound`: Sound name (required) - a name from the sound library below, or a direct `https://` URL to an audio file
    - `volume`: Volume level (optional) - between **0 and 1.0**
    - `time`: Duration in ms (optional)

    **Sound library**: `office`, `beep`, `cough1`, `cough2`, `female-crying`, `male-crying`, `ringback`. The library grows over time - newly added sounds work with the same syntax as soon as they are available.

    `ringback` is ~6 seconds of a phone ringback tone (one ring, the gap, and the start of the next). The crying sounds are ~10 seconds of a person sobbing. Use them to check whether your agent notices a caller in distress and asks if everything is okay; shorten with `time` if needed.

    ```json
    {
      "action": "<noise sound=\"female-crying\" time=\"6000\" /> Sorry... I just got some bad news."
    }
    ```
  </Accordion>
...
Attach a pre-recorded audio clip to a scenario and the testing agent **plays the recording instead of speaking the text with TTS** when a referencing condition fires. A single clip can be referenced from any number of conditions and even multiple times within one action. Use it when the exact voice, accent, tone, or non-speech sound matters — reproducing a real caller's recording, a specific IVR prompt, background audio a synthetic voice can't recreate, or a regression case captured from a live call.
...
1. You upload an audio file to the scenario, optionally attaching it to a condition (from the editor UI or the API). Each clip gets a human-readable **name** that becomes its id — for example, `greeting` or `hold-music`.
...
3. Reference the clip from any number of fixed-message actions by pasting its tag: `<audio id="greeting"/>`. You can write this tag by hand, or let the upload endpoint insert it for you when you pass `condition_id`.
4. When the condition fires, text, recordings, and tags run from left to right. The same clip plays wherever it is referenced.
...
Four endpoints on the scenario manage attached audio. Clips are uploaded as `multipart/form-data`.

<CodeGroup>

```bash Upload (with condition)
# Attach a clip named "greeting" to condition 2 of scenario 39001
...
```bash Upload (without condition)
# Add a clip to the scenario without touching any action
curl -X POST "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/" \
  -H "Authorization: Bearer $CEKURA_API_KEY" \
  -F "name=hold-music" \
  -F "file=@hold-music.wav"
# Then reference it from any action: <audio id="hold-music"/>
```

```bash List
# List every clip on the scenario with its current status and references
curl "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/" \
  -H "Authorization: Bearer $CEKURA_API_KEY"
```

```bash Replace
# Replace a clip's audio (every reference plays the new recording)
curl -X PUT "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/greeting/" \
  -H "Authorization: Bearer $CEKURA_API_KEY" \
  -F "file=@greeting-v2.mp3"
# Pass name=new-name to rename the clip at the same time
```

```bash Delete
# Delete a clip and remove every <audio> reference to it
curl -X DELETE "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/greeting/" \
  -H "Authorization: Bearer $CEKURA_API_KEY"
# Returns 409 if deleting would leave a non-opening action empty
```
...
```json
{
  "audio_id": "greeting",
  "condition_ids": [2, 5],
  "tag": "<audio id=\"greeting\"/>",
  "status": "ready",
  "transcript": "Hello, thanks for calling.",
  "duration": 1.78,
  "filename": "greeting.mp3",
  "playback_url": "https://…/greeting.wav"
}
```

The scenario's GET response also includes a read-only `condition_audio` summary with every clip's tag and referencing conditions, so an author editing `instructions` directly can discover which clip ids exist.

<ParamField path="name" type="string">
  The clip's id, used in its `<audio>` tag and as its stored object name. Letters, digits, underscores and hyphens, max 64 characters; must be unique within the scenario (case-insensitive). Omit to derive a readable name from the filename (lowercased, hyphenated, trimmed to 32 characters).
</ParamField>

<ParamField path="condition_id" type="integer">
  The `id` of the condition to attach the clip to. The condition must already exist and have `fixed_message: true`. Omit to add the clip to the scenario without touching any action — then reference it yourself with `<audio id="..."/>` from as many actions as needed.
</ParamField>
...
<ParamField path="action_template" type="string">
  Requires `condition_id`. The complete action with one `<audio />` marker where the new recording should be inserted. Existing managed audio tags must remain in the template.
</ParamField>

<ParamField path="replace_audio_id" type="string">
  Requires `condition_id`. Points that condition's existing tag at the new clip. To swap a clip's audio while keeping every reference to it, PUT to `condition-audio/{audio_id}/` instead.
</ParamField>
