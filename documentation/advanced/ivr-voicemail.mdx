...
**Testing agent repeats the voicemail message after it has been delivered**

If the testing agent plays the voicemail greeting and then loops back to repeat it, the scenario is missing an explicit end-call step after the voicemail is delivered. Once the voicemail message plays, the testing agent has no condition telling it what to do next, so it falls back to the closest matching condition — which is often the voicemail greeting itself.

Add a final condition that ends the call. Two approaches work:

- **Explicit end-call after delivery** — add a condition such as `"The voicemail greeting has been delivered and the message has been recorded"` with `action: "<endcall />"` and `fixed_message: true`. This closes the call once the message has played.
- **No-response condition** — add a condition that fires when the main agent does not respond within a set window (e.g., `"No response from the main agent for 10–15 seconds after the voicemail message"`) with the same `<endcall />` action and `fixed_message: true`.

Both approaches require `fixed_message: true` for the `<endcall />` action. Including either one of these as the last condition in your voicemail scenario ensures the testing agent has a clear exit path.
