...
## Is there a way to export call data or simulation run results, or share reports?

Yes, Cekura supports several options for exporting call data, simulation run results, and sharing reports.

### 1. CSV Export from the Calls (Observability) Page
...
### 2. CSV Export of Simulation Runs (Results Page)
On the **Results** page, you can select individual simulation runs using the checkboxes and export them as CSV. The available run filters on the Results page are evaluator name (via **Filter by evaluator name**) and agent (via the **Views** tab). Filtering by pass rate or score percentage is not currently a built-in filter option on the Results page — for exports filtered by custom criteria, contact the support team.

### 3. Dashboards with Daily Reports
