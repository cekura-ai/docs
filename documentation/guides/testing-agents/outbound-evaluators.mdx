...
iconType: "regular"
description: "This guide walks you through the process of testing outbound calls using evaluators."
---
...
<Note>
  **Custom agents and Trigger URL webhook:** If your agent uses a custom or non-standard voice stack (not Vapi, Retell, ElevenLabs, Bland, or LiveKit) and you want Cekura to notify your system when a run is ready — with dynamic variables delivered in the request body — see [Auto Outbound Calls](/documentation/guides/testing-agents/outbound-auto-call). That page covers the *Trigger URL* option, where Cekura sends a webhook to your endpoint so your code can initiate the dial.
</Note>
...
<AccordionGroup>
  <Accordion title="Calls show Busy / NoAnswer on your telephony provider but never reach Cekura">
    Two separate issues produce `Busy` / `NoAnswer` on your telephony provider side. Check them in order:

    **1. Dialed after the timeout window.** Each run's dial window opens when the run reaches `pending` status and closes after 300 seconds (`outbound_dial_window_closes_at`). If your agent dials the `outbound_number` after that deadline, Cekura drops the incoming call — and your telephony provider sees the same `Busy` / `NoAnswer` symptom as a caller-ID mismatch, with no audio and no descriptive SIP error. Check the run's `status` field: if it is `timeout` and your telephony logs confirm the call was placed, a late dial is the most likely cause. Dial as soon as a run's status reaches `pending` to stay within the window.

    **2. Caller ID mismatch.** Cekura rejects incoming outbound-test calls whose caller ID doesn't match a configured `contact_number` on any agent in your project. Open the agent under test and confirm its `contact_number` is set to the exact E.164 number your agent dials from. An empty or mismatched `contact_number` causes every outbound attempt to be silently dropped. After setting the correct `contact_number`, trigger a new run; existing runs that hit this state will remain in `timeout` and cannot be recovered.
  </Accordion>
