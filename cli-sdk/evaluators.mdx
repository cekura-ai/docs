...
<Note>
Every evaluator is assigned to a folder. If you omit `folder_path` when creating one via the API, it is automatically placed in the project's **default** folder. To put it in a specific folder, pass `folder_path` with a dot-separated path (e.g. `"Sales.Inbound"`). The folder must exist first — use the [Create Folder](/api-reference/test_framework/create-folder) endpoint to create it.
</Note>
