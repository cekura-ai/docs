...
## How can I correlate Cekura calls with my distributed tracing system using OpenTelemetry?

Cekura supports OpenTelemetry trace ID correlation, allowing you to link call logs and runs with traces in your observability backend. This is useful for end-to-end debugging across your voice agent infrastructure.

### Passing a Trace ID

When creating a call log via the API, include a `trace_id` field in your request payload. The trace ID must be a 32-character lowercase hexadecimal string, matching the standard OpenTelemetry format.
...
Once a call or run has an associated trace ID, you can retrieve the trace spans through the Cekura API:

- **For call logs:** `GET /observability/v1/call-logs/{id}/trace/`
- **For runs:** `GET /test_framework/v1/runs/{id}/trace/`

These endpoints return the trace ID and associated spans, enabling you to correlate Cekura evaluation data with your distributed tracing backend.

### Use Cases

- **Debugging latency issues:** Correlate slow evaluations with specific spans in your agent's execution pipeline.
- **Cross-system observability:** Link Cekura call data with traces from your LLM provider, telephony infrastructure, or backend services.
- **Production monitoring:** Include trace IDs when forwarding production calls to Cekura for unified observability.
