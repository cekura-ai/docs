...
<Tabs>
  ...
  <Tab title="Auto-fetch">
    Auto-fetch polls the VAPI API every 30 seconds to fetch completed calls—no webhook configuration required.

    **Prerequisites:**
    - Your **VAPI API key** configured in Cekura Agent Settings
    - Your **VAPI Assistant ID** configured in Cekura Agent Settings
    ...
    <Steps>
      ...
      <Step title="Verify calls are syncing">
        Once enabled, completed calls from your VAPI account will automatically appear in the **Calls** section.
      </Step>
    </Steps>
