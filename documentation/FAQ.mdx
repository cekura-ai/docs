...
*   **Monitoring & Observability (Transcription):** 0.1 credits per minute of audio, charged only when a call log arrives with a recording but no transcript. Transcription supports 40+ languages, including calls that switch language mid-conversation.
...
If that call is a **5-minute recording sent without a transcript**, add **0.5 credits** for transcription (5 minutes × 0.1 credits), for a total of **2.5 credits**. Calls sent with their own transcript are not charged for transcription.
...
Cekura does not currently store or display webhook delivery logs in the dashboard UI. You cannot view a history of outgoing webhook events, their delivery status, or your endpoint's responses directly from the dashboard.
...
For information on the structure and payload format of the webhooks Cekura sends, see the [Webhook Format](https://docs.cekura.ai/documentation/advanced/webhook-format) reference page.
...
Yes, Cekura supports several options for exporting call data and sharing reports.
...
Create a [dashboard](/documentation/guides/dashboards) using the metrics and breakdowns you need — call volumes, outcome distributions, metric scores, and more. Use the **⋮** menu on any dashboard to include it in a daily report email sent to project members. Daily reports can be routed to a specific address by configuring project membership. See [Configuring daily report recipients](#how-can-i-configure-daily-reports-to-be-sent-to-a-specific-email-address-instead-of-all-project-members).

### 3. Using the Cekura API
For custom or automated reporting, use the [Get Call API](https://docs.cekura.ai/api-reference/observability/get-call) to programmatically fetch call data and metric evaluations and format them to match your reporting requirements.
...
To request access to our SOC 2 report, please reach out to us via **support@cekura.ai** or through your dedicated account manager. A summary certificate is available on request. Sharing the full SOC 2 report requires an NDA to be signed first — contact us to initiate that process. Additional compliance information is available on our [Trust Site](https://tatva-labs-inc.trust.site/).
...
For most enterprise due diligence and legal review purposes, Cekura's published Terms of Service available on the [Cekura website](https://www.cekura.ai) is the standard document provided for customer review. If your enterprise contract or BAA references an "Enterprise Service Agreement," that term refers to the published Terms of Service — no separate document exists by that name.

If your legal team requires additional compliance agreements — such as a Business Associate Agreement (BAA) for HIPAA compliance, or a signed agreement tailored to your account — please reach out to your Cekura Account Manager or contact us at **support@cekura.ai**.
...
*   **Extra Phone Lines:** Additional phone lines can be provisioned to increase concurrent call capacity. Extra lines are priced at **$10 per line per month**, billed based on the number of days the line was active (prorated daily). For example, a single extra line used for one day in a 30-day month costs $10 ÷ 30 ≈ $0.33. Lines can be added and removed as needed for temporary testing.
...
The Cekura Slack integration uses a single setup path that covers both simulation test result notifications and live production call evaluation alerts. Once configured, the Cekura bot posts to your chosen channel whenever a metric evaluation completes — whether from a simulation run or a live production call.

To set up the integration:

1. Go to **Settings** in the Cekura platform.
2. Ensure you are in the **relevant project**.
3. Navigate to **Integrations** and click **Configure** next to **Slack**.
4. Follow the steps listed under 'How to setup Slack notifications' to connect your workspace.
5. **Add the Cekura Bot:** After the integration is set up, you must manually add the bot to your desired Slack channel. You can do this by typing `/invite @Cekura` in the channel or using the 'Add apps' option in Slack.
6. **Enable Notifications:** Make sure notifications are enabled in your project settings.
...
Cekura is fully GDPR compliant, ensuring that all data processing activities meet the rigorous standards set by the European Union for data protection and privacy. A Data Processing Agreement (DPA) is typically only available to customers on enterprise plans. If you are not on an enterprise plan, please reach out to **support@cekura.ai** or your dedicated account manager to share your use case and discuss options. For further documentation regarding our compliance standards, contact our support team.


## Does Cekura use customer data to train AI models, sell it to third parties, or share it for marketing?

No. Cekura does not use customer data to train AI models, does not sell it to third parties, and does not share it for marketing purposes. This is documented in Cekura's Privacy Policy and Terms of Service, available on the Cekura website.
...
To avoid vendor lock-in, Cekura provides full API access to retrieve your data at any time. See our [API Reference for Getting Call Data](https://docs.cekura.ai/api-reference/observability/get-call) for details on programmatically exporting call data and metadata.

For custom contract arrangements or enterprise pricing, contact [support@cekura.ai](mailto:support@cekura.ai).


## Does Cekura support VPC deployment, on-premises deployment, or bring-your-own-cloud (BYOC) setup?

Yes, Cekura supports VPC (Virtual Private Cloud) deployment for organisations that need private infrastructure. If your organisation requires a VPC deployment, on-premises setup, or a bring-your-own-cloud arrangement, reach out to your account manager or contact [support@cekura.ai](mailto:support@cekura.ai) to discuss requirements and configuration options.
...
### Browsing the voice catalog
A personality's voice — and therefore its accent and gender — comes from its `voice_id`. To see what is available with the accent, gender and preview audio for each voice:

- `GET /test_framework/v1/personalities/elevenlabs_voices/?language=en` — ElevenLabs voices. Each carries a `labels` object with the accent, gender, age and use case ElevenLabs publishes for that voice, plus a preview URL. This is the catalog to use when you need a *named* accent.
- `GET /test_framework/v1/personalities/cartesia_voices/?language=en` — Cartesia voices, with language, gender, description and preview. Cartesia publishes no accent label, so accent has to be read from the name and description.

Omit `language` for the full catalog. Pass the id you pick as `voice_id` on the personality, together with the matching `provider` (`11labs` or `cartesia`).

<Note>
The `accent` field on a personality is a **read-only label derived from its voice** — it is not a setting. Speech is synthesised from `voice_id` alone, so setting an accent value would not change how the call sounds. To test a different accent, choose a voice that has it.
</Note>

If none of the catalog voices has the accent, language or timbre you need, the catalog is not a hard limit — Cekura can add voices on request. Reach out at **support@cekura.ai** or via your dedicated Slack support channel with what you need.
...
## Is there a limit to the number of personalities I can create?

There is no limit to the number of personalities you can create in Cekura. Creating dedicated personalities for individual scenarios — varying speaking pace, background noise, or voice characteristics — is actively recommended when scaling test coverage.

A practical approach for high-volume testing is to replicate real call recordings (with PII censored) as structured test cases, each with its own personality, rather than maintaining a single generic caller profile. See [Creating Custom Personalities](/documentation/advanced/creating-custom-personalities) and [Conditional Actions](/documentation/key-concepts/evaluators/conditional-actions) for setup guidance.

### Expected Outcome vs. Metrics

Both are used to evaluate whether a conversation succeeded, but they differ in scope and reusability:

- **Metrics** are reusable across multiple evaluators. You define a metric once (such as a relevancy or politeness check) and attach it to as many scenarios as needed.
- **Expected Outcome** is an inline, per-evaluator assertion — it defines what success looks like for that specific scenario without creating a global reusable metric.

The Expected Outcome appears as a metric in results views because it is evaluated the same way, but it is defined per-evaluator in the API via the `expected_outcome_prompt` field rather than as a standalone reusable metric object.

### Evaluators vs. Agents

- **Evaluator** — the test case. It defines the conversation scenario, the personality of the testing caller, and the expected outcome.
- **Agent** — the connection to your AI voice or chat agent that Cekura tests against.

Every evaluator needs an agent connection to run. The UI enforces this at creation time, but the API is more flexible: you can create an evaluator with `agent: null` as long as a `project` is provided, and the agent can be supplied or overridden at runtime. See the [Create Evaluator API reference](https://docs.cekura.ai/api-reference/test_framework/create-evaluator) for details.
...
Users can lose access to a Cekura workspace when an organisation Admin removes their membership. This most commonly occurs when the organisation has reached its plan's member limit — an Admin may clean up the member list to free up capacity for other users.

If a team member unexpectedly loses access, an Admin can re-invite them at any time: go to **Settings → Members → Invite** and enter their email address. If you need to check your plan's member limit or request an increase, reach out to [support@cekura.ai](mailto:support@cekura.ai).
...
The testing agent ends a call when its evaluator condition contains the `<endcall />` tag in the action text. To prevent the testing agent from ending the call, **remove the `<endcall />` tag from the evaluator's condition instructions**.
...
Without the `<endcall />` tag in any condition, the testing agent will stay on the line and let the main agent terminate the session. See the [Conditional Actions guide](/documentation/key-concepts/evaluators/conditional-actions) for full tag reference.
...
1. Navigate to the **Results** page for your project.
2. Select the checkboxes next to the runs you want to re-evaluate. You can select individual runs or use the header checkbox to select all.
3. Click the **Reevaluate Metrics** button that appears in the toolbar.
4. Choose the specific metrics you want to rerun from the list.
5. Confirm — Cekura will queue the evaluations and update the results.

Note: Re-evaluating metrics consumes credits at the standard rate of 0.2 credits per metric evaluation. See the [Credit Optimization section above](#bestpractices-for-credit-optimization) for details.
...
- You can find detailed instructions here: [Structured Tests Documentation](https://docs.cekura.ai/documentation/key-concepts/evaluators/conditional-actions)
...
- **TTS:** ElevenLabs and Cartesia — both are widely used in production. ElevenLabs leads on voice quality; Cartesia prioritises speed and cost efficiency. See [How do ElevenLabs and Cartesia compare?](/documentation/FAQ#how-do-elevenlabs-and-cartesia-compare-for-tts-voice-quality-and-speed) for the trade-offs.

For architecture guidance and latency targets at each stage, see [Best practices for achieving low end-to-end latency](/documentation/FAQ#what-are-best-practices-for-achieving-low-end-to-end-latency-in-a-cascading-voice-ai-pipeline-stt--llm--tts).

## How do ElevenLabs and Cartesia compare for TTS voice quality and speed?

Both are among the most widely used TTS providers for production telephony, but they optimise for different things:

- **ElevenLabs** is the current quality leader — natural prosody, expressive delivery, and the most human-like voice output available at scale. Choose ElevenLabs when voice quality is the primary concern.
- **Cartesia** prioritises speed and cost efficiency. Time-to-first-audio is consistently lower than ElevenLabs, and pricing is generally more competitive at high call volumes. The trade-off is voice quality that is good but less nuanced than ElevenLabs.

In practice: use ElevenLabs when quality matters most; use Cartesia when minimising latency and operating cost is the priority. Both perform well in telephony — the choice comes down to your product's quality-versus-speed trade-off.

You can use Cekura to compare providers on your own evaluators: run the same scenario against two agent configurations and measure latency and quality metrics side by side.
...
- **TTS:** ElevenLabs and Cartesia both provide low time-to-first-audio and are the most common choices for production voice agents. ElevenLabs leads on voice quality; Cartesia is faster and more cost-efficient. See [ElevenLabs vs Cartesia comparison](/documentation/FAQ#how-do-elevenlabs-and-cartesia-compare-for-tts-voice-quality-and-speed).

### Pipeline orchestration

Frameworks like [Pipecat](https://docs.cekura.ai/documentation/integrations/pipecat/automated) are well-suited to cascading voice pipelines. Pipecat handles the streaming handoff between STT, LLM, and TTS stages and keeps end-to-end latency predictable.
...
Once your pipeline is running, use Cekura's [latency metric](https://docs.cekura.ai/documentation/key-concepts/metrics/pre-defined-metrics) to track per-turn response times across test runs, and the [metadata field](https://docs.cekura.ai/documentation/key-concepts/metrics/basic-advanced-metrics#2-metadata) to tag calls by region or configuration variant so you can compare latency across deployment contexts.
...
- **Around 2s:** Achievable with an optimised cascading pipeline (fast STT + small LLM + low-latency TTS). See [best practices for low end-to-end latency](/documentation/FAQ#what-are-best-practices-for-achieving-low-end-to-end-latency-in-a-cascading-voice-ai-pipeline-stt--llm--tts) for how to reach this range.
...
Use Cekura's [latency metric](https://docs.cekura.ai/documentation/key-concepts/metrics/pre-defined-metrics) to track your agent's per-turn p95 across test runs and production calls, and use [metadata tagging](https://docs.cekura.ai/documentation/key-concepts/metrics/basic-advanced-metrics#2-metadata) to segment by provider, region, or configuration to identify where latency accumulates.

## Why does the latency Cekura reports differ from what my voice platform's dashboard shows?

Voice platforms such as Retell typically report *internal pipeline latency* — the time from when speech recognition (ASR) finishes to when the LLM generates a response, which may exclude TTS rendering and the final audio-delivery step. Cekura measures *end-to-end (E2E) latency*: the full gap from when the Testing Agent finishes speaking to when the Main Agent's audio begins, capturing every stage — STT, LLM inference, TTS rendering, and network round-trip time.

Because Cekura's measurement is more comprehensive, it reads higher than a provider's own dashboard figure. A gap of 800–1000 ms between the two is typical. A Cekura latency reading of **1500–2000 ms** is generally good for a production voice agent. See the [Latency metric reference](https://docs.cekura.ai/documentation/key-concepts/metrics/pre-defined-metrics) for how per-turn values are computed.

## How does Cekura measure latency across different pipeline architectures (cascade, half-cascade, and speech-to-speech)?

Cekura measures latency at the phone audio level rather than inside the provider pipeline, making the measurement consistent across all architectures. The metric captures the gap between the end of the caller's last utterance and the start of the agent's first audible response, detected by applying VAD to the stereo call recording to identify the caller-to-agent turn boundary.

Because the measurement happens at the recorded audio layer, it is pipeline-agnostic: the same method works for full-cascade (STT→LLM→TTS), half-cascade, and speech-to-speech pipelines (e.g. GPT Realtime, Gemini Live) — even when VAD is collapsed into the LLM provider. Every stage that contributes to the delay — provider endpointing, model inference, TTS rendering, and audio delivery — is captured in the resulting figure. Caller-side silence is not counted; only the gaps where the caller has finished speaking and is waiting for the agent to respond are measured.

See the [Latency metric reference](https://docs.cekura.ai/documentation/key-concepts/metrics/pre-defined-metrics) for the full metric definition, percentile breakdown, and interpretation guidance.
...
The best provider often varies by audio characteristics (telephony audio vs. high-quality microphone) and language mix. Use Cekura's [Transcription Accuracy metric](https://docs.cekura.ai/documentation/key-concepts/metrics/pre-defined-metrics) to compare providers objectively on your own audio before committing to one in production.
...
**Evaluators can be duplicated across projects.** Select one or more evaluators in the dashboard, click **Actions** > **Duplicate**, then choose the target project and folder. This copies the evaluators to the new project without modifying the originals, giving you a consistent baseline that you can then customize per project. See [Organizing Projects](/documentation/guides/organizing-projects#duplicating-and-moving-evaluators) for a full walkthrough.

**Agents and metrics do not have a built-in UI duplication tool.** To replicate agent or metric configuration across projects, use the [REST API](https://docs.cekura.ai/api-reference/test_framework/create-agent) or the [MCP server](https://docs.cekura.ai/mcp/overview) to create them programmatically with the same settings.

For environment separation (e.g., staging vs. production), another option is to keep all agents in a single shared project and use separate API keys or agent IDs to route webhook payloads to different backend environments. This avoids the need to keep multiple copies of metrics and evaluators in sync. See [Organizing Projects](/documentation/guides/organizing-projects) for recommended patterns.

## How do I assign an evaluator to a specific folder when creating it via the API?

Include the `folder_path` field in your request body when calling the [Create Evaluator](https://docs.cekura.ai/api-reference/test_framework/create-evaluator) endpoint. The value is a dot-separated path string matching the folder hierarchy in your project — for example, `"Booking.Happy path"` or `"Sales.Inbound"`.

**The folder must exist before you reference it.** Use the [Create Folder](https://docs.cekura.ai/api-reference/test_framework/create-folder) endpoint (`POST /test_framework/v1/scenarios/create_folder/`) to create the folder first. If you omit `folder_path`, the evaluator is automatically placed in the project's **default** folder — every project has one created for it automatically.

## How do I find or create my Cekura API key?

Navigate to **Settings** → **Organization** → **API Key** in the Cekura dashboard (direct link: [dashboard.cekura.ai/dashboard/settings/org/api-key](https://dashboard.cekura.ai/dashboard/settings/org/api-key)). From there you can copy an existing key or create a new one.

**Admin access required.** The Organization section of Settings is only visible to users with Admin role. If you don't see it, ask an admin on your organization to create or share a key with you. For more on role permissions, see [Enterprise Setup](/documentation/advanced/enterprise-setup).

## I already post call data to the observability endpoint from my Pipecat agent — can I also add the Pipecat SDK?

The Pipecat SDK (`observe_pipeline()` / `observe_and_create_task()`) and the direct observability endpoint (`POST /observability/v1/observe/`) are **alternative, mutually exclusive** integration paths — they cannot be combined for the same session.

Each method independently creates its own call record in Cekura. If both run for the same session, Cekura creates two separate records; there is no deduplication or merging. The SDK is designed to replace the direct API for Pipecat-based agents.
...
1. Add `observe_pipeline()` (or `observe_and_create_task()`) to your Pipecat pipeline.
2. Move any metadata from your direct API call to `set_custom_metadata({...})` — see [Custom Metadata](/documentation/integrations/pipecat/tracing#custom-metadata) in the Pipecat Tracing docs. This can be called in a cleanup or post-processing handler that runs before the pipeline closes.
3. Remove the `observability/v1/observe/` call from your post-processing step.


## If I already capture and store audio in my own Pipecat pipeline, will the Cekura SDK record it again?

Yes — when you use `observe_pipeline()` (or `observe_and_create_task()`), the SDK registers its own audio frame processor and performs a parallel recording alongside any existing audio capture you have in your pipeline. This is intentional: the SDK needs its own recording to power Cekura's observability and evaluation features.

The parallel recording is designed to have minimal impact on your pipeline:
...
Your existing audio artifacts are unaffected — the SDK records independently and does not interfere with how you store or process audio elsewhere in your pipeline.

If you want to avoid the parallel recording entirely, use `track_pipeline()` instead — see [Can I disable or selectively control audio recording in the Pipecat SDK?](#can-i-disable-or-selectively-control-audio-recording-in-the-pipecat-sdk) for the full set of recording control options.

## Can I disable or selectively control audio recording in the Pipecat SDK?

The Pipecat SDK offers several recording control options depending on your requirements:
...
## Do I need a separate Pipecat deployment to run chat tests?

No. Cekura's [Pipecat Chat](/documentation/integrations/pipecat/chat-testing) uses the same Pipecat Cloud deployment that serves voice. The bot must use the Daily transport and have RTVI enabled (`PipelineTask(..., enable_rtvi=True)`). Cekura starts one session per run, joins its Daily room, and sends every turn as an RTVI `send-text` message with `audio_response=false`, so no TTS runs. If RTVI is disabled, the run fails before any scenario content is sent with `pipecat-agent-readiness-timeout`.

## Why does my Pipecat chat transcript show `function_call` with no name or arguments?

Pipecat's RTVI observer hides function names, arguments, and results by default. Upgrade to a Cekura SDK version with text-session support and let `track_and_create_task` build the task, or pass `rtvi_observer_params=RTVIObserverParams(function_call_report_level={"*": RTVIFunctionCallReportLevel.FULL})` to `PipelineTask` yourself. See [Pipecat Chat Testing](/documentation/integrations/pipecat/chat-testing#troubleshooting-and-readiness-errors).
...
If your account runs out of credits, Cekura stops ingesting and processing new calls. The **auto-fetch** feature does not backfill calls that were missed during a credit gap — only calls that arrive after credits are restored are picked up going forward.

To analyze calls from the missed window, you must manually resend them using the [Send Calls API](https://docs.cekura.ai/api-reference/observability/send-calls):

1. Identify the timeframe of the gap (start and end timestamps).
2. Export the call recordings or transcripts from your voice provider or internal systems for that window.
3. POST them to the Send Calls endpoint to ingest them into Cekura for evaluation.
...
Cekura does not provide phone numbers for all regions. If you need a testing number in a region Cekura doesn't cover natively, you can bring your own number from a third-party telephony provider such as [Plivo](/documentation/key-concepts/phone-numbers/plivo) or [Twilio](/documentation/key-concepts/phone-numbers/twilio) and import it into Cekura.

Once imported, the number works as a normal Cekura testing number — your system calls it and Cekura's testing agent answers. No additional integration with the telephony provider is required beyond the import step.

See the [Plivo import guide](/documentation/key-concepts/phone-numbers/plivo) for step-by-step instructions.


## How do I submit a bug report or feature request?

Email [support@cekura.ai](mailto:support@cekura.ai) — this routes directly to the engineering team. For bugs, include a brief description of the issue, any relevant run IDs, evaluator URLs, or dashboard links, and your expected vs. actual behavior. For feature requests, a short description of the use case is sufficient. The team will triage and follow up in your support channel.
...
1. Cekura computes a rolling baseline (e.g., 90% success rate on your adherence metric).
2. You configure a sensitivity margin based on standard deviation.
3. The alert fires only when the metric falls outside that margin — for example, dropping to 84% when the baseline is 90% and the margin is 5%.

This approach surfaces real regressions while ignoring normal LLM variance.

### How to set up an alert

1. Navigate to the **Alerts** section in the Cekura dashboard.
2. Click **Add New Alert** in the top right corner.
3. Select the agent and metric you want to monitor.
4. Configure the Significant Change Alert sensitivity to match your needs.

For more on how metrics are structured, see the [Metrics documentation](https://docs.cekura.ai/documentation/key-concepts/metrics/basic-advanced-metrics).


## Is there a way to pull observability call data across multiple projects in a single API call?

No — the [List Calls API](https://docs.cekura.ai/api-reference/observability/list-calls) requires you to scope results to a single project or agent. Cross-project querying in a single call is not supported; the database is indexed at the project level, and cross-project joins are prohibitively expensive.
...
## Are agent IDs globally unique, or only unique within a region?

Agent IDs are unique within a Cekura region (for example, all agents in the US region have distinct IDs among themselves, and all agents in the EU region have distinct IDs among themselves). However, the same integer agent ID can appear in both regions — there is no guarantee of uniqueness across regions.

For most API operations, this does not matter: when you scope a query using `agent_id`, Cekura automatically resolves it within the region your API key belongs to. If your platform operates across multiple Cekura regions, use `project_id` or the full region-qualified context to avoid ambiguity.


## Does the internal Cekura call ID change when call data is migrated between projects?

Yes. Cekura's internal integer call ID — the `callId=` query parameter in dashboard URLs (e.g. `https://dashboard.cekura.ai/{project_id}/calls?callId=7293214`) — is not guaranteed to survive a project migration. When the Cekura team moves historical call data to a different project, the internal ID can change.

The external `call_id` you supply at submission time (via the [Send Calls API](https://docs.cekura.ai/api-reference/observability/send-calls#body-call-id)) is your own identifier and always stays the same — Cekura never modifies it.

**Recommended approach for building stable dashboard links:**

1. Store your external `call_id` (the string you sent to Cekura) as the long-term identifier for each conversation.
2. When you need to construct a dashboard link, query the [List Calls API](https://docs.cekura.ai/api-reference/observability/list-calls) filtering by `call_id` to retrieve the current internal `id`.
3. Use that internal `id` as the `callId=` parameter in the URL.

Note: Cekura does not offer automatic URL redirection from an external `call_id` to an internal ID, because external call IDs are not guaranteed unique across all organisations and regions.
...
The trade-off is a slower initial setup and more back-and-forth to get mock data and traces configured correctly. You can also grant access for the initial integration period and revoke it afterward.

If you have questions about the specific access scope required for your engagement, ask in your support channel and a team member will clarify what applies to your setup.


## What does "Create Evaluator from Call" actually do, and can I use it to replicate a production failure as a test case?

When you use the "Create Evaluator from Call" (or "Create Scenarios from Call") feature on a production call log, Cekura reads the transcript of that call and generates a new structured evaluator that recreates the same conversation flow, intent, and expected outcomes. The resulting evaluator can then be run repeatedly as a regression test — this is the recommended approach for turning a specific production failure into a repeatable test scenario. For finer control over branching conditions, turn sequences, or IVR flows, build the evaluator directly using [Conditional Actions](/documentation/key-concepts/evaluators/conditional-actions).

**Important limitation for STT testing:** When the evaluator is run, the Testing Agent replays the transcript text using synthetic TTS audio (ElevenLabs), *not* the original production audio recording. This means the evaluator cannot be used to test a different STT provider against the original audio — the STT input will be re-synthesised speech, not the real recording.

If your goal is to benchmark STT providers against real production audio, the correct approach is:

1. Pull the call log for each target call via the Cekura API to get the per-turn `start_time` and `end_time` values from the transcript JSON (see [Transcript Format](https://docs.cekura.ai/documentation/advanced/transcript-format)).
2. Use those timestamps to slice the original audio recording into per-turn segments (e.g. with `ffmpeg`).
3. Send those audio segments to each STT provider you want to compare and compute WER against a ground-truth transcription.
...
The Testing Agent generates its own synthetic speech for every run — it does not accept externally provided audio files as test inputs. To replicate a specific failure scenario or conversation flow, use [Structured Tests](/documentation/key-concepts/evaluators/conditional-actions): write a rule-based evaluator whose conditions follow the conversation path you want to reproduce, and the Testing Agent will simulate that sequence with its own synthesised voice.

If the failure you want to reproduce came from a production call already captured in Cekura, the *Create Evaluator from Call* feature converts that call's transcript into an evaluator, preserving the turn-by-turn dialogue as conditions. The Testing Agent replays the conversation using synthetic audio rather than the original recording.

For STT benchmarking against original audio recordings, see [What does "Create Evaluator from Call" actually do?](#what-does-create-evaluator-from-call-actually-do-and-can-i-use-it-to-replicate-a-production-failure-as-a-test-case).
...
1. On the **Results** page, use the **Filter by evaluator name** option to scope the view to the evaluator you care about.
2. Select the filtered runs you want to review, then use the **Create Result** action (sometimes labelled "Group into Result" or "Club Runs") to merge them into a single combined result. This brings all selected runs into one result view where you can navigate between individual conversation transcripts.

This workflow lets you review transcripts across runs to spot when a regression was introduced, compare responses across LLM versions, or audit how the agent has behaved over time on that specific test case. For more on organising and filtering results, see [Organising Projects](https://docs.cekura.ai/documentation/guides/organizing-projects).


## Is there a way to add notes or annotations to individual test call runs?

Currently, Cekura does not have a dedicated free-form note-taking feature for individual test call runs. The closest option is **Metric Lab annotations**: within a Labs review session you can click the feedback/notes icon on any evaluation row and write a note explaining why a call passed or failed a given metric. These notes are primarily designed to help refine the metric definition via Auto Improve, but they also serve as a per-call qualitative record tied to that metric. See the [Metric Lab](/documentation/guides/metric-lab) guide for the full annotation workflow.

If you need observations that are not tied to a specific metric, the typical approach is to maintain an external log alongside the Cekura run IDs — referencing the run ID later links your notes back to the dashboard record.

If you find yourself repeatedly noting the same category of failure, consider creating a dedicated metric for it. That automates the tracking and surfaces the pattern directly in your results without manual note-keeping.

If you are using **Observability** to monitor production calls (rather than running evaluations in the test framework), note that individual call logs in that view support a per-metric feedback mechanism: you can vote thumbs-up or thumbs-down on a specific metric evaluation result and add a free-text note explaining why the evaluation was correct or incorrect. This feedback is accessible from the call log detail view and from Slack alert notifications (via the 👎 button next to **Go to call**). Like Metric Lab annotations, this feedback is tied to specific metric evaluations rather than being a free-form call-level note. See [Creating a Good Metric](/documentation/guides/creating-good-metric) for the annotation workflow.


## Why is a count metric failing even when the measured value is zero?

This happens when the **Rubric** threshold for that metric is configured with the wrong comparison direction. In Cekura, a metric measures a value — for example, the number of unnecessary repetitions — and the Rubric determines whether that value is a pass or fail based on an operator and threshold you configure.

For **"lower is better" count metrics** (such as Unnecessary Repetition Count), a measured value of 0 is the best possible result. However, if the rubric rule uses "greater than or equal to" with a threshold like 3, the evaluation fails whenever the count is *below* 3 — including 0 — because the rule requires a minimum count to pass, which is the reverse of what you want.
...
The same principle applies to any metric whose interpretation says "Lower is better" (see the *Interpretation* note on each entry in [Pre-defined Metrics](/documentation/key-concepts/metrics/pre-defined-metrics)): use **≤** for those, and **≥** only for metrics where higher values indicate better performance (such as boolean pass/fail metrics, which use ≥ 5).


## How does Cekura handle decision-level reasoning and explainability, and what data do I need to provide for audit purposes?

Cekura takes model reasoning into account when explaining why specific outcomes occurred and why evaluations passed or failed. Every LLM Judge and Python metric evaluation produces an `explanation` field that describes why the rubric was or wasn't met.

For workflows where you need a complete audit trail — including the internal logic behind each decision, not just whether the outcome was correct — you need to provide the reasoning data yourself, because Cekura cannot reconstruct internal agent decision logic that isn't present in the transcript or the data you send.
...
- **Alternative — OpenTelemetry tracing.** If your agent's decisions are expressed as tool calls, you can instead emit reasoning via [OTel tracing](https://docs.cekura.ai/documentation/guides/observability/tracing). `tool_call` spans can carry `function.input` / `function.output` attributes, so decision rationale attached to tool calls flows into Cekura and appears linked to the call record.
...
How you update depends on how you installed the skills.

**Claude Code marketplace**: Run `/upgrade-skills` in any Claude Code session. To get updates automatically at every session startup, open the plugin manager (`/plugin`), go to the **Marketplaces** tab, select **cekura-skills**, and enable auto-update.

**Other agents (Cursor, Codex, Windsurf, etc.)**: Auto-update is not available — unlike the Claude Code marketplace, there is no background refresh option for these agents. Run the update command manually whenever you want the latest skills:
...
Yes. Write a [Python metric](/documentation/key-concepts/metrics/python-metric) that reads the results of other Cekura metrics already evaluated on the same call, then applies your own comparison logic.

Inside any Python metric, every other metric that ran on that call is available by name in the `data` dictionary:
...
You can supply the manual score via a [Test Profile](/documentation/key-concepts/evaluators/test-profiles) so it varies per test case without editing the metric code. For the full `data` dictionary reference, see [Metric Results Access](/documentation/key-concepts/metrics/python-metric#metric-results-access).
