...
| Method | Endpoint |
|--------|----------|
| `POST` | `https://new-prod.cekura.ai/test_framework/v1/results-external/:result-id/rerun/` |
...
```bash Curl
curl -X POST https://new-prod.cekura.ai/test_framework/v1/results-external/:your-result-id/rerun/ \
...
def rerun_result(api_key, result_id):
    url = f'https://new-prod.cekura.ai/test_framework/v1/results-external/{result_id}/rerun/'
    headers = {
...
async function rerunResult(apiKey, resultId) {
  const url = `https://new-prod.cekura.ai/test_framework/v1/results-external/${resultId}/rerun/`;
