---
title: Calls Embedding (Deprecated)
sidebarTitle: "Calls Embedding"
icon: "phone-volume"
iconType: "regular"
description: Learn how to embed Cekura agent calls in your application using our embed widget. For token management details, see our [Token Generation](/embed/generate-token) and [Token Refresh](/embed/refreshing-expired-token) guides.
---
...
All embedding features are being deprecated and will be removed shortly.\
...
If you have existing integrations using Calls Embedding, please plan to migrate to our APIs before the removal date.
</Warning>
...
```
https://dashboard.cekura.ai/embed/{agentId}/calls?token={token}&theme={theme}
```
...
export default {
  ...
  computed: {
    embedUrl() {
      return `https://dashboard.cekura.ai/embed/${this.agentId}/calls?token=${this.token}&theme=${this.theme}`;
    }
  },
  methods: {
...
export class VoceraCallsEmbedComponent implements OnInit, OnDestroy {
  ...
  get embedUrl(): SafeUrl {
    const url = `https://dashboard.cekura.ai/embed/${this.agentId}/calls?token=${this.token}&theme=${this.theme}`;
...
iframe.loading = 'lazy';
iframe.title = 'Cekura Calls Embed';
...
this.iframe.src = `https://dashboard.cekura.ai/embed/${this.agentId}/calls?token=${this.token}&theme=${this.theme}`;
}
}
...
// Usage examples
const callsEmbed = new VoceraCallsEmbed('vocera-calls-embed', 'YOUR_AGENT_ID', 'dark');
const lightCallsEmbed = new VoceraCallsEmbed('vocera-calls-embed', 'YOUR_AGENT_ID', 'light');
