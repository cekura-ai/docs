...
<Note>
**Dynamic variables and inbound telephony**: When testing via inbound telephony (Cekura dials your phone number and Retell handles the call), dynamic variable values from test profiles and agent description placeholders (`{{variableName}}`) are **not** passed to your Retell agent — only standard Twilio call metadata is available. To test with dynamic variables, use **WebRTC testing** or **Auto-Outbound Call** (where calls are initiated through Retell's API directly). See the [WebRTC tab](#) or [Auto-Outbound Call guide](/documentation/guides/testing-agents/outbound-auto-call) for details.
</Note>
...
<Tip>
WebRTC tests skip phone infrastructure entirely—no phone number needed, no telephony costs, and tests start immediately. WebRTC tests also fully support dynamic variables — values from test profiles and agent description placeholders (`{{variableName}}`) are passed to your Retell agent at call time.
</Tip>
