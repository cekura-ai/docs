...
The LLM model and parameters (such as temperature) used by UI-based LLM Judge metrics are managed by Cekura and are not user-configurable. If you need full control over model selection or inference parameters, use a [Python Metric](/documentation/key-concepts/metrics/python-metric) instead — Python metrics run free of charge (no credit consumption), and can call any external inference provider with your own API keys.
