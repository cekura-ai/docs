...
Already have an agent on a supported cloud provider? Import it directly instead of setting everything up from scratch. Cekura pulls your system prompt, language, first-message settings, tool calls (with mock data), and dynamic variables automatically.

**Supported providers:** VAPI, Retell, ElevenLabs, and Synthflow.

<Steps>
  <Step title="Start the import">
    Go to **Agents** and click **Import from Provider** (or select the import option when creating a new agent).
  </Step>
  <Step title="Select your provider">
    Choose the cloud provider where your agent is configured.
  </Step>
  <Step title="Enter credentials">
    Provide your provider API key and the assistant ID of the agent you want to import.
  </Step>
  <Step title="Wait for import to complete">
    Cekura connects to your provider and imports your agent's configuration. A progress indicator shows each stage: connecting to provider, importing system prompt, importing settings, fetching tools, extracting dynamic variables, and finalizing.
  </Step>
</Steps>

### What Gets Imported

| Setting | VAPI | Retell | ElevenLabs | Synthflow |
|---------|------|--------|------------|-----------|
| System prompt | ✓ | ✓* | ✓ | ✓ |
| Language | ✓ | ✓ | ✓ | ✓ |
| First message / who speaks first | ✓ | ✓ | ✓ | ✓ |
| Tool calls with mock data | ✓ | ✓ | ✓ | — |
| Dynamic variables | ✓ | ✓ | ✓ | ✓ |

<Note>*Retell agents using a custom-llm engine type store their prompt externally, so the prompt cannot be imported automatically.</Note>

After import, Cekura enables **prompt auto-sync** so future prompt changes on the provider are reflected automatically. Production call auto-import stays off by default.
...
<AccordionGroup>
  ...
  <Accordion title="WebRTC">
    **WebRTC** enables real-time browser-based voice testing without needing a phone number or external device.
    ...
    <Note>WebRTC calls are only supported by [LiveKit](/documentation/integrations/livekit/testing), [Pipecat](/documentation/integrations/pipecat-integration), [VAPI](/documentation/integrations/vapi/testing), and [Retell](/documentation/integrations/retell/testing).</Note>
  </Accordion>
</AccordionGroup>
