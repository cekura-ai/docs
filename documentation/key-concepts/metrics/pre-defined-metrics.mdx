...
<AccordionGroup>
  ...
  <Accordion title="Transcription Accuracy">
    ...
    **Minor variations.** Every remaining difference — articles, pronouns, fillers, prepositions and other function words — is a *minor variation*. These don't count toward the weighted error count, but each one applies a small **absolute 0.1% penalty** to the score. For calls longer than ten Testing Agent turns, minor variations are normalized to the same ten-turn baseline as significant errors. Significant errors still dominate; the minor penalty only lets you tell a flawless transcript apart from one with a few small differences by looking at the score alone. This applies to both Runs and Call Logs.
...
<AccordionGroup>
  ...
  <Accordion title="Latency (in ms)">
    ...
    **How latency is measured.** Cekura measures latency directly from the recorded phone audio — the gap between the end of the caller's speech and the start of the agent's first audible response, detected by applying VAD to the stereo recording. This approach is pipeline-agnostic: the same measurement applies to full-cascade (STT→LLM→TTS), half-cascade, and speech-to-speech architectures (including providers such as GPT Realtime and Gemini Live where VAD is collapsed into the model). Because the measurement happens at the audio channel level rather than inside the provider pipeline, it captures every contributing stage — provider endpointing, model inference, TTS rendering, and audio delivery. Only caller-to-agent handoffs are measured; caller-side silence (pauses where the caller has not yet started speaking or has already stopped) is not counted as agent latency.
