...
### How do I cleanly reinstall the Cekura plugin?

If `/upgrade-skills` puts you in a stuck state — usually after a structural change to the marketplace — run all four commands in order:
...
Step 3 is the critical one. It forces Claude Code to re-read the marketplace.json from disk; without it, step 4 may try to install against stale parsed metadata and fail with a "Source path does not exist" error.

After step 4, `claude plugin list` should show one `cekura@cekura-skills` entry, and all 14 slash commands resolve under `cekura:*`.

### `/plugin install` errors with "Source path does not exist: …/plugins/cekura" — what's wrong?

Claude Code is using cached marketplace metadata that points at an older repo layout. Re-run `/plugin marketplace update cekura-skills`, then retry the install. If the error persists, fully restart Claude Code (close and reopen the session), then redo `/plugin marketplace add … → update → install`.

### Why didn't `/plugin marketplace add` install the plugin automatically?

Marketplaces and plugins are separate concerns in Claude Code. `marketplace add` registers where to find plugins; `plugin install <name>@<marketplace>` installs a specific plugin from a registered marketplace. Adding a marketplace doesn't auto-install everything in it.

### I have stale cache dirs from older plugin versions — should I clean them up?

Yes, safe to remove. After upgrading from v0.4.x (which shipped 3 separate plugins) to v0.5.0+ (which merged them into a single `cekura` plugin), you may see leftover `cekura-evals/` and `cekura-metrics/` dirs under `~/.claude/plugins/cache/cekura-skills/`. They're harmless but take up a few MB:

```bash
rm -rf ~/.claude/plugins/cache/cekura-skills/cekura-evals
rm -rf ~/.claude/plugins/cache/cekura-skills/cekura-metrics
```

You can also delete older versioned dirs of the active plugin (e.g. `cekura/0.2.0/`, `cekura/0.3.0/`) — Claude Code only uses the version recorded in `~/.claude/plugins/installed_plugins.json`.

### Where does the Cekura plugin live on disk?

Two places:
- **Marketplace source** (the git clone): `~/.claude/plugins/marketplaces/cekura-skills/`
- **Active plugin cache** (what Claude Code serves at runtime): `~/.claude/plugins/cache/cekura-skills/cekura/<version>/cekura/` — contains `skills/`, `commands/`, `agents/`, `hooks/`.

`git pull` on the source dir alone doesn't refresh the cache — you have to run `/plugin marketplace update cekura-skills` (or reinstall) to rebuild it.
