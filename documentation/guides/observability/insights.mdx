...
1. Pulls all failing calls for that (project, metric) from the last 24 hours.
...
- A "No failing calls in the last 24 hours" message when the metric had no failures during that period
...
If a metric has no failing calls in the last 24 hours, the card displays a clean "No failing calls in the last 24 hours" message.
...
Poll the returned row's `status` field until it resolves (`succeeded`, `failed`, or `skipped_no_failures`):
