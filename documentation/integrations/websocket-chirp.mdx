...
Run tests directly from the dashboard against your WebSocket voice endpoint.

<Steps>
<Step title="Select Custom provider">
Open your agent settings and select **Custom** in the provider grid. This is the provider type for raw-PCM WebSocket voice agents.
</Step>

<Step title="Configure WebSocket voice connection">
In the **Connection** section, enable the **WebSocket (CHIRP)** toggle and fill in your endpoint details:
...
<Step title="Run tests from the dashboard">
  ...
  2. With WebSocket voice configured, you will see the **Websocket** option under **VOICE** options in the **Configure Run** dialog.
...
Use the API to trigger WebSocket voice runs from your own code or CI.

## Prerequisites

Configure WebSocket voice on your agent first (same as the **Dashboard** tab):
...
When creating or updating an agent via the API, configure WebSocket voice in the `telephony` block:
...
For the V1 API, use the flat fields `websocket_voice_url` and `websocket_auth` instead.
...
```json
{
  "websocket_url": "Agent does not have a websocket endpoint configured. Please configure telephony.websocket_url in the agent settings."
}
```
