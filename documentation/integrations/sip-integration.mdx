...
<Note>
  If your SIP endpoint is behind a firewall or SIP trunk uses IP-based access control, you will need to whitelist **Twilio's SIP signaling and media IPs** — Cekura routes SIP calls through Twilio's infrastructure, so those are the source IPs your firewall will see. See [Twilio's SIP trunking IP addresses](https://www.twilio.com/docs/sip-trunking/ip-addresses) for the current list. For general Cekura outbound IPs (webhooks, API callbacks), see [IP Whitelisting](/documentation/advanced/ip-whitelisting).
</Note>

</Step>

<Step title="Configure custom headers (optional)">
  To inject custom headers into every SIP INVITE for a test run — for example, a routing token or tenant ID your SIP proxy requires — configure a Test Profile in the evaluator:

  1. Open the evaluator (test case) you want to run and scroll to the **Test Profiles** section.
  2. Add a new test profile or edit an existing one. Under **Main Agent Variables**, add your header key-value pairs (e.g., `X-Tenant-Id: <value>`, `X-Auth-Token: <value>`).
  3. Keys **must** start with `X-` to be forwarded as SIP headers. Keys that do not start with `X-` are not sent over the wire.
  4. Attach the test profile to the evaluator. The headers will be included in the SIP INVITE alongside the default `X-Run-Id`, `X-Scenario-Id`, and `X-Result-Id` headers.

  <Note>
  Custom headers are configured at the evaluator level via Test Profiles — not in agent settings. See [Test Profiles](/documentation/key-concepts/evaluators/test-profile) for full details.
  </Note>
</Step>
