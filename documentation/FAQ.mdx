...
## What does the "Triage on Fail" toggle do for metrics?

The **Triage on Fail** toggle enables automatic root-cause analysis when a metric fails. When enabled, if a call or run produces a `passed=False` result for that metric, Cekura runs an LLM-based triage that analyzes the failure and generates a structured explanation.

### How to enable it

1. Navigate to the **Metrics** section in your dashboard.
2. Locate the metric you want to enable triage for.
3. Toggle on the **Triage on Fail** option in the metric row.

### What the triage provides

When a metric fails with triage enabled, you'll see a triage result in the evaluation details containing:

- **Concise reason**: A customer-facing explanation of why the metric failed, citing the specific turn, timestamp, or condition.
- **Responsible side**: Classification of what caused the failure — `agent_under_test`, `simulation`, `scenario_design`, `metric_implementation`, `external_provider`, or `data_missing`.
- **Confidence**: How confident the triage is in its assessment (`high`, `medium`, or `low`).
- **What would change verdict**: A short sentence describing what single change would make the metric pass on re-evaluation.

### Viewing triage results

Triage results appear in the expanded metric card when viewing call log or run details. The concise reason and responsible-side badge are always visible; click to expand for the full reasoning.

### Cost and defaults

Triage is **off by default** and opt-in per metric. Enabling it incurs additional LLM costs only when a metric actually fails — there is no cost if the metric passes.
