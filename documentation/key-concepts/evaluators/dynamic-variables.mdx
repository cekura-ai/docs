...
<Note>
**Retell + inbound telephony**: Dynamic variable values are **not** passed to Retell agents when running inbound telephony tests (where Cekura dials your phone number). Use **WebRTC testing** or **outbound calls** to propagate dynamic variables with Retell. See [Retell Testing](/documentation/integrations/retell/testing) for details.
</Note>
