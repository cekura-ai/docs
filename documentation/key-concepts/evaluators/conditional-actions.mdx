...
<Note>
The first message plays **once** and is never retried. If the main agent talks over it, for example with its own greeting during a leading `<silence>`, the rest of the action is dropped and the testing agent's next reply is not scripted. To let the main agent greet first and then say an exact line, leave `id: 0` empty and put the line on a `standard` condition with `id: 1` that matches the greeting:
