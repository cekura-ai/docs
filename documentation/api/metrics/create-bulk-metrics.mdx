...
| Method | Endpoint |
|--------|----------|
| `POST` | `https://new-prod.vocera.ai/test_framework/v1/metrics-external/bulk_create/` |
...
async function createBulkMetrics(apiKey, metrics) {
  const url = 'https://new-prod.vocera.ai/test_framework/v1/metrics-external/bulk_create/';
