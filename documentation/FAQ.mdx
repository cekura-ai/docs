...
## How can I export simulation test results or filter runs by outcome on the Results page?

The **Results** page (also called the Simulations page) has its own export button for downloading test run data. If the button appears stuck or unresponsive, this is a platform-side issue — contact support so the team can investigate.

To work with a specific subset of runs before exporting:

1. Use **Filter by evaluator name** to scope the view to the evaluator you care about.
2. Select the runs you want, then click **Create Result** (also labelled "Group into Result" or "Club Runs") to merge them into a single combined result. The combined result shows per-run pass/fail status and transcripts side-by-side.

There is no native single-click filter for "only runs where every metric passed." To isolate 100% passing runs, filter by evaluator, review the per-run outcome column in the Results view to identify the passing runs, select them, and group them with *Create Result*.

For programmatic access — filtering by metric score, exporting bulk run data, or automating result retrieval — use the [Results API](https://docs.cekura.ai/api-reference/testing/get-result) and [Runs API](https://docs.cekura.ai/api-reference/testing/list-runs).
...
- **Results Page:** This page shows a per-metric breakdown across all runs in the result. Each metric displays its pass/fail totals, and you can click any metric row to see the specific runs where that metric failed, along with **action items** to improve your agent. This is the primary view for understanding which metrics are failing and drilling straight to the failing calls.
...
## Does Cekura have a CLI or Python SDK?

Yes — `pip install cekura` ships both the **CLI** and the **Python SDK** in a single package. They share the same authentication and API surface.

- **CLI** (`cekura` command): for one-off commands, shell scripts, CI pipelines, and quickly inspecting your workspace from the terminal. Supports tab-complete, table output, and `--json` mode for piping.
- **Python SDK**: for application code, custom automation, batch jobs, and integrating Cekura into existing Python workflows. Both sync (`Cekura`) and async (`AsyncCekura`) clients are available.

Authentication: run `cekura auth login` for a one-time browser sign-in, or set `CEKURA_API_KEY` for API-key-based auth (recommended for CI).

For full documentation including installation, authentication, available commands, and code examples, see the [CLI & SDK guide](/cli-sdk/overview).
