...
## How can I increase the project limit for my account after receiving a 'Max projects limit reached' error?

The `400: ["Max projects limit reached"]` error indicates that your account has hit the maximum number of projects allowed under your current Cekura subscription plan. Projects are used to isolate different environments (such as Dev, Staging, and Production) or different AI agents to ensure that testing data and production monitoring remain separate.

### Steps to Resolve the Limit Error

1. **Audit Existing Projects**: Review your current project list in the Cekura dashboard. If there are any old, temporary, or unused projects, you can delete them to free up a slot for your new environment.
2. **Upgrade Your Subscription**: If you need to maintain all existing projects and require more capacity, you may need to move to a higher subscription tier. You can view plan details and upgrade options within your account billing settings.
3. **Contact Support for Custom Limits**: For Enterprise users or teams with specific architectural needs requiring many isolated projects, please contact our support team or your account manager. We can help adjust your limits to accommodate your dev/testing workflow.

### Why Isolate Projects?
It is a recommended best practice to use separate projects for development and production. This ensures that:
- **Metric Accuracy**: Test conversations and evaluations do not pollute your production observability data.
- **Access Control**: You can manage permissions separately for developers and production support staff.
- **Configuration Safety**: Changes to evaluators or metrics in a dev project won't accidentally impact your production monitoring setup.
