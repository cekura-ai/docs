...
- **Max 2 MB per file**
- **Supported formats:** `.txt`, `.md`, `.csv`, `.json`, `.yaml`, `.yml`, `.xml`, `.xlsx`, `.xls`

The agent reads file contents and uses them directly — for example,
generating scenarios from a spec document or creating metrics from a
checklist. Excel files (`.xlsx`, `.xls`) are automatically converted to CSV.
Binary formats like PDFs, images, and audio aren't supported yet.
