...
When trace data is available, the run detail view displays three tabs:

- **Transcript**: The conversation transcript with timestamps. With traces available, playback controls let you jump directly to the corresponding trace span.
- **Traces**: A flamegraph visualization of all trace spans (STT, LLM, TTS, tool calls, etc.). Click any span to seek the audio player to that moment. The playhead follows audio playback in real time.
- **Logs**: A chronological list of trace spans showing timestamps, span types, durations, and provider/model information.
