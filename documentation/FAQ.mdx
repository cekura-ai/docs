...
### Creating a Voice Variant Not in the Predefined Library

If the predefined library does not include a personality that matches the exact voice type you need — for example, a "Normal"-style caller with a female voice — you can [create a custom personality](/documentation/advanced/creating-custom-personalities) and select a voice ID from your voice provider that has the desired characteristic (gender, accent, or any audio quality your provider supports). This gives you full control over both the behavioral settings and the voice.

If you want to preserve the exact behavioral profile of a predefined personality (speaking pace, interruption behavior, tone) and only change the voice, create a new custom personality that replicates those behavioral settings and selects a different voice ID. For example, to get a female-voiced equivalent of *Normal Male*, create a custom personality with the same Personality Prompt, Temperature, and Model configuration as Normal Male, and choose a female voice ID from your voice provider. This approach is more targeted than building a personality from scratch when the behavioral settings are already right and only the voice needs to change.
...
## Why does the "Please wait while we check your organization status" screen appear and not clear?

This screen appears during sign-up while Cekura provisions your organization. Under normal circumstances it clears automatically within about 30 seconds.

If the screen has been spinning for longer than a minute, the provisioning step may need attention on the Cekura side. Reach out via [support@cekura.ai](mailto:support@cekura.ai) or your shared Slack channel and the team will investigate.
...
## Why does a team member appear as already a member when I try to add them to a new project?

Users with the **Admin** role have access to all projects by default — they do not need to be manually added to individual projects. If you attempt to invite an Admin to a project, the system recognises they already have organisation-wide access and may indicate they are already a member.

**Member**-role users, by contrast, only have access to projects they are explicitly assigned to. For a full breakdown of roles and their project-access scope, see [Enterprise Setup](/documentation/advanced/enterprise-setup).
