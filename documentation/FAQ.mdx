...
## What is the v2 Agent API and how does it differ from v1?

The v2 Agent API (`/v2/aiagents`) provides a cleaner, more structured way to create and update AI agents. The v1 API remains fully functional and unchanged, but v2 is the recommended API for new integrations.

### Key differences

**Nested provider block:** v2 replaces the 26 flat provider credential fields from v1 with a single nested `provider` object containing `type`, `agent_id`, and `credentials`.

**Nested telephony block:** Phone number and SIP configuration are grouped under a `telephony` object.

**Cleaner field names:** Several fields have been renamed for clarity:

| v1 field | v2 field |
|---|---|
| `agent_name` | `name` |
| `contact_number` | `telephony.phone_number` |
| `sip_endpoint` | `telephony.sip_uri` |
| `outbound_auto_call` | `provider.auto_dial_outbound` |
| `auto_fetch_calls_enabled` | `provider.auto_import_calls` |
| `auto_sync_prompt_enabled` | `provider.auto_sync_prompt` |
| `agent_gives_first_message` | `agent_speaks_first` |
| `assistant_provider` + credential fields | `provider.type` + `provider.credentials` |
| `websocket_url` / `websocket_headers` | `provider.chat_agent_details.config.url` / `.headers` |

**Default values:** When creating an agent, `provider.auto_dial_outbound` defaults to `false` if not specified. This matches v1 behavior — auto-dial is opt-in.
...
The v2 API includes three nested collections that can be managed inline with agent create/update requests:

| Collection | Description |
|---|---|
| `mock_tools` | Mock tool definitions with input/output mappings for testing |
| `dynamic_variables` | Variable definitions injected into agent context at runtime |
| `knowledge_base_files` | Files uploaded to the agent's knowledge base |

**Full-replace semantics:** When you include a nested collection array in a PATCH or POST request, it becomes the complete desired state for that collection:

- Items with an `id` (or matching `name` for mock tools) update existing records
- Items without an `id` create new records
- **Existing items omitted from the list are deleted**
- Omitting the field entirely leaves the collection unchanged

<Warning>
Including a collection array replaces the entire collection. Any existing items not listed in the array are permanently deleted.
</Warning>

**Example: updating mock tools**

```json
PATCH /v2/aiagents/{id}/
{
  "mock_tools": [
    {"id": 123, "description": "Updated description", "mock_data": [{"input": {"q": 1}, "output": {"a": 2}}]},
    {"name": "new_tool", "description": "Brand new tool", "mock_data": [{"input": {}, "output": {"ok": true}}]}
  ]
}
```

This request updates tool ID 123, creates `new_tool`, and deletes any other mock tools previously on the agent.

**Example: adding a knowledge base file**
...
PATCH /v2/aiagents/{id}/
{
  "knowledge_base_files": [
    {"id": 456},
    {"file_name": "notes.txt", "content_base64": "aGVsbG8gd29ybGQ="}
  ]
}
```

This keeps file ID 456, adds `notes.txt`, and deletes any other files.

### Response fields

The v2 agent response includes additional fields for mock tool state:

| Field | Type | Description |
|---|---|---|
| `mock_tools_enabled` | boolean | Whether mock-tool mode is currently active on the provider |
| `mock_tools_provider` | string or null | Provider the mock tools were fetched from (`vapi`, `retell`, `elevenlabs`, `custom`). Null until auto-fetch has been run. |

### Mock lifecycle endpoints

The v2 API provides endpoints for managing the mock tool lifecycle directly on the agent resource:

| Endpoint | Method | Description |
|---|---|---|
| `/v2/aiagents/{id}/auto-fetch-tools/` | POST | Discover tools from the provider and generate mock data |
| `/v2/aiagents/{id}/auto-fetch-tools-progress/` | GET | Poll auto-fetch task status |
| `/v2/aiagents/{id}/enable-mock-tools/` | POST | Activate mock mode on the provider |
| `/v2/aiagents/{id}/enable-mock-tools-progress/` | GET | Poll enable task status |
| `/v2/aiagents/{id}/disable-mock-tools/` | POST | Restore original (non-mock) tool configuration |
| `/v2/aiagents/{id}/disable-mock-tools-progress/` | GET | Poll disable task status |

**Typical workflow:**

1. **Auto-fetch:** Call `auto-fetch-tools` with the provider type and assistant ID. This discovers the agent's tools and generates mock input/output data.
2. **Enable:** Once auto-fetch completes, call `enable-mock-tools` to activate mock mode on the provider.
3. **Test:** Run your scenarios. The agent returns mock tool responses instead of calling live integrations.
4. **Disable:** Call `disable-mock-tools` to restore the original tool configuration.

All lifecycle endpoints return a `progress_id`. Poll the corresponding progress endpoint until `status` is `completed` or `failed`.

### Which API should I use?

Use v2 for new integrations. The nested structure makes provider configuration easier to understand and maintain. Existing v1 integrations continue to work without changes.
