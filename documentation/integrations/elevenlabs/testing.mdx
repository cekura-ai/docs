...
With auto-sync enabled, Cekura automatically pulls your agent's prompt from ElevenLabs every 30 seconds and updates the description field—no manual updates required.

The feature syncs the prompt from your ElevenLabs conversational AI agent configuration.

To enable auto-sync, toggle on **Auto-sync Prompt** in the ElevenLabs voice integration settings on your Agent Settings page.

<Note>
Auto-sync requires a valid ElevenLabs API Key and Assistant ID. If credentials become invalid or the agent is deleted, auto-sync will automatically disable itself.
</Note>
