...
Every save to a scenario creates a version record. You can view past versions, restore an earlier configuration, and label important versions for reference.

<Tip>
In the dashboard, open the **Edit Evaluator** page and click **Version History** in the sidebar to view, compare, and restore versions.
</Tip>
...
<Tabs>
  <Tab title="CLI">
    ...
    cekura scenarios history 9876

    # Restore a previous version
    cekura scenarios restore 9876 --history-id 42

    # Label a version
    cekura scenarios edit-history 9876 --history-id 42 --version-name "before refactor"
    ```
  </Tab>
  ...
  <Tab title="SDK">
    ```python
    # View version history
    history = client.scenarios.history(scenario_id=9876)
    for record in history["results"]:
        print(record["version_id"], record["history_date"], record.get("version_name"))

    # Restore a previous version
    client.scenarios.restore(scenario_id=9876, history_id=42)

    # Label a version
    client.scenarios.edit_history(scenario_id=9876, history_id=42, version_name="before refactor")
    ```
  </Tab>
</Tabs>
...
- `metrics_snapshot` — which metrics were assigned at that version

You can also pass `version_name` when creating or updating a scenario to label that version immediately.
