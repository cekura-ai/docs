...
<ParamField path="condition" type="string | integer">
  ...
  **Special cases**:
<<<<<<< HEAD
  - When `id` is 0, the condition must be `"FIRST_MESSAGE"` (represents the first message)
||||||| 4fe89cc
...
<<<<<<< HEAD
  **Note**: For the first message (`id: 0`, `condition: "FIRST_MESSAGE"`), `fixed_message` must be `true`.
||||||| 4fe89cc
...
<<<<<<< HEAD
The condition with `id: 0` is special—it represents the **first message** the testing agent sends to start the conversation. This condition has specific requirements:

- **`condition`** must be set to `"FIRST_MESSAGE"`
- **`id`** must be `0`
- **`fixed_message`** must be `true`
||||||| 4fe89cc
...
<<<<<<< HEAD
If your agent speaks first and you want the testing agent to wait before responding, you can leave the `action` field empty:
...
<Tip>
The first condition must always have `condition: "FIRST_MESSAGE"`, `id: 0`, and `fixed_message: true`. The `action` field contains the exact message to send, or can be empty if the main agent speaks first.
</Tip>
||||||| 4fe89cc
<Tip>
...
Conditional actions are validated when you create or update an evaluator. The following rules are enforced:

<AccordionGroup>
  <Accordion title="First Message Requirements" icon="play">
    The first condition in your conditions array must:
    - Have `condition` set to `"FIRST_MESSAGE"`
    - Have `id` set to `0`
    - Have `fixed_message` set to `true`

    The `action` field can contain the message to send, or be empty if your agent speaks first.
  </Accordion>

  <Accordion title="Unique Condition IDs" icon="fingerprint">
    Each condition must have a unique `id`. Duplicate IDs will cause a validation error.
  </Accordion>

  <Accordion title="Action Content Requirements" icon="message">
    Actions cannot be empty, whitespace-only, or contain only special characters—except for the first message (`FIRST_MESSAGE`), which may have an empty action when your agent speaks first.
  </Accordion>

  <Accordion title="Supported Tags Only" icon="tags">
    Only supported XML tags are allowed in action content. Using unsupported tags will cause a validation error. See the [Supported Tags](#supported-tags) section for the complete list.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="First Message Not Sending" icon="message">
    ...
    **Solutions**:
<<<<<<< HEAD
    - Ensure the first condition has `condition: "FIRST_MESSAGE"`, `id: 0`, and `fixed_message: true`
    - Verify the action field contains the message to send (or is empty if your agent speaks first)
||||||| 4fe89cc
...
>>>>>>> main
  ...
  <Accordion title="Validation Errors" icon="circle-exclamation">
    **Issue**: You receive a validation error when creating or updating an evaluator.

    **Common causes and solutions**:
    - **First message format**: The first condition must have `condition: "FIRST_MESSAGE"`, `id: 0`, and `fixed_message: true`
    - **Duplicate IDs**: Each condition must have a unique `id` value
    - **Empty action**: Non-first-message conditions must have meaningful content in the `action` field
    - **Unsupported tags**: Only use tags listed in the [Supported Tags](#supported-tags) section
  </Accordion>
</AccordionGroup>
