...
Cekura places SIP calls through Twilio's Virginia (`us1`) region. If your SIP endpoint is behind a firewall, allow:

| Traffic | Source | Ports |
|---|---|---|
| RTP audio (Twilio global media range) | `168.86.128.0/18` | UDP `10000` – `60000` |
| SIP signaling (Twilio Virginia) | `54.172.60.0/30` | UDP/TCP `5060`, TLS `5061` |

Both are required: if signaling is allowed but the global media range is not, calls connect with no audio. Twilio also asks you to allow the signaling ranges for all of its regions, so calls survive a regional failover. See [IP Whitelisting: SIP Calls](/documentation/advanced/ip-whitelisting#sip-calls) for the full list, a setup checklist, and links to Twilio's documentation.
...
<Note>
  If your SIP endpoint is behind a firewall, allow Twilio's global media IP range and its SIP signaling IPs. Cekura places SIP calls through Twilio. See [Firewall Requirements](#firewall-requirements).
</Note>
