...
<Note>This section covers exporting and sharing *observability* (live production call) data. For reports on simulation test runs and evaluation results, see [How can I access an overall report of the results?](#how-can-i-access-an-overall-report-of-the-results).</Note>
...
<Note>Slack notifications are run-completion alerts — the bot posts when an evaluation finishes so your channel can click through to individual results. For a comprehensive self-service quality report covering scenario outcomes, metric scores, and improvement suggestions, see [Generating a Quality Report](#how-can-i-access-an-overall-report-of-the-results).</Note>
...
### 1. Accessing Overall Reports in the Dashboard
...
### 2. Generating a Quality Report via Claude Code
For a self-service end-to-end quality report, use the `/cekura-report` command in [Claude Code with the Cekura plugin](/mcp/claude-code-guide). This generates a comprehensive analysis of your evaluation runs — covering scenario outcomes, metric scores, and improvement suggestions — from a natural-language interface without navigating the dashboard UI. See the [Claude Code guide](/mcp/claude-code-guide) for setup instructions.

### 3. Ad-hoc Reports via Cekura Agent
The [Cekura Agent](/documentation/guides/cekura-agent) (available in the dashboard sidebar) lets you query your evaluation results in plain English without navigating through the UI — for example: *"Show me the results from the last run"*, *"What metrics failed most often for agent 5?"*, or *"Generate a report for agent X and highlight the failures."* No additional setup is required.
...
## How do ElevenLabs and Cartesia compare for telephony use cases in terms of voice quality, speed, and cost?

<Note>
ElevenLabs and Cartesia are the **only two voice providers** available in Cekura's testing personality Voice Provider dropdown — no other TTS providers appear there. See [Creating Custom Personalities](/documentation/advanced/creating-custom-personalities) for configuration steps.
</Note>

Both ElevenLabs and Cartesia are supported TTS providers within Cekura and are the most common choices for production telephony voice agents, but they serve different priorities:

- **ElevenLabs** is widely regarded as the quality leader — it produces highly expressive, natural-sounding voices and is the best choice when voice realism is the top priority.
- **Cartesia (Sonic)** is optimised for speed and efficiency. It generally offers lower latency and higher cost-effectiveness, making it a strong choice for high-volume deployments where Time-to-First-Audio (TTFA) matters more than maximum voice fidelity.

In practice, the choice between them is a quality-vs-latency trade-off: ElevenLabs for premium realism, Cartesia when you need faster, cheaper synthesis.

When testing voice agents in Cekura, note that the TTS provider used by the *Testing Agent* affects the simulated caller's voice, not your Main Agent's voice. For most evaluation workflows, the workflow logic and accent handling are more critical to test coverage than which TTS the Testing Agent uses. See [Accent Testing](/documentation/guides/testing-agents/accent-testing) for how to configure custom voice personalities for specific accent variants.


## Can new languages be added to Cekura's predefined testing personalities?

Yes. If you need a predefined personality for a language that is not yet in the default set, contact the support team and it can be added on request at no additional cost.

For fully self-service configuration, you can also create your own personality in any language by selecting ElevenLabs as the voice provider and entering a voice ID from the public [ElevenLabs Voice Library](https://elevenlabs.io/voice-library) — no ElevenLabs account required. See [Creating Custom Personalities](/documentation/advanced/creating-custom-personalities) for setup steps.
...
<Note>This section covers credit and billing usage. For reports on simulation test run quality and evaluation results, see [How can I access an overall report of the results?](#how-can-i-access-an-overall-report-of-the-results).</Note>
...
## Where can I check Cekura's service status or subscribe to incident notifications?

Cekura maintains a public status page at [status.cekura.ai](https://status.cekura.ai). The status page shows real-time uptime for the Cekura platform and records past incidents with timelines.

To receive notifications when an incident is opened or resolved, use the **Subscribe** button on the status page to enter your email address or configure a webhook. If you do not see a subscribe option, try refreshing the page.
