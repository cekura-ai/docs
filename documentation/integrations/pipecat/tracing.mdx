...
**What the SDK captures by default** (no OTel instrumentation required):

- Complete agent-side conversation transcripts with per-turn UTC timestamps
- Tool/function calls with inputs and outputs
- Custom metadata (via `set_custom_metadata()`)
- Dual-channel audio recording (agent + user) — observability mode only; not recorded in testing mode
- Session logs

**What OpenTelemetry adds** (optional — the SDK configures OTel automatically when `enable_tracing=True` is set on the PipelineTask; no manual span-wrapping required):

- Per-component latency breakdown: STT, LLM, and TTS latency spans
- Full conversation and turn spans for detailed trace waterfall views

Tool calls, custom metadata, and transcripts are captured automatically by the SDK in both observability and testing modes — OTel is optional and adds the per-component latency breakdown when enabled.
