...
When a user with a matching email domain requests to join your organization, Admins receive a notification. Join requests start with no project access—the Admin chooses which projects to grant when approving.

<Steps>
  <Step title="Review Pending Requests">
    Navigate to **Settings** > **Team** and locate the pending join request.
  </Step>

  <Step title="Approve and Assign Projects">
    Click **Approve** to open the approval dialog. Select the specific projects the new member should have access to, or leave empty to grant organization-level membership only.
  </Step>

  <Step title="Confirm Approval">
    Click **Confirm** to complete the approval. The user receives organization membership with access to only the projects you selected.
  </Step>
</Steps>

<Tip>
Leave the project selection empty to add the member to the organization without project access. You can assign projects later—useful when you need to coordinate with team leads before granting permissions.
</Tip>
