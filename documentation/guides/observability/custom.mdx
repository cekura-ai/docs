  ...
  <Tab title="Bland AI">
    <Tabs>
      <Tab title="No Code">
        ...
        Cekura can pull completed Bland calls every 30 seconds without requiring a webhook.

        <Steps>
          <Step title="Connect your Bland agent">
            In Cekura, create or open an agent, select **Bland AI** as its provider, and enter the Bland API key and Persona ID.
          </Step>
          <Step title="Enable automatic call import">
            In the provider settings, enable **Auto-fetch Production Calls** and save the agent.

            <img src="/images/bland/auto-fetch-production-calls.png" alt="Auto-fetch Production Calls enabled in Cekura Bland AI provider settings" />

            Cekura checks Bland for new completed calls every 30 seconds. The option is disabled by default.
          </Step>
          <Step title="Test your integration">
            Complete a call with the Bland agent. The call should appear in **Observability → Calls** within 30 seconds.
          </Step>
        </Steps>
      </Tab>
    </Tabs>
