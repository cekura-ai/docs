...
- **LLM:** Smaller, faster models such as GPT-4o mini balance response quality with reduced generation time compared to larger variants.
- **TTS:** ElevenLabs provides high-quality synthesis with low time-to-first-audio.
...
## What does "Create Evaluator from Call" actually do, and can I use it to test a different STT provider with my original audio?

When you use the "Create Evaluator from Call" (or "Create Scenarios from Call") feature on a production call log, Cekura reads the transcript of that call and generates a new structured evaluator that recreates the same conversation flow, intent, and expected outcomes. The resulting evaluator can then be run repeatedly as a regression test.
