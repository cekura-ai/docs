...
## Is there a way to duplicate agents, metrics, and other configurations between projects to support separate development and production environments, or must they be recreated manually?

Currently, Cekura allows the duplication of **evaluators** across projects, but other components like agents and metrics cannot be directly duplicated through the UI at this time. To manage separate environments (e.g., development and production), we recommend the following strategies: 

### 1. Recommended Approach: Single Project with Multiple Agents
Instead of creating separate projects, the most efficient way to manage environments is to keep separate agents (e.g., 'Agent-Dev' and 'Agent-Prod') within a **single project**. This allows you to reuse metrics, evaluators, and personalities across both agents without the overhead of keeping them in sync across projects. 

**Handling Webhooks:** While Cekura currently supports one webhook URL per project, you can use the `agent_id` included in the webhook payload to route the data to your respective development or production servers on your backend. This is often a cleaner approach than managing multiple projects.

### 2. Programmatic Setup via API
If you prefer to maintain separate projects, you can automate the creation and synchronization of agents, metrics, and configurations using the Cekura API to avoid manual recreation. Useful resources for automation include:
- **Cekura API:** Use the [Agent Creation API](https://docs.cekura.ai/api-reference/test_framework/create-agent) to set up agents programmatically.
- **MCP Server:** Leverage the [Cekura MCP Server](https://docs.cekura.ai/mcp/overview) for faster integration.
- **Claude Skills:** Use [Cekura Claude Skills](https://github.com/cekura-ai/cekura-skills) to manage configurations through AI-assisted workflows.

### 3. Using Metadata for Environment Context
If you need to pass environment-specific information (like recording URLs or environment tags) to your metrics, you can use the `metadata` field in your call objects. This metadata can then be accessed within your metrics for evaluation: 
- [Python Metrics Metadata](https://docs.cekura.ai/documentation/key-concepts/metrics/python-metric#metadata)
- [LLM as a Judge Metadata Variables](https://docs.cekura.ai/documentation/key-concepts/metrics/llm-judge-available-variables#metadata-)
