...
| Method | Endpoint |
|--------|----------|
| `PATCH` | `https://new-prod.cekura.ai/test_framework/v1/scenarios-external/:scenario-id/` |
...
```bash Curl
curl -X PATCH https://new-prod.cekura.ai/test_framework/v1/scenarios-external/{scenario_id}/ \
...
def update_evaluator(api_key, scenario_id, **kwargs):
    url = f'https://new-prod.cekura.ai/test_framework/v1/scenarios-external/{scenario_id}/'
...
async function updateEvaluator(apiKey, scenarioId, updateData) {
  const url = `https://new-prod.cekura.ai/test_framework/v1/scenarios-external/${scenarioId}/`;
