...
<Note>
**No WebSocket support?** If your chatbot runs in a serverless environment (e.g., Vercel) and doesn't expose a persistent WebSocket endpoint, a standard HTTP API endpoint also works as an alternative — Cekura can send and receive messages over HTTP. See the *Custom WebSocket* tab below for details.
</Note>
...
<Note>
**No WebSocket support?** If your chatbot is hosted in a serverless environment (e.g., Vercel) that doesn't expose a persistent WebSocket endpoint, a standard HTTP API endpoint can work as an alternative — Cekura can send and receive messages over HTTP. Let the Cekura team know your endpoint URL and message schema to enable this.
</Note>
