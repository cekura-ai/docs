...
## Can I use the Pipecat SDK (`observe_pipeline`) and the direct observability API (`/observability/v1/observe/`) together for the same session?

No — the Pipecat SDK (`observe_pipeline()` / `observe_and_create_task()`) and the direct observability endpoint (`POST /observability/v1/observe/`) are **alternative, mutually exclusive** integration paths — they cannot be combined for the same session.
