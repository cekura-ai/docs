...
## Viewing Expected Mock Tool Calls

When you use Auto-Fetch and auto-generate evaluators for an agent with mock tools, Cekura stores the expected mock tool calls for each evaluator. You can view these expectations in two ways:

**In the Dashboard:**
Expand an evaluator row to see the **Expected Mock Tools** section. Each entry shows the tool name and the expected input and output JSON.

**Via API/MCP:**
The evaluator retrieve endpoint returns a `generated_mock_tool_entries` field containing the expected mock tool calls. Each entry has this shape:
...
```json
{
  ...
  "new_entry": {
    ...
    "output": {"name": "Alex Chen", "plan": "pro"}
  }
}
```

This lets you verify that auto-generated evaluators have the correct mock tool expectations before running tests.
