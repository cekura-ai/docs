...
<Tabs>
  ...
  <Tab title="SDK">
    ...
    client.test_profiles.create(
        ...
        information={"customer_name": "Alex", "account_id": "A-552"},
    )
    client.test_profiles.update(7, name="Returning customer (EU)")
    ```
  </Tab>
</Tabs>
...
Dynamic variables let you parameterize a scenario at run time (e.g. customer name, account ID). Their values come from a [test profile](#test-profiles): attach one to the scenario, or pick profiles when you start the run. Each profile passed multiplies the run count.

<Tabs>
  <Tab title="CLI">
    ```bash
    cekura run start --agent-id 123 --scenario-ids 101 --mode text --test-profile-ids 7
    ```
  </Tab>

  <Tab title="SDK">
    ```python
    client.scenarios.run_text(agent_id=123, scenarios=[101], test_profile_ids=[7])
    ```
  </Tab>
</Tabs>
