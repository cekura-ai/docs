...
You can also check whether a metadata key exists or is absent using `null` values:

```json
{ "field": "metadata.ringing_duration", "op": "neq", "value": null }
```

This returns call logs where the `ringing_duration` key **exists** in the metadata. To match call logs where the key is **absent**, use `eq` with `null`:

```json
{ "field": "metadata.ringing_duration", "op": "eq", "value": null }
```
