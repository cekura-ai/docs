...
## Can I simulate multiple speakers in a single test case?

Yes. Use [Conditional Actions](/documentation/key-concepts/evaluators/conditional-actions) to script each speaker's lines as separate `fixed_message: true` conditions. The Testing Agent delivers them in sequence, giving the appearance of different speakers taking turns.

**Without audio attachments:** The Testing Agent speaks all lines in its own TTS voice. There is no voice differentiation between speakers — the conversation structure is reproduced but all utterances sound the same.

**With distinct voices per speaker:** Attach a pre-recorded audio clip to each `fixed_message: true` condition where a specific speaker should be heard. Each clip plays back when that condition fires, giving each speaker a distinct voice. See [Attached Audio](/documentation/key-concepts/evaluators/conditional-actions#attached-audio) for the upload flow.

Per-condition TTS voice selection (changing the synthetic voice for individual conditions without uploading audio) is not yet supported and is on the roadmap.

### Evaluating speaker-aware agent behaviors

Even without ground-truth speaker diarisation in the transcript view, you can evaluate how your agent handles multi-speaker input:

- Pass candidate diarisation output (speaker labels and timing) into the run via call metadata.
- A [Python metric](/documentation/key-concepts/metrics/python-metric) or [LLM Judge metric](/documentation/key-concepts/metrics/llm-judge-metric) can then compare the agent's output against those labels, transcript timing, or any ground-truth diarisation you supply.

This lets you verify behaviors like speaker recognition (did the agent address the right person?) or fact attribution (did the agent remember which speaker said what?) without requiring the platform to natively diarise multi-speaker audio.
