...
<AccordionGroup>
  <Accordion title="Run evaluations from CI">
    ...
    3. In your pipeline:
       ...
       `--wait` polls the result until it is `completed`, `failed`, `timeout` or `cancelled`. `--fail-on-threshold 1.0` exits `1` unless every run passed; lower it (e.g. `0.9`) to allow some failures. The command exits `2` when it cannot start the run or the wait times out (`--timeout`, default 3600 seconds).

       For a committed test suite, use `cekura run spec` with the same flags — see [Tests-as-Code](/documentation/guides/testing-agents/tests-as-code). [Result statuses and gating](/documentation/guides/testing-agents/result-statuses) explains what each status and count means.
  </Accordion>
