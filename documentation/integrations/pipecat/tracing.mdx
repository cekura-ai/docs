...
export const pipecatPrompt = [
  ...
  "1. Install the SDK: pip install cekura[pipecat]==1.4.0",
...
```bash
pip install cekura[pipecat]==1.4.0
```
...
### Transcript, Traces, and Logs Tabs

When trace data is available, the run detail view shows three tabs:

- **Transcript**: The conversation transcript with timestamps. With traces available, you get playback controls and can jump directly to the corresponding trace span.
- **Traces**: A flamegraph visualization of all trace spans (STT, LLM, TTS, tool calls, etc.). Click any span to seek the audio player to that point. The playhead follows audio playback in real time.
- **Logs**: A chronological view of trace spans showing timestamps, span types, durations, and provider/model information.

The Traces and Logs tabs appear automatically when the run has trace data from the SDK integration.
...
Stops all capture mid-call and prevents data from being sent to the Cekura backend.
