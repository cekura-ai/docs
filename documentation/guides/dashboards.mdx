...
Quick Add lets you add preconfigured widgets to your dashboard with a single click. When creating a widget, the Quick Add section shows a list of predefined templates.

Each template includes preset configurations for the field, chart type, and aggregation settings. Selecting a template immediately adds the widget to your dashboard with previewed data.

<Note>
Some widget types, such as `stat` widgets for KPI display, are only available through Quick Add and cannot be created manually.
</Note>
...
| `stat` | KPI statistics with trend indicators | Displaying key metrics with period-over-period comparisons |

<Note>
The `stat` chart type can only be added through the [Quick Add](#quick-add) feature. It is not available in the manual chart type selector.
</Note>

### Stat Widgets

Stat widgets display KPIs in a compact row format. Each widget shows multiple metrics side by side:

- **Label** — The metric name
- **Value** — The current value
- **Change indicator** — Percentage change from the prior period, with a trend arrow

Trend indicators are color-coded:
- Green with an up arrow indicates positive change
- Red with a down arrow indicates negative change

For latency metrics, the color logic is inverted — lower latency is better, so a decrease shows as green and an increase shows as red.

Stat widgets span the full width of the dashboard and have no title bar.
...
**Stat widget** — KPI data points with trend indicators:

```json
[
  { "label": "Total Calls", "value": 1250, "change": 12.5, "format": "number" },
  { "label": "Success Rate", "value": "94.2%", "change": 3.1, "format": "number" },
  { "label": "Average Latency", "value": "245ms", "change": -8.3, "format": "duration" }
]
```

The `change` field represents the percentage change compared to the prior period. A `null` value indicates no comparison data is available.
