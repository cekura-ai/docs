...
In the automated flow (both voice and chat endpoints), Cekura creates the LiveKit room **and** dispatches your agent. During dispatch, Cekura populates the agent's **job metadata** with test context that your agent code can use at runtime.
...
| `text_mode` | boolean | Only for chat runs. |
...
async def entrypoint(ctx: JobContext):
    ...
    is_text_mode = job_metadata.get("text_mode", False)

    # Use the variables as per your needs
```

<Note>
**`ctx.job.metadata`** contains test context from Cekura (scenario ID, run ID, test profile data, text mode flag). This is different from **`ctx.room.metadata`** which contains the LiveKit Config JSON — see the [room metadata example above](#configure-livekit-credentials).
</Note>
