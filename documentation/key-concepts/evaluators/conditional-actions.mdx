...
<AccordionGroup>
  <Accordion title="<background_noise> - Background Sounds" icon="waveform-lines">
    ...
    **Attributes:**
    - `sound`: Sound preset name or custom URL (required) - see supported sounds below, or provide an `http://` or `https://` URL for a custom sound file
...
<<<<<<< HEAD
    | `beep` | Beep tone |
    | `cough1` | Cough sound (variant 1) |
    | `cough2` | Cough sound (variant 2) |
    | `off` | Disable sound |

    You can also use a custom sound file by providing a full URL:
    ```json
    {
      "action": "<background_noise sound=\"https://example.com/custom-ambience.wav\">Hi there</background_noise>"
    }
    ```
||||||| f12dceb
=======
...
>>>>>>> main
  ...
  <Accordion title="<noise> - Play Sound Effects" icon="music">
    ...
    **Supported sound presets:**
    ...
    ```json
    {
      "action": "<noise sound=\"https://example.com/custom-sound.wav\" />"
    }
    ```
  </Accordion>
