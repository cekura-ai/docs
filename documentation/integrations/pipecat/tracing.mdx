...
- For [Pipecat Chat](/documentation/integrations/pipecat/chat-testing) runs, RTVI enabled on the pipeline task (Pipecat's default) and `runner_args` passed to the tracer

<Note>
**Text sessions.** When Cekura runs a chat simulation it sets `cekura_run_mode: "text"` in the session body. The SDK reads it from `runner_args` and, for that session only, captures typed user turns, skips audio recording and storage setup (the webhook reports `recording_unavailable_reason: "text_session"`), and reports function calls over RTVI at full detail. Voice sessions carry no marker and behave exactly as before.
</Note>
