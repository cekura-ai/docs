...
|------|-----------------------|
| Developer | 10 |
| Paid plans | Configurable — the platform supports **2,000+ concurrent calls** |

<Note>
  If you need a higher concurrency limit than your current plan allows, contact [support@cekura.ai](mailto:support@cekura.ai) and the team can increase your limit directly. No formal plan change is required — the team adjusts the limit for your account on request.
</Note>

<Note>
  When running multiple evaluators simultaneously, each run moves through several statuses in the dashboard UI — **Running** (queued, not yet started), **Waiting for call** (ready; your system should dial for outbound evaluators), **In progress** (call is active), and **Completed**. The label "Running" is a known naming quirk — it does **not** mean the call is active. For the full status reference, see [Understanding Run Statuses](/documentation/guides/testing-agents/outbound-evaluators#understanding-run-statuses).
</Note>
