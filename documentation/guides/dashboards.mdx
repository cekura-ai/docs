...
| `metric_evaluations.confidence` | integer | Judge confidence score (0-100) |
...
<Tip>
Filter by confidence to spot potential false negatives — calls where a metric passed but the judge was uncertain. For example, to find calls where "Hallucination" passed with confidence below 70:

```json
{
  "operator": "same_row",
  "conditions": [
    { "field": "metric_evaluations.metric.name", "op": "eq", "value": "Hallucination" },
    { "field": "metric_evaluations.value", "op": "eq", "value": true },
    { "field": "metric_evaluations.confidence", "op": "lt", "value": 70 }
  ]
}
```
</Tip>
