...
<Steps>
  <Step title="Set Up WebSocket Server">
    ...
    **Authentication:**
    ...
    | `X-Custom-Provider-Api-Key` | Your custom provider API key, if configured in agent settings. |
  ...
  <Step title="Configure in Cekura">
    ...
    4. (Optional) Enter a **Custom Provider API Key** for authentication
    5. Save configuration
    ...
    **Custom Provider API Key (Optional):**
    ...
    If your WebSocket server requires API key authentication, configure a Custom Provider API Key in agent settings. Cekura sends this key in the `X-Custom-Provider-Api-Key` header, allowing your server to authenticate connections alongside the `X-VOCERA-SECRET` header.
  </Step>
