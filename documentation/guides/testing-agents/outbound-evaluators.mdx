...
base_url = "https://new-prod.cekura.ai/test_framework"
...
def monitor_runs(response_json: Dict, func: Callable, call_func: bool = False) -> None:
    ...
    while run_ids:
        ...
        for run in runs_list:
            ...
            if run["status"] in ["pending", "in_progress"] and call_func:
                if run.get("outbound_number"):
                    func(run["outbound_number"])  # Call function immediately
                run_ids.remove(str(run["id"]))
            
            if not call_func:
                phone_number = run.get("outbound_number", "No phone number")
