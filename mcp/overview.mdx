...
Cekura supports MCP OAuth for Claude clients. Instead of manually configuring an API key, you can authenticate through an OAuth flow that automatically issues a 30-day token.
...
<Steps>
  ...
  <Step title="Authorization Complete">
    Cekura issues a token and redirects back to Claude. Your MCP connection is ready to use.
  </Step>
</Steps>
...
| Refresh | Re-authenticate through the OAuth flow before the token expires |
