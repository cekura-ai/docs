...
<AccordionGroup>
  ...
  <Accordion title="Plugin or tools do not appear">
    Restart Claude Desktop, confirm the **cekura** plugin is installed under **Customize → Plugins**, and (for a connector) verify the URL matches your workspace region. The US endpoint is the default; India and EU endpoints are listed in [manual setup](/mcp/mcp-only#regional-endpoints).
  </Accordion>
