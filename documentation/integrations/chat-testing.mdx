...
<Steps>
  ...
  <Step title="Wait until the run is ready">
    ...
    If the scenario has an opening line, the simulated user speaks first — it's the first `transcript_object` entry. Otherwise you speak first.
  </Step>
  ...
  <Step title="Submit a turn">
    ...
    <Info>
    Submitting a new turn while the previous one is still generating supersedes it — the older turn becomes `dropped` and only your newest message is answered.
    </Info>
  </Step>
  ...
  <Step title="Poll for the reply">
    ...
    Statuses: `generating` (keep polling), `ready` (reply in `message`), `dropped` (superseded — use the newer turn), `error` (submit again). Turns stay pollable after the run finishes.
  </Step>
