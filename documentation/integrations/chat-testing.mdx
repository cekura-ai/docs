...
<Steps>
  <Step title="Set Up WebSocket Server">
    ...
    **Authentication:**
    ...
    |--------|-------------|
<<<<<<< HEAD
    | `X-VOCERA-SECRET` | A secret value for authenticating the connection. By default, this is your Cekura API key. You can configure a custom value in **Agent Settings** → **Chatbot Integrations** → **Websocket Secret**. |
||||||| 1787782
...
<<<<<<< HEAD
    **Configuring a Custom Websocket Secret:**

    To use a custom secret instead of your Cekura API key:
    1. Go to **Agent Settings** → **Chatbot Integrations**
    2. Select **Custom** as the provider
    3. Enter your WebSocket URL
    4. In the **Websocket Secret** field, enter your custom secret value
    5. Save configuration

    Your custom secret will be sent in the `X-VOCERA-SECRET` header for all WebSocket connections.

    To authenticate incoming connections, validate the `X-VOCERA-SECRET` header against your configured secret:
||||||| 1787782
...
<<<<<<< HEAD
    ...
    4. (Optional) Enter a **Websocket Secret** for custom authentication
    5. Save configuration
||||||| 1787782
    3. Enter your WebSocket URL (starting with `wss://`)
    4. Save configuration
