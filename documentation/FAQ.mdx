...
## Why does my simulation show "No testing agent content in Cekura transcript"?

"No testing agent content in Cekura transcript" appears when Cekura's transcription pipeline recorded no speech on the testing agent's audio channel during a run. This does not necessarily mean the testing agent failed to speak — there are two distinct causes:

- **Language or personality configuration mismatch**: The assigned personality does not support the language of the simulation, preventing the testing agent from generating audio. Confirm that the personality's language setting matches the language your agent expects. If you are testing a non-English agent, ensure you have selected a personality with a matching language (e.g. a Japanese personality for a Japanese-language agent).

- **Transcription pipeline issue for the configured language**: The testing agent produced speech, but Cekura's transcription system failed to capture or process it for the specific language or audio configuration. This is more likely for non-English languages where transcription requires specific language settings that may not be active for all configurations.

A WER (Word Error Rate) of 100% on the Transcription Accuracy metric is a related signal — it indicates the transcription pipeline produced output that did not match the testing agent's intended speech, which can occur for the same reasons.

If you have confirmed the personality language is correct and are consistently seeing this error across multiple runs for a specific language, contact support with the affected run IDs so the transcription configuration can be investigated.
