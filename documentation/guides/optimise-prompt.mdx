...
iconType: "regular"
description: "Close the loop on agent quality. The Optimise Prompt button takes failing evaluators, diagnoses where the agent's prompt or config has gaps, applies edits, and re-validates — iterating until the agent passes. Supports VAPI, ElevenLabs, and self-hosted (pipecat + websocket) agents."
---
...
<Note>
**Optimise Prompt is BETA.** The optimiser edits a live agent (VAPI assistants via the VAPI API, ElevenLabs Conversational AI agents via the ElevenLabs API, self-hosted pipecat agents via the Cekura agent record, self-hosted websocket agents via source-file edits). Review proposed diffs before applying for the first few runs.
</Note>
...
| Phase | What happens |
|---|---|
| **Setup** | Resolves the agent's provider mode (VAPI, ElevenLabs, or self-hosted — pipecat vs. websocket sub-flavor), loads the system prompt and tool definitions. For self-hosted agents, asks once for the **redeploy command** so later iterations can restart the live process automatically. |
...
The optimiser supports four agent modes, resolved automatically during Setup from the agent's provider configuration on Cekura.

| Mode | Editable surfaces | How edits land |
|---|---|---|
| **VAPI** | System prompts on every squad member, tool function declarations, tool spoken `messages` (`request-start` / `request-complete` / `request-failed`), squad `model.toolIds`, handoff `destinations`. | VAPI PATCH — the live assistant picks up edits immediately. No redeploy step. |
| **ElevenLabs** | System prompt at `conversation_config.agent.prompt.prompt`, referenced standalone tools (name, description, webhook `api_schema` / client `parameters`), legacy inline tools, and built-in tool config flags (`end_call`, `transfer_to_agent`, etc.). ElevenLabs is single-agent — no squads, no per-member prompts, no spoken `request-start` / handoff `destinations`. | ElevenLabs API PATCH (`xi-api-key`) — edits land live. No redeploy step. |
| **Self-hosted · pipecat** | The Cekura agent record's `description` field (which the pipecat pipeline reads as the prompt source for this flavor) and mock-tool definitions. Business logic in the pipecat code remains out of scope. | Cekura PATCH on the agent + mock-tool records, followed by your **redeploy command** to restart the pipecat agent. |
| **Self-hosted · websocket** | The system prompt string constant in your source file, tool schemas, **and** orchestration code (conversation-history management, message wiring, state preservation, keepalive / retry plumbing). Business logic, auth code, and dependencies stay out of scope. A degenerate **offline** variant covers the "no live websocket reachable" case — prompt edits only, manually applied. | Direct edit on your source file, followed by your **redeploy command**. |

<Note>
**Retell is not currently supported by Optimise Prompt** and will be re-enabled in a future revision. LiveKit agents are routed through the self-hosted path.
</Note>
...
<Steps>
  ...
  <Step title="For self-hosted websocket agents — the live source file open in the IDE">
    The optimiser locates the system prompt by first checking the file open in your editor, then grepping the workspace for the prompt string constant. Files like `original_*.py`, `*.bak`, anything under `archive/` are skipped — the assistant will pause and confirm if there's ambiguity. This step does not apply to VAPI, ElevenLabs, or pipecat agents — their prompt source is fetched via API.
  </Step>
  <Step title="For self-hosted agents (pipecat or websocket) — a redeploy command">
    Provide the shell command that restarts your agent (e.g. `pkill -f my_agent && python my_agent.py &` for websocket, or your pipecat redeploy invocation). The assistant asks once at setup. Pass `"manual"` instead to gate each iteration on a manual restart. VAPI and ElevenLabs ignore this field — their edits land live and nothing needs redeploying.
  </Step>
</Steps>
...
| `mode` | auto-detected | Explicit override (`vapi` / `elevenlabs` / `self_hosted`) when detection is ambiguous. For `self_hosted`, also pass `self_hosted_flavor` (`pipecat` / `websocket`) when the router can't disambiguate. |
