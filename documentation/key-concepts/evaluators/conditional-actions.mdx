...
1. You upload an audio file to the scenario — optionally targeting a specific condition, or leaving it unattached for later reference.
...
3. Each clip gets a human-readable **name** (its `audio_id`). The name defaults to a slug of the filename (e.g. `greeting.mp3` → `greeting`) and must be unique per scenario, case-insensitively.
4. Reference a clip from any number of fixed-message actions by pasting its `<audio id="…"/>` tag — the same clip can play in several conditions.
5. When the condition fires, text, recordings, and tags run from left to right.
...
```bash Upload
# Attach a clip named "greeting" to condition 2
...
# Add a clip to the scenario without targeting a condition
curl -X POST "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/" \
  -H "Authorization: Bearer $CEKURA_API_KEY" \
  -F "name=hold-music" \
  -F "file=@hold-music.mp3"
# Reference it later from any action with <audio id="hold-music"/>
```

```bash List
# List every clip on the scenario and which conditions reference each one
curl "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/" \
  -H "Authorization: Bearer $CEKURA_API_KEY"
```

```bash Replace
# Swap the audio file for an existing clip (every reference picks it up)
curl -X PUT "https://api.cekura.ai/test_framework/v1/scenarios/39001/condition-audio/greeting/" \
  -H "Authorization: Bearer $CEKURA_API_KEY" \
  -F "file=@greeting-v2.mp3"
# Optionally rename: -F "name=greeting-v2"
```

```bash Delete
# Remove a clip and all its references from every action
...
<ParamField path="name" type="string">
  The clip's id — used in its `<audio>` tag and as its stored object name. Letters, digits, underscores and hyphens, max 64 characters; must be unique within the scenario, case-insensitively. Defaults to a readable slug derived from the filename (e.g. `greeting.mp3` → `greeting`).
</ParamField>
...
<ParamField path="action_template" type="string">
  Requires `condition_id`. The complete action with one `<audio />` marker where the new recording should be inserted.
</ParamField>

<ParamField path="replace_audio_id" type="string">
  Requires `condition_id`. Points that condition's existing tag at the new clip. To swap a clip's audio while keeping every reference to it, use `PUT condition-audio/{audio_id}/` instead.
</ParamField>
...
A scenario **cannot be run while any referenced clip is not `ready`** — the run is rejected with a clear message (`"still transcribing — wait for it to finish"` for pending/processing, `"failed to transcribe — retry or remove it"` for failed, or `"does not exist"` for a tag referencing a clip that hasn't been uploaded yet). This gate applies to interactive runs, scheduled (cron) runs, and email-triggered runs alike.
