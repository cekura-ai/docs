...
| Method | Endpoint |
|--------|----------|
| `GET` | `https://new-prod.cekura.ai/test_framework/v1/results-external/` |
...
{
    "count": 147,
    "next": "https://new-prod.cekura.ai/test_framework/v1/results-external/?agent_id=1&page=1",
    "previous": null,
    "results": [
        {
            "id": 185,
...
```bash Curl
curl -X GET "https://new-prod.cekura.ai/test_framework/v1/results-external/?agent_id=1" \
...
def list_results(api_key, agent_id=None, page=1, page_size=30):
    url = 'https://new-prod.cekura.ai/test_framework/v1/results-external/'
    headers = {
...
async function listResults(apiKey, agentId = null, page = 1, pageSize = 30) {
  const url = new URL('https://new-prod.cekura.ai/test_framework/v1/results-external/');
