...
| `agent_version_id` | no | Run against a saved version of the agent instead of the live one, e.g. `v12`. |
...
Everything that describes a test case lives in the file: instructions, metrics, personality, test data, and the Cekura-side `phone_number`. Settings that describe *this run* go on the request instead, so one committed file can run against different numbers, versions, or load levels without edits:
...
- **`mock_tool_names`** and **`agent_version_id`** work as they do on evaluator runs. Send `mock_tool_names: []` to mock no tools; leave it out to mock every tool configured on the agent.

Personalities and test profiles are chosen per test case in the file (see [Personalities](#personalities) and [Test data](#test-data)), so `personality_ids` and `test_profile_ids` are not accepted here. Neither are `scenarios`, `tags` or `folder_path` — the spec is the list of test cases. Sending any of them returns a `400` that says where the setting belongs, rather than silently ignoring it.

A dry run runs the same checks as the real run, so an override that would be rejected fails the dry run too. It echoes the overrides it applied under `plan.run_overrides`, and the numbers the run will use under `plan.agent_number` and `plan.outbound_phone_number`, so you can confirm them before placing any calls.
