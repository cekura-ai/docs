...
<Note>
  **Phone number pool.** Your effective concurrent call capacity is also capped by the number of phone numbers available in your Cekura account — Cekura needs a distinct number for each simultaneous call leg. For example, if your organisation limit is 50 but you only have 5 phone numbers imported, you can run at most 5 calls concurrently. To increase it, import additional numbers via your telephony provider. See the [Plivo](/documentation/key-concepts/phone-numbers/plivo) or [Twilio](/documentation/key-concepts/phone-numbers/twilio) import guides for instructions.
</Note>
