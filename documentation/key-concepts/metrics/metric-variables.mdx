...
icon: "brackets-curly"
iconType: "solid"
description: "Canonical reference for variables available in metric prompts and Python metric scripts"
---
...
Provider execution logs as plain text, one line per entry. Each line is prefixed with the entry's timestamp, source component, and severity level when available. Only populated when execution logs were captured for the call or run (`is_log_present` is true).

Useful for evaluating runtime behavior that the transcript alone does not capture — tool-call failures, latency spikes, pipeline errors, or provider-level events.

**Example value:**
```
[00:00 session info] Session started
[00:05 transcriber info] Final transcript | provider=deepgram confidence=0.98
[00:07 tools error] Tool call failed: get_order timed out after 5s
[00:09 session info] Session ended
```
