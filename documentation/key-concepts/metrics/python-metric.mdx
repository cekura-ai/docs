...
<AccordionGroup>
  ...
  <Accordion title="test_profile" href="#test-profile">
    **Availability:** ✅ Simulation | ❌ Observability
    ...
    Test scenario data configured for simulation runs. Python metrics receive a **flat merged view** — section keys (`main_agent_variables`, `testing_agent_variables`) are flattened away so you can read keys directly, regardless of which section they live in. Legacy flat profiles look identical.

    ```python
    # test_profile is only available if the run has an associated test profile
    test_profile = data.get("test_profile", {})
    # Sectioned profile example (flattened on delivery):
    #   {"company": "Cekura", "customer_type": "frustrated", "issue": "billing"}
    # — keys come from both main_agent_variables and testing_agent_variables.
