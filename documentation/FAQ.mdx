...
## How do I submit metric feedback or flag false positives programmatically via the API or CLI?

To submit thumbs-up/down votes and annotate expected values from a custom call-review pipeline, Cekura provides both a CLI and REST API.

**For test run metrics** (simulation runs):

- **CLI:** `cekura runs mark-metric-vote <run_id> --metric-id <id> --thumbs-down --feedback "reason"` — see the [CLI/SDK Runs reference](/cli-sdk/runs#vote-on-a-metric-result) for full options.
- **Python SDK:** `client.runs.mark_metric_vote(run_id=..., metric_id=..., thumbs_up=False, feedback="reason")`

**For production call logs (Observability)**, the workflow is two steps:

1. Add the call to a test set via the [Create Test Set from Call Log](/api-reference/test_framework/post-test_frameworktest-setscreate_from_call_log) endpoint.
2. Submit the metric vote via the [Vote on Call Log Metric](/api-reference/observability/vote-call-log-metric) API endpoint.

To review flagged calls and run the auto-improve workflow, navigate to **Labs** in the left panel, select the relevant metric, and annotate directly in the table. See the [Metric Lab guide](/documentation/guides/metric-lab) for the full annotation and auto-improve workflow.
...
## What does the daily report contain, and how can I add a dashboard snapshot to it?

Cekura's daily report automatically highlights the metrics that are most worth your attention:

- **Most changed metrics** — metrics whose scores shifted the most over the past two days.
- **Worst-performing metrics** — metrics with the highest failure rates in the same window.

These are selected automatically based on recent signal; you do not configure which metrics appear in the report itself.

### Adding a dashboard snapshot

You can attach a *Dashboard* snapshot to the daily report so it appears as a visual summary in the email PDF:

1. Create a dashboard in the **Overview** page (see the [Dashboards guide](/documentation/guides/dashboards) for setup steps).
2. Go to **Settings → Project → Integrations**.
3. Enable the option to add a dashboard snapshot to the daily report.

The snapshot is included in the next day's email alongside the metrics summary.

For help configuring who receives the daily report, see [How can I configure daily reports to be sent to a specific email address?](#how-can-i-configure-daily-reports-to-be-sent-to-a-specific-email-address-instead-of-all-project-members)
