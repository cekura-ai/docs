...
<Note>
  **Plivo is a phone number provider, not a system integration.** You use Plivo to obtain a phone number and then import it into Cekura — no Plivo API, SDK, or webhook configuration is required in your own calling system. Once the number is imported, testing works by placing regular phone calls. See the [outbound evaluators guide](/documentation/guides/testing-agents/outbound-evaluators) if your system is the one placing the calls during testing.
</Note>
