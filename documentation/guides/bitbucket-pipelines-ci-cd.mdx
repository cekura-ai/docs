...
Point the pipe at a different Cekura environment with `API_URL`, and use a key issued by that environment:
...
pipelines:
  branches:
    main:
      - step:
          name: Cekura tests (staging)
          script:
            - pipe: docker://public.ecr.aws/t8m3v5l2/bitbucket:1.0.0
              variables:
                CEKURA_API_KEY: $STAGING_CEKURA_API_KEY
                TAGS: "smoke"
                API_URL: https://staging.example.cekura.ai
      - step:
          name: Cekura tests (production)
          script:
            - pipe: docker://public.ecr.aws/t8m3v5l2/bitbucket:1.0.0
              variables:
                CEKURA_API_KEY: $CEKURA_API_KEY
...
<AccordionGroup>
  ...
  <Accordion title="The Step Fails With HTTP 401">
    The API key was rejected. Check that `CEKURA_API_KEY` is spelled the same in the pipeline file and in Repository variables, and that the key belongs to the environment `API_URL` points to.
  </Accordion>
