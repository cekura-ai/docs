...
| `sampling_enabled` | boolean | No | Enable sampling for this metric using the project-level sample rate. Defaults to false |
...
| `sampling_enabled` | boolean | Whether sampling is enabled for this metric |
...
def create_metric(
    ...
    if sampling_enabled is not None:
        data['sampling_enabled'] = sampling_enabled
...
async function createMetric(
  ...
  if (samplingEnabled !== null) data.sampling_enabled = samplingEnabled;
