...
Variables in this section are sent to your agent under test as dynamic variables. They populate `{{...}}` placeholders in your agent's system prompt and appear in provider-specific fields (VAPI `variableValues`, Retell `dynamic_variables`, ElevenLabs `dynamic_variables`, Pipecat `session_config`, LiveKit dispatch metadata, or custom HTTP headers).
...
Variables in this section are used by the evaluator to play a convincing caller. They shape the evaluator's instructions and context but are never sent to your agent.
