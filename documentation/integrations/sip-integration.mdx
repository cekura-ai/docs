...
<Note>
**Legacy flat profiles** still work — any `X-`-prefixed key in a flat `information` dict will be sent as a SIP header (preserving pre-sectioning behavior). New profiles should use the sectioned shape so persona keys (`customer_name`, `date_of_birth`, etc.) don't accidentally end up on the wire.
</Note>
