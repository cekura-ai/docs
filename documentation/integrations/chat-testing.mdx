...
<Steps>
  <Step title="Set Up WebSocket Server">
    ...
    **Messages your server sends to Cekura** (your agent's messages):
    ...
    Metadata-only message (no content):
    ```json
    {
      "metadata": {
        "session_id": "abc-123",
    ...
    <Info>
    Your server can include a `metadata` field (JSON object) in messages sent to Cekura. This metadata merges into the test run's metadata when the run completes. If multiple messages include the same key, the last value wins. Messages containing only `metadata` (no `content` or `role`) are stored without triggering a testing agent response.
    </Info>
