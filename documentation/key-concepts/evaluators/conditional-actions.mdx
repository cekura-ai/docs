...
<AccordionGroup>
  <Accordion title="<dtmf> - Send DTMF Tones" icon="grid">
    Send DTMF (touch-tone) digits. Supports two formats:
    ...
    **Inline Format:**
    ```json
    {
      "action": "Let me enter that now.<dtmf> Done entering."
    }
    ```

    The inline format places DTMF input between surrounding text without needing the `digits` attribute.

    **Attributes (XML format):**
    - `digits`: DTMF digits to send - e.g., "123", "456#", "*9"

    **Supported Characters:** `0-9`, `*`, `#`, `A`, `B`, `C`, `D`
