...
Test your Retell agents through text-based interactions. Chat tests run 10x faster and cost 90% less than voice tests—ideal for rapid iteration on conversation logic.
...
<Steps>
  <Step title="Configure Agent Settings">
    ...
    5. Click **Save Changes**.
       ...
       <Note>
       For **voice agents**, Cekura automatically creates a temporary chat agent at runtime using your voice agent's response engine, prompt, tools, model, and knowledge base. The temporary agent is deleted after the test completes. No manual setup in Retell required.
       </Note>
  ...
  <Step title="Run Chat Tests">
    ...
    4. Click **Run** to open the **Configure Run** dialog. Under **Connections**, select **Retell** under the **CHAT** section to conduct a chat run.
       ...
       <Note>Greyed out connections are not configured. Set them up in your [Agent Setting's **Provider**](/documentation/key-concepts/agents/Agent_Setup_Guide#chat-connection) panel if you want to run with SMS or WhatsApp.</Note>
...
- **Run Workflow Testing:** Even if you're building voice agents on Retell, running them in text mode is much faster and cheaper. Test your agent's workflow effectively using this approach—Cekura uses Retell's chat endpoint automatically.
