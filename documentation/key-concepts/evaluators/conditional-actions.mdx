...
<AccordionGroup>
  ...
  <Accordion title="<noise> - Play Sound Effects" icon="music">
    ...
    **Attributes:**
    - `sound`: Sound name (required) - supported: `office`, `beep`, `cough1`, `cough2`, `female-crying`, `male-crying`
    - `volume`: Volume level (optional)
    - `time`: Duration in ms (optional)

    The crying sounds are ~10 seconds of a person sobbing. Use them to check whether your agent notices a caller in distress and asks if everything is okay; shorten with `time` if needed.
