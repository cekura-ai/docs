...
**Important:** only one task should ever call `websocket.send()` at a time. Do **not** start a second keepalive loop inside a retry helper — two concurrent senders can corrupt WebSocket frames.
...
Set `ping_interval=None` when starting `websockets.serve` to stop the server from sending its own WebSocket ping frames. These are separate from the metadata keepalives above and add unnecessary control-frame traffic through the tunnel:
...
server = await websockets.serve(
    ...
    "0.0.0.0",
    8765,
    ping_interval=None,  # Cekura manages keepalive via its own ping interval
)
```