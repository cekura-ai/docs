...
Test profiles organize variables into two sections based on where they are used:
...
Variables in `main_agent_variables` are sent to your agent under test as dynamic variables. They populate `{{...}}` placeholders in your agent's system prompt and appear in provider-specific fields (VAPI `variableValues`, Retell `dynamic_variables`, ElevenLabs `dynamic_variables`, Pipecat `session_config`, LiveKit dispatch metadata, or custom HTTP headers).

Use this section for values your agent needs at runtime:
- Customer IDs or account numbers
- Session tokens
- Configuration flags

### Testing Agent Variables

Variables in `testing_agent_variables` are used by the evaluator to play a convincing caller. They shape the evaluator's instructions and context but are never sent to your agent.

Use this section for persona details the evaluator needs:
- The caller's name and personal information
- Appointment dates and order numbers
- Security question answers

This separation ensures your agent receives only the values it needs, while the evaluator has the full persona context to respond naturally.
...
When you create or edit a test profile, the dialog displays both sections with labeled fields:

1. **Main agent variables** — Dynamic variables sent to the agent under test
2. **Testing agent variables** — Persona and context used by the simulated caller

Add key-value pairs to each section based on where the variable should be used.
...
<Note>
Existing test profiles created before this feature will show all variables in the **Testing agent variables** section when opened. Move any variables that should reach your agent into **Main agent variables** before saving.
</Note>
...
In this example, `customer_id` is sent to the agent under test as a dynamic variable, while the persona details (name, email, order info) are used by the evaluator to play the role of a customer.
