...
Runs are asynchronous: the call returns once they are queued. To gate a merge on the outcome, poll the returned result until it reaches a terminal status and then check its `success_rate`. The [GitHub Actions](/documentation/guides/github-actions-ci-cd), [GitLab CI/CD](/documentation/guides/gitlab-ci-cd), and [Bitbucket Pipelines](/documentation/guides/bitbucket-pipelines-ci-cd) tutorials cover the polling and gating pattern in full — the only difference is which endpoint starts the run.
